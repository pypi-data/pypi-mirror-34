...
class   RTCManagerPlugin(TileBusProxyPlugin):
    ...
    def rtc_set_timestamp(self, timestamp):
    ...
    def rtc_get_secs(self):
        ...
        err, secs = self.rpc(0xAB,0x04, result_format="LL")
        if err:
            raise HardwareError("Error getting seconds from RTC", code=err)
        return secs
    ...
    def _set_time(self, dt):
        ...
        hund    = _int2bcd(dt.microsecond / 10000)
        second  = _int2bcd(dt.second)
        minute  = _int2bcd(dt.minute)
        hour    = _int2bcd(dt.hour)
        day     = _int2bcd(dt.day)
        month   = _int2bcd(dt.month)
        year    = _int2bcd(dt.year - 2000)
        weekday = _int2bcd(int(dt.strftime("%w")))
        timenow = struct.pack("<LBBBBBBBB", 0, hund, second, minute, hour, day, weekday, month, year)
    ...
    def _get_time(self):
        err, hund, second, minute, hour, day, wday, month, year = self.rpc(0xAB,0x02, result_format="LBBBBBBBB")
        if err:
            raise HardwareError("Error getting time from the RTC", code=err)
