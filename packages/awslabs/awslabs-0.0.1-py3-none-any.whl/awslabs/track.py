...
def load_challenge(track_id, track_name, challenge_id):
...
class Track(object):
    ...
    def start(self):
        if self.get('current') is '0' or self.get('current') is '':
            ...
            click.echo(click.style("# Track: " + self.name, fg='red', bold=True))
            click.echo(click.style("#################################", fg='red', bold=True))
            ...
            print("Track is in progress, use validate to check your current challenge or stop the Track.")
    ...
    def _go(self):
        try:
            ...
            click.echo("No next challenge found for {}. Have you finished this track?".format(self.id))
            exit()
        ...
        if challenge.validate():
            try:
                challenge = load_challenge(self.id,
                                           self.name,
                                           challenge_id(int(self.get('current'))+1))
            except:
                # if there is no next challenge, you're finished!
                self.save('current', int(self.get('current'))+1)
                click.echo("Congratulations, you finished the track: {}".format(self.name))
                exit()
            challenge.start()
