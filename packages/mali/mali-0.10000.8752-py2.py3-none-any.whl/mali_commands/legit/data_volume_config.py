...
class DataVolumeConfig(object):
    ...
    @property
    def is_local(self):
        return self.get_boolean('general', 'is_local')
    ...
    def object_store_config(self):
        ...
        if object_store_config.get('auth') == 'gcloud' and not object_store_config.get('bucket_name'):
            warnings.warn('When working with gcloud auth a bucket needs to be explicitly specified')
            del object_store_config['auth']
