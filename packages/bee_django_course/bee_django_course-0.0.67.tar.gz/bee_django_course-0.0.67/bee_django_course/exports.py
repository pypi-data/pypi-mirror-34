...
from .models import CourseSectionMid
...
def get_user_last_course_section(user, start_dt=None, end_dt=None):
    user_courses = user.usercourse_set.order_by('-created_at')
    if user_courses.exists():
        latest_user_course = user_courses.first()
        section_list = get_user_course_section_list(latest_user_course)
        if section_list.exists():
            if start_dt:
                section_list = section_list.filter(updated_at__gte=start_dt)
            if end_dt:
                section_list = section_list.filter(updated_at__lte=start_dt)
            list = section_list.filter(status=1)
            if list.exists():
                return list.first()
            else:
                finished = section_list.filter(status__in=[2, 4])
                if finished.exists():
