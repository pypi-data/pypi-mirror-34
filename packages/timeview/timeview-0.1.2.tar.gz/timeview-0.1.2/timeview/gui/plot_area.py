...
class DumbPlot(pg.GraphicsView):
    ...
    def buildLayout(self):
        ...
        self.layout.scene().sigMouseClicked.connect(
            self.onClick, QtCore.Qt.UniqueConnection
        )
        ...
        if event.double():
            ...
            # double click on view box implies add partition
                ...
                partitions = [
                    ...
                    if isinstance(view.renderer, rendering.PartitionEdit)
                ]
                # no partitions found, exiting
                if not partitions:
                    event.ignore()
                    return False
                x = self.main_vb.mapFromItemToView(self.main_vb, event.pos()).x()
                ...
                if len(partitions) == 1:
                    ...
                    logger.info(
                        "Multiple Partition Views detected, "
    ...
    def updateViews(self):
        ...
        for _, view_box in self.vbs.items():
