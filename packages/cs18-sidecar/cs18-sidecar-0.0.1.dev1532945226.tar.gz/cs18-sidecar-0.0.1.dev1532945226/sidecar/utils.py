...
def log_call(func):
    global calls_logger
    if not calls_logger:
        calls_logger = logging.getLogger('calls_logger')

    def func_wrapper(*args, **kwargs):
        bound_args = inspect.signature(func).bind(*args, **kwargs)
        bound_args.apply_defaults()

        args_list = ["{}: '{}'".format(k, v) for (k, v) in bound_args.arguments.items() if k != 'self']
        calls_logger.debug(func.__qualname__ + "(" + ", ".join(args_list) + ")")
