...
def predict_error_gaussian(y,y_est):
     ...
     Parameters
     ----------
     y: array_like, with shape (n_samples,), true response variable.
     
     y_est: array_like, with shape (n_samples, n_knots), the estimated label by the path.
     ...
     return np.linalg.norm(y_est.T - y.T,ord=2,axis=1)

def predict_error_binomial(y,prob,pos_label=1):
     ...
     prob: array_like, with shape (n_samples, n_knots), the estimated probability of the positive label in the path.
     ...
     """
     label = np.unique(y)
     if pos_label not in label:
          raise ValueError('The label of the positive class should be one of the labels in y!')
     else:
          neg_label = label[label != pos_label][0]
     # estimat y
     y_est = (prob >= 0.5) * pos_label
     y_est[y_est == 0] = neg_label
     # y_est_path - y
     err = (y_est.T - y.T)
     ...
     return np.mean(err!=0,axis=1)


def plot_pred_error_path(t_seq,pred_error_,beta_=None,beta_star=None):
     
     """
     Plot prediction error curve (e.g. regularization solution path)
     
     Parameters
     ----------
     t_seq: array, regularization solution path, with shape (n_knots,)
     pred_error_: array, prediction error, with shape (n_knots,)
     beta_: array, optional, default None
          regularization solution path, with shape (n_features,n_knots)
     beta_star: array, optional default None
          true signal vector, with shape (n_features,)
     
     """
     if len(t_seq) != len(pred_error_):
          raise ValueError('The dimension of t_seq and pred_error_ should be the same!')
     ind_t = np.argmin(pred_error_)     
     fig = plt.figure()
     ax = fig.add_axes([0,0,1,1])
     plt.rc('text', usetex=True)
     plt.plot(t_seq,pred_error_,'o-',label=r'error curve')  
     plt.axvline(t_seq[ind_t],color='red',linestyle='--',label=r'$t^{\star}$')
     plt.xlim([np.min(t_seq) * 0.5, np.max(t_seq) * 1.2])
     plt.ylim([np.min(pred_error_) * 0.5,np.max(pred_error_) * 1.5])
     plt.xlabel('regularization parameter t')
     plt.ylabel('prediction error')
     plt.grid(True)
     plt.title('Prediction Error Path')
     
     plt.legend(loc='upper right',fontsize=20)
     if beta_ is not None and beta_star is not None:
          beta_est = beta_[:,ind_t]
          if len(beta_est) != len(beta_star):
               raise ValueError('The row-number of beta_ should be equal to the dimension of beta_star !')
          est_err = np.linalg.norm(beta_est - beta_star)
          textstr = r'$\| \beta(t^{\star}) - \beta^{\star} \|_{2} = %.2f$' % (est_err)
          props = dict(boxstyle='round', facecolor='wheat', alpha=0.8,)
          plt.text(0.25, 0.5, textstr,horizontalalignment='center',verticalalignment='center', fontsize=14, bbox=props,transform=ax.transAxes)
...
class LBI():
     ...
     def Solution_path(self,ind_true=None):
          """ 
          Plot the Regularization solution path.
