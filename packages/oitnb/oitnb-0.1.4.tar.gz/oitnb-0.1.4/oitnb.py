...
# lib2to3 fork
from _oitnb_lib2to3.pytree import Node, Leaf, type_repr
from _oitnb_lib2to3 import pygram, pytree
from _oitnb_lib2to3.pgen2 import driver, token
from _oitnb_lib2to3.pgen2.parse import ParseError
...
CACHE_DIR = Path(user_cache_dir(_package_data['full_package_name'], version=__version__))

diff_prog = None
# original
quotes = ["'", '"', "'''", '"""']
messages = dict(
    NO_PATHS_GIVEN=['No paths given. Nothing to do', '.', ' 😴'],
    BANG_OK=["", 'Ok!', '✨ 🍰 ✨'],
    BANG_NOT_OK=["", 'Errors encountered', '💥 💔 💥'],
)
post_processors = []
...
def read_package_data(
    ...
    """Inject ointb configuration from oitnb.pon and __init__.py into defaults in `ctx`.

    Returns the path to a successfully found and read configuration file, None
    otherwise.
    """
    from .parseconfig import literal_eval, _package_data_parser
    ...
    if not value:
        user_data = Path(user_config_dir(_package_data['full_package_name'])) / (
            _package_data['full_package_name'] + '.pon'
        )
        if user_data.is_file():
            config.update(literal_eval(user_data.read_text()).get('default', {}))

        root = find_project_root(ctx.params.get('src', ()))
        path = root / '__init__.py'
        if path.is_file():
            config.update(
                _package_data_parser(path).get(_package_data['full_package_name'], {})
            )
    ...
    if ctx.default_map is None:
        ctx.default_map = {}
    for k in config:
        ks = k.replace('--', "").replace('-', '_')
        v = config[k]
        if isinstance(v, bool):
            v = str(v).lower()
        ctx.default_map[ks] = v
    return value
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Prefer black's single double-quotes over single single-quotes. Double-quotes "
        'are always preferred for triple-quoting, and for the empty string.'
    ),
)
@click.option(
    '--check',
...
@click.option(
    '--meld',
    is_flag=True,
    help="Don't write the files back, output to a temporary file and run meld.",
)
@click.option(
    '--fast/--safe',
    is_flag=True,
...
@click.option('--icons', is_flag=True, help='Do add the icons back in to the output')
@click.option(
    '--multi-line-unwrap',
    is_flag=True,
    help='undo pushing of trip-quoted multi-lines between parenthesis to their own lines',
)
@click.option(
    '-q',
    '--quiet',
...
def main(
    ...
    global diff_prog  # cludge because WriteBack is too simple
    write_back = WriteBack.from_configuration(check=check, diff=diff or meld)
    for msg in messages:
        messages[msg][0] += messages[msg][2 if icons else 1]
    if meld:
        diff_prog = 'meld'
    if double:
        # this swaps single single and double quotes
        quotes[0], quotes[1] = quotes[1], quotes[0]
    if multi_line_unwrap:
        post_processors.append(pp_multi_line_unwrap)
    ...
    if len(sources) == 1:
        ...
        try:
            ...
            shutdown(loop)
    if verbose or not quiet:
        bang = messages['BANG_NOT_OK' if report.return_code else 'BANG_OK'][0]
        out(f'All done! {bang}')
    ...
    try:
        ...
        if not src.is_file() and str(src) == '-':
            ...
            if changed is not Changed.CACHED and format_file_in_place(
                src, line_length=line_length, fast=fast, write_back=write_back, mode=mode
            ):
                changed = Changed.YES
    ...
    if sources:
        ...
        tasks = {
            loop.run_in_executor(
                executor, format_file_in_place, src, line_length, fast, write_back, mode, lock
            ): src
    ...
    if write_back == write_back.YES:
        ...
        if diff_prog is not None:
            ...
            with tempfile.NamedTemporaryFile('w', prefix='oitnb_') as fp:
                fp.write(dst_contents)
                fp.flush()
                os.system(diff_prog + ' ' + fp.name + ' ' + str(src))
            return
        now = datetime.utcnow()
...
def format_file_contents(
    src_contents: str, *, line_length: int, fast: bool, mode: FileMode = FileMode.AUTO_DETECT
) -> FileContent:
...
class BracketTracker:
    ...
    def mark(self, leaf: Leaf) -> None:
        ...
        If a leaf is a delimiter (a token on which ointb can split the line if
...
class LineGenerator(Visitor[Line]):
    ...
    def visit_INDENT(self, node: Node) -> Iterator[Line]:
        ...
        # In _oitnb_lib2to3 INDENT never holds comments.
...
def whitespace(leaf: Leaf, *, complex_subscript: bool) -> str:  # noqa C901
    ...
    if not prev:
        ...
        if prevp.type == token.EQUAL:
            ...
            prevp.parent and prevp.parent.type == syms.factor and prevp.type in MATH_OPERATORS
        ):
            return NO
...
def is_split_before_delimiter(leaf: Leaf, previous: Leaf = None) -> int:
    ...
    if (
        ...
        and not (
            previous is not None and previous.type == token.NAME and previous.value == 'not'
        )
    ):
        return COMPARATOR_PRIORITY
    ...
    if (
        ...
        and not (
            previous is not None and previous.type == token.NAME and previous.value == 'is'
        )
    ):
        return COMPARATOR_PRIORITY
...
def list_comments(prefix: str, *, is_endmarker: bool) -> List[ProtoComment]:
    ...
    for index, line in enumerate(prefix.split('\n')):
        ...
        result.append(
            ProtoComment(type=comment_type, value=comment, newlines=nlines, consumed=consumed)
        )
        nlines = 0
    return result
    ...
    for split_func in split_funcs:
        ...
        try:
            for l in split_func(line, py36):
                ...
                result.extend(split_line(l, line_length=line_length, inner=True, py36=py36))
...
def delimiter_split(line: Line, py36: bool = False) -> Iterator[Line]:
    ...
    for index, leaf in enumerate(line.leaves):
        ...
        if leaf.bracket_depth == lowest_depth and is_vararg(leaf, within=VARARGS_PARENTS):
...
def normalize_string_quotes(leaf: Leaf) -> None:
    ...
    if len(value) == 2:  # empty string
        leaf.value = '""'
        return

    elif value[:3] == quotes[2]:
        orig_quote = quotes[2]
        new_quote = quotes[3]
    elif value[0] == quotes[0]:
        orig_quote = quotes[0]
        new_quote = quotes[1]
    else:
        orig_quote = quotes[1]
    ...
    if new_escape_count == orig_escape_count and orig_quote == quotes[0]:
...
def find_project_root(srcs: Iterable[str]) -> Path:
    """Return a directory containing .git, .hg, or __init__.py with _package_data.

    That directory can be one of the directories passed in `srcs` or their
    common parent.

    If no directory in the tree contains a marker that would specify it's the
    project root, the users config dir for oitnb is returned.
    """
    if not srcs:
        return Path('/').resolve()
    ...
    for directory in common_base.parents:
        ...
        initpy = directory / '__init__.py'
        if (initpy).is_file():
            if '\n_package_data = ' in initpy.read_text():
                return directory
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f'INTERNAL ERROR: ointb produced invalid code: {exc}. '
    ...
    if src_ast_str != dst_ast_str:
        log = dump_to_file(diff(src_ast_str, dst_ast_str, 'src', 'dst'))  # NOQA
        raise AssertionError(
            f'INTERNAL ERROR: ointb produced code that is not equivalent to '
            f'the source.  '
            # f"Please report a bug on https://github.com/ambv/black/issues.  "
...
def assert_stable(
    ...
    if dst != newdst:
        log = dump_to_file(  # NOQA
            diff(src, dst, 'source', 'first pass'),
            diff(dst, newdst, 'first pass', 'second pass'),
        )
        raise AssertionError(
            f'INTERNAL ERROR: oitnb produced different code on the second pass '
            f'of the formatter.  '
            # f"Please report a bug on https://github.com/ambv/black/issues.  "
            # f"This diff might be helpful: {log}"
        ) from None
    ...
    for idx, line in enumerate(lines):
        if len(line) > 1 and line[-2] == '(':
            if check_comment(line):
                continue
            ls = lines[idx + 1].strip()
            if ls == '"""' or ls == '"""\\':
                # check for line length
                lines[idx] = lines[idx][:-1] + ls + '\n'
                lines[idx + 1] = ""
        if len(line) > 1 and line[-4:-1] == '"""':
            if idx > len(lines) - 1:
                continue
            # if check_comment(line):
            #     continue
            ls = lines[idx + 1].lstrip()
            if ls and ls[0] == ')':
                # check for line length
                lines[idx] = lines[idx][:-1] + ls
                lines[idx + 1] = ""
