...
class PrvManager(object):
        ...
        if flag:
            with open(self.output_name, 'a') as f:
                f.write(self.format_line(
                    (0, thread.get_ident(), int(time.time() * 1000000),
    ...
    def add_function_trace(self, enter, full_name, prv_value):
        ...
        self._add_trace(prv_traces.TraceType.METHOD.value, int(time.time() * 1000000),
    ...
    def add_network_receive(self, origin_host_ip, origin_host_port, request_id, method_id):
        ...
        if not _is_network_tracing_enabled():
            return
        self._add_trace(prv_traces.TraceType.RECEIVE.value, int(time.time() * 1000000),
                        "{}:{:d}:{:d}:{:d}".format(
    ...
    def activate_tracing(self):
        try:
            ...
            logger.debug("activateTracing was interrupted while sleeping %s", ex)
