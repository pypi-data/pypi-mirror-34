...
def get_events(type=None, sub_type=None, asset_id=None, **kwargs):
    ...
    headers = {'api-key': api_key, 'content-type': 'application/json', 'accept': 'application/json'}
    if asset_id:
        params = {'assetId': asset_id, 'sort': kwargs.get('sort'), 'cursor': kwargs.get('cursor'), 'limit': (kwargs.get('limit', 25) if (not kwargs.get('autopaging')) else _constants.LIMIT_AGG)}
    else:
