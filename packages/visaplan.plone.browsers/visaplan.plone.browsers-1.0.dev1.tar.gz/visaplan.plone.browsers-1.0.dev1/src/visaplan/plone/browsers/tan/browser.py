...
import StringIO
from csv import writer as csv_writer, excel, register_dialect
...
# Unitracc:
from ...config import MANAGE_TANS

# Unitracc-Tools:
from ...config import (CREATE_TAN as PERM_CREATE_TAN,
        VIEW_TANS as PERM_VIEW_TANS,
        MANAGE_TANS as PERM_MANAGE_TANS,
        )
from visaplan.tools.classes import Mirror
from ...tools.forms import tryagain_url, merge_qvars, parse_date
from visaplan.tools.dicts import subdict, make_key_injector
from visaplan.tools.coding import make_safe_stringrecoder
from ...tools.misc import (make_sequencer,
        translate_dummy as _,
        )

# Andere Browser und Adapter:
from ...adapters.sqlwrapper.utils import (make_where_mask,
        extract_dict,
        )
from ...browser.groupsharing.browser import (
        groupinfo_factory, is_direct_member__factory,
        )

# Dieser Browser:
from .utils import (default_expiration_date, DATE_FORMAT,
        default_duration,
        tan2int, group3 as format_tan, check_tan,
        invalid_tan_txt, TAN_EXAMPLE,
        start_and_ends, start_and_ends2, today_matches_spec,
        date_as_string,
        )
...
from ...tools.debug import (trace_this, print_call_and_result,
        make_sleeper,
        )
...
class Browser(BrowserView):
    ...
    def can(self, perm):
        if DEBUG_PERMISSIONS:
            ...
            ok = self.context.getAdapter('checkperm')(perm)
            LOGGER.info('%r permission: %s', perm, ok and 'OK' or 'missing!')
            return ok
        return self.context.getAdapter('checkperm')(perm)
        ...
        if tan:
            try:
                ...
                with context.getAdapter('sqlwrapper') as sql:
                    ...
                    usermanagement = getBrowser('usermanagement')
    ...
    def addTAN(self):
        ...
        try:
            ...
            error_url = tryagain_url(request, tan_data.keys())
            ...
            message(str(e), 'error')
            return request.RESPONSE.redirect(tryagain_url(request, tan_data.keys()))
    ...
    def change(self):
        ...
        try:
            ...
            # Hier kein extract_dict, weil 'tan' wirklich vorhanden
            # sein muß!
            ...
            with getAdapter('sqlwrapper') as sql:
                ...
                values_for_changeset = extract_dict(['additional_info'],
                                                    values_for_tan)
                if values_for_changeset:
                    sql.update('tan_changeset',
                               values_for_changeset,
                               query_data={'id': changeset})
                untracked = extract_dict(TAN_FIELDS_UNTRACKED,
                                         values_for_tan,
                                         pop=1)
                ...
                sql.insert('tan_history',
                           values_for_history)
                message(_('Changes saved in changeset #%(changeset)d'
                          ) % locals(),
                        'info')
                return redirect(tryagain_url(request, ['tan']))
        except KeyError as e:
            print e.__class__, e
            message(""'No TAN given!', 'error')
            return redirect(error_path)
        except ValueError as e:
            message(str(e), 'error')
            return redirect(error_path)
    ...
    def _tans_data(self, form=None, pop=0):
        ...
        if form is None:
            ...
            form = request.form
        return extract_dict(TAN_FIELDS, form, pop)
    ...
    def _check_newtan_data(self, dic):
        ...
        if msglist:
            getAdapter = self.context.getAdapter
            message = getAdapter('message')
            _ = getAdapter('translate')
            for msg in msglist:
                message(_(msg) % dic, 'error')
    ...
    def list(self, sql=None):
        ...
        request = context.REQUEST
        form = request.form
        query_data = extract_dict(TAN_FIELDS + ['changeset'],
                                  form, noempty=1)
    ...
    def history(self, sql=None):
        ...
        request = context.REQUEST
        form = request.form
        query_data = extract_dict(TAN_FIELDS, form)
        if not 'group_id' in query_data:
            query_data.update(extract_dict(['gid'], form))
        if sql is not None:
    ...
    def _export_as_csv(self, fieldnames, table=None,
        ...
        query_data = extract_dict(filterfields, form, pop=0)
        if table is None:
        ...
        if 1:  # schonmal eingerückt, für künftiges "with":
            ...
            writer.writerow(map(recode, fieldnames))
    ...
    def extract_filters(self, pop=0):
        ...
        dic = extract_dict(FREQUENTLY_USED_FILTERS,
                           form, pop=pop)
        if not 'group_id' in dic:
            dic2 = extract_dict(['gid'],
                                form, pop=pop)
            if dic2:
    ...
    def auth_unitracc_manage_tans(self):
        ...
        context = self.context
        if context.getBrowser('tpcheck').can_unitracc_manage_tans():
            return True
