...
class LogHubTail:
    ...
    def start_tail_task(self):
        ...
        self.last_time_5sec_int = math.floor(time.time() / 5)  # 每5秒种这个值变化一次
        self.total_line = 0
        self.total_size = 0
        ...
        def handle_message(message):
            if message['type'] == 'message' or message['type'] == 'pmessage':
                if not self.isOnlyStat:
                    ...
                    if cur_time_5sec_int == self.last_time_5sec_int:
                        self.total_line += 1
                        self.total_size += len(message['data'])
                    else:
                        logger.info("5 second stat -- line:%s size:%s" % (self.total_line, self.total_size))
                        total_line = 1
    ...
    def start_tail(self):
        ...
        if self.f_arg is not None:
            ...
            logger.error("-f parameter is not exists.")
