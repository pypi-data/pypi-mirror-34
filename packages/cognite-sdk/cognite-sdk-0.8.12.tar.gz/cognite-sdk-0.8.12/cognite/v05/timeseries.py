...
from cognite.v05.dto import Datapoint, DatapointsResponse, DatapointsResponseIterator, LatestDatapointResponse, TimeSeries, TimeSeriesResponse, TimeseriesWithDatapoints
...
def get_datapoints(name, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    if kwargs.get('limit'):
        return _get_datapoints_user_defined_limit(name, aggregates, granularity, start, end, limit=kwargs.get('limit'), protobuf=kwargs.get('protobuf'), api_key=api_key, project=project)
    diff = (end - start)
...
def post_multi_tag_datapoints(timeseries_with_datapoints, **kwargs):
    ...
    timeseries_to_upload_binned = _utils.first_fit(list_items=timeseries_with_datapoints_limited, max_size=ul_dps_limit, get_count=(lambda x: len(x.datapoints)))
...
def get_datapoints_frame(time_series, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    (start, end) = _utils.interval_to_ms(start, end)
    if kwargs.get('limit'):
        return _get_datapoints_frame_user_defined_limit(time_series, aggregates, granularity, start, end, limit=kwargs.get('limit'), api_key=api_key, project=project)
    diff = (end - start)
    num_of_processes = kwargs.get('processes', os.cpu_count())
    granularity_ms = 1
    if granularity:
...
def _get_datapoints_frame_user_defined_limit(time_series, aggregates, granularity, start, end, limit, **kwargs):
    ...
    (api_key, project) = (kwargs.get('api_key'), kwargs.get('project'))
    url = (config.get_base_url(api_version=0.5) + '/projects/{}/timeseries/dataframe'.format(project))
    body = {'items': [({'name': '{}'.format(ts)} if isinstance(ts, str) else {'name': '{}'.format(ts['name']), 'aggregates': ts.get('aggregates', [])}) for ts in time_series], 'aggregates': aggregates, 'granularity': granularity, 'start': start, 'end': end, 'limit': limit}
    headers = {'api-key': api_key, 'content-type': 'application/json', 'accept': 'text/csv'}
    res = _utils.post_request(url=url, body=body, headers=headers, cookies=config.get_cookies())
