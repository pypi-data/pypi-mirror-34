    ...
    ,plan = Plan([PyBlock(get
                 ...
                 ,PyBlock(get_
                          ...
                          ,args=noIndex(['struct_dict'
                                        ,'nickname'
                                        ,'unknown']))
                ,SqlBlock(text = mkInsCmd('pure_struct',['name'])
                       ,name = 'insert_pure_struct'
                       ,args = [Arg('pure_struct_name')])
                ,SqlBlock(text = """SELECT pure_struct_id FROM pure_struct WHERE name=%s"""
                       ,name = 'query_pure_struct'
                       ,args = [Arg('pure_struct_name')],deps=['insert_pure_struct'])
                ,SqlBlock(text = mkUpdateCmd('struct',['pure_struct_id_catalog'],['struct_id'])
                       ,name = 'update_fk'
...
pop_surf_struct_catalog = Rule('pop_surf_struct_catalog'
    ...
    ,plan=Plan([PyBlock(get
             ...
             ,PyBlock(get_catalog_facet,args=[Arg('paramdict')])
...
pop_ads_catalog = Rule('pop_ads_catalog'
    ...
    ,plan=Plan([PyBlock(get
                        ...
                        ,args=noIndex(['paramdict'
                                        ,'ads'
                                        ,'str']))
               ,PyBlock(get_catalog_ads,args=[Arg('ads_json')])
               ,SqlBlock(text=mkUpdateCmd('atom'
