...
@click.pass_context
def attach(ctx, pod_id, port_forwarding):
   """ Connect to a started pod """
   if not TokenManager.is_authenticated():
       ctx.invoke(login)
   connect(pod_id, port_forwarding=port_forwarding)

def connect(pod_id, jupyter=False, port_forwarding=""):
    """ Connect to a started pod """
    logger.info("Connecting to the pod...")
    is_proxy, address, username, private_key, port = PodControlClient().get_connect_info(pod_id)
    ...
    if is_proxy:
        ...
        os.system('ssh -i "{}" -oStrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\
                {}@{} -p {} {} -L 8888:localhost:8888 jupyter notebook --ip=0.0.0.0 --port=8888'.format(key_file_path, username, address, port, port_forwarding))
    else:
        os.system('ssh -i "{}" -oStrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\
                {}@{} -p {} {}'.format(key_file_path, username, address, port, port_forwarding))
