...
# need install dawn-sdk by pip/pip3
#  $ pip install dawn-sdk
#  $ pip3 install dawn-sdk
# if you had installed dawn-sdk you should clean cache for update.
#  $ rm -rf ~/.cache/pip
#  $ rm -rf ~/.cache/pip3

import os
from dawn_sdk import config
from dawn_sdk import auth
from dawn_sdk import bucket
from dawn_sdk.xml_body import PartInfo
from dawn_sdk.progress import LimitedReader
...
def list_object2(access_key, secret_key, endpoint, bucket_name):
  print("===")
  bkt = prepare(access_key, secret_key, endpoint, False, bucket_name)
  objs = bkt.list_objects()
  print(objs.is_continous)
  print(objs.next_marker)
  for item in objs.objects:
    print (item.key)
  print("---")
  objs = bkt.list_objects(max_keys=2)
  print(objs.is_continous)
  print(objs.next_marker)
  for item in objs.objects:
    print (item.key)
...
def list_objects(access_key, secret_key, endpoint, bucket_name):
  bkt = prepare(access_key, secret_key, endpoint, False, bucket_name)
  key = 'xxx/a.txt'
  data = 'new bee, hello, world!'
  ...
  print("directory: xxx/ object")
  objs = bkt.list_objects('xxx/', '/')
  for item in objs.objects:
    print(item.key)
  for pfx in objs.prefixes:
    print(pfx)

  print("directory: xxx/yyy/ object")
  objs = bkt.list_objects('xxx/yyy/', '/')
  for item in objs.objects:
    print(item.key)
  for pfx in objs.prefixes:
    print(pfx)
...
if __name__ == '__main__':
  ...
  list_object(access_key, secret_key, endpoint, "oss-sdk")

  list_object2(access_key, secret_key, endpoint, "oss-sdk")

  list_objects(access_key, secret_key, endpoint, "oss-sdk")