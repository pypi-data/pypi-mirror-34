...
class Logged_RequestHandler(RequestHandler):
    ...
    def generate_proxy_HTTPRequest(self, protocol: str = None, server: str = None, uri: str = None, **kwargs) -> HTTPRequest:
        ...
        _headers = HTTPHeaders(request_obj.headers)
        ...
        if (_method == 'GET'):
            ...
            pass
        elif (request_obj.body):
            _body = request_obj.body
            _headers['Content-Length'] = len(_body)
        else:
            _body = b''
        
        self.logger.trace("Proxying request:\n{0} {1} HTTP/1.1\n{2}\nBody: {3}", _method, new_url, _headers, len(_body or ''), prefix='proxy')
        self.logger.info("Redirecting to url: {}", new_url, prefix='proxy')
        _proxy_request = HTTPRequest(url=new_url, method=_method, body=_body, headers=_headers, **kwargs)
    ...
    def __proxied(self, response: HTTPResponse):
        _code = response.code
        self.logger.trace("Proxying response:\nHTTP/1.1 {0} {1}\n{2}\nBody: {3}", response.code, response.reason, response.headers, len(response.body or ''))
        if (_code == 599):
