...
__all__ = [
	...
	'RMSContrastRatio', 'MichelsonContrastRatio',
	'IdealToPhysicalLuminance', 'PhysicalToIdealLuminance',
	'IdealToPhysicalContrastRatio',  'PhysicalToIdealContrastRatio',
	'IdealContrastRatioToNormalizedContrast', 'NormalizedContrastToIdealContrastRatio',
...
def FindGamma( world, finish=None, xBlue=True, **kwargs ):
	"""
	...
	def wrap_finish( gamma ): world.stimuli.gammatest.Leave(); finish and finish( gamma )
	...
	*unlinearized*.  At extreme values, color-contrast-enhancement may
	...
	def Animate( self, t ):
		...
		if triggered:
			target = self.target()
			if target:
				if self.textureSlotNumber < 0 or self.textureID < 0: self.LoadTexture( target.Capture() ) # bit of a hack but by far the easiest way of initializing everything right
...
def PlotTimings( arg, savefig=None, traces=(), axes=None, finish=True, **kwargs ):
	...
	with warnings.catch_warnings():
		...
		if ax1: plt.sca( ax1 ); plt.grid( True ); plt.xlim( [ t.min(), t.max() ] ); plt.legend( loc=( 1.01, -1 if cpu else 0 ), labelspacing=0 ).draggable( True )
		if ax2: plt.sca( ax2 ); plt.grid( True ); plt.xlim( [ t.min(), t.max() ] ); plt.legend( loc=( 1.01, 0 ), labelspacing=0 ).draggable( True )
