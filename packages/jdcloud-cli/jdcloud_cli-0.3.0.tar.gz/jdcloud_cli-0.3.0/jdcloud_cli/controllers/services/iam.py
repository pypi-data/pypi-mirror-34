...
class IamController(BaseController):
    ...
    @expose(
        arguments=[
            (['--region-id'], dict(help="""(string) Region ID """, dest='regionId', required=False)),
            (['--action'], dict(help="""(string) 操作action eg.<serviceName>:<actionName> """, dest='action', required=True)),
            (['--input-json'], dict(help='(json) 以json字符串或文件绝对路径形式作为输入参数。\n字符串方式举例：--input-json \'{"field":"value"}\';\n文件格式举例：--input-json file:///xxxx.json', dest='input_json', required=False)),
            (['--headers'], dict(help="""(json) 用户自定义Header，举例：'{"x-jdcloud-security-token":"abc","test":"123"}'""", dest='headers', required=False)),
        ],
        formatter_class=RawTextHelpFormatter,
        ...
        description='''
            ...
            示例: jdc iam get-sensitive-op-setting  --action xxx
        ''',
    )
    def get_sensitive_op_setting(self):
        ...
        try:
            from jdcloud_sdk.services.iam.apis.GetSensitiveOpSettingRequest import GetSensitiveOpSettingRequest
            params_dict = collect_user_args(self.app)
            headers = collect_user_headers(self.app)
