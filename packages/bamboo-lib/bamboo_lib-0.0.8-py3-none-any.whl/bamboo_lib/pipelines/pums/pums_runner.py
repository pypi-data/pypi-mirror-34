...
class TransformStep(PipelineStep):
    ...
    def run_step(self, prev, params):
        ...
        df['wage_bin'] = pd.cut(df.wagp, cutoffs, labels=[
            ...
            '$180-190k',
            '$190-200k',
            '$200k+'], inlcude_lowest=True)
        df['wage_bin'] = pd.cut(df.wagp, cutoffs)
        df.loc[df.wagp.isnull(), 'wage_bin'] = 'N/A'
        # TODO some values >200000 showed up as NA. todo, test values >= 200k
        raise Exception("TODO: validate wage_bin numbers!")
        df.wage_bin = df.wage_bin.astype(str)
        df['in_workforce'] = df.agep.astype(int) >= 16 & df.wagp > 0 & df.esr.astype(str).isin(["1", "2", "4", "5"])

        # TODO review and test
        df.loc[df.in_workforce & (df.wkhp.astype(int) >= 35), 'time_status'] = 1
        df.loc[df.in_workforce & (df.wkhp.astype(int) < 35), 'time_status'] = 2
        df.loc[df.agep >= 5, 'fiveplus'] = True
        ...
        df['wage_bin_id'] = df.wage_bin.apply(wage_bin_map)
        return df
...
def run(params, **kwargs):
    ...
    connector = Connector.fetch("monetdb-remote", params.get("output-db", fallback_source))
    ...
    table_name = "pums_{}".format(estimate)
    chunkstep = ChunkStep()
    step3 = TransformStep(connector)
    step4 = SaveStep()
    # LoadStep(table_name, connector, if_exists="append",
...
if __name__ == '__main__':
    run({"year": 2013, "estimate": 5, "only-gen-schema": False, "regional-mode": "pr"})
