...
class Statement(parse.ParseItem):
    ...
    @property
    def raw_args(self):
        return ''.join(a.source if isinstance(a, QuotedString) else a
                       for a in self.args)
    ...
    for item in parse_result:
        if isinstance(item, Statement) and item.is_openclose_tag():
            ...
            if name and name != head.name:
                raise parse.PigletParseError("Unexpected {} at {}:{}"
                                             .format(item.source,
                                                     item.pos.line,
                                                     item.pos.char))
            head.end_tag = item
            ...
            head.children.append(item)
    if len(stack) > 1:
        raise parse.PigletParseError("Missing end tag for {}, "
                                     "opened at {}:{}"
                                     .format(stack[-1].source,
                                             stack[-1].pos.line,
                                             stack[-1].pos.char))
    return stack[0]
...
def compile_intermediate(parse_result):
    ...
    cx.strip_spaces_between_directives(tree,
                                       element_cls=Statement,
                                       is_directive=lambda x: isinstance(x, Statement))
...
def strip_ws_on_directive_lines(node):
    ...
    trim_nothing = lambda s: s

    trim_start = {'+': trim_nothing,
                  '-': trim_all_start,
                   None: trim_line_start}
    trim_end = {'+': trim_nothing,
                '-': trim_all_end,
                None: trim_line_end}
...
def compile_extends(stmt, href):
    return im.ExtendsNode(href=href,
                          pos=stmt.pos,
                          children=[_compile_node(c) for c in stmt.children])


def compile_block(stmt, name):
    return im.BlockNode(name=name,
                        pos=stmt.pos,
                        children=[_compile_node(c) for c in stmt.children])


def compile_for(stmt, *args):
    return im.ForNode(each=stmt.raw_args,
                      pos=stmt.pos,
                      children=[_compile_node(c) for c in stmt.children])


def compile_def(stmt, *args):
    return im.DefNode(function=stmt.raw_args, pos=stmt.pos,
                      children=[_compile_node(c) for c in stmt.children])
...
def compile_if(stmt, *args):
    ...
    if children:
        ...
        if isinstance(last, Statement) and last.name == 'else':
            ...
            n.else_ = im.ElseNode(
                pos=last.pos,
                children=[_compile_node(c) for c in last.children])
    n.children = [_compile_node(c) for c in children]
    return n


def compile_with(stmt, *args):
    return im.WithNode(vars=stmt.raw_args,
                       children=[_compile_node(c) for c in stmt.children],
                       pos=stmt.pos)
