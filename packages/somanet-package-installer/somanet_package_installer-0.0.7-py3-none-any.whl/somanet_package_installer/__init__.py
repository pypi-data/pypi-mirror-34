...
def write_sii_to_slave(path, position=0):
  ...
  subprocess.run(['ethercat', '-p', str(position), 'sii_write', path])
...
def write_file_to_slave(path, position=0):
  ...
  subprocess.run(['ethercat', '-p', str(position), 'foe_write', path])
...
def change_state_for_slave(state, position=0):
  ...
  subprocess.run(['ethercat', 'state', state])
...
def unlock_stack_for_slave(secret, position=0):
  ...
  subprocess.call(['ethercat', '-p', str(position), 'foe_read', 'fs-stackunlock=' + secret])
...
def remove_file_from_slave(path, position=0):
  ...
  subprocess.run(['ethercat', '-p', str(position), 'foe_read', 'fs-remove=' + path])
...
def split_esi_into_parts(dtemp, esi='SOMANET_CiA_402.xml.zip', bytes=9000):
  ...
  subprocess.run(['split', '--bytes=' + str(bytes), '-a', '3', '-d', dtemp + '/' + esi, dtemp + '/' + esi + '.part'])
...
def erase_all(bsp_path, adapter_id=0):
  ...
  subprocess.run(['xflash', '--id', str(adapter_id), '--erase-all', '--target-file', bsp_path])
...
def install_bootloader(bin_path, bsp_path, adapter_id=0):
  ...
  subprocess.run(['xflash', '--id', str(adapter_id), '--write-all', bin_path, '--target-file', bsp_path, '--boot-partition', '0x90000'])
