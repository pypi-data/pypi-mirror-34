...
def divide(image, flat, ROI=None):
    ...
    filtered = numpy.asarray(Parallel(n_jobs=num_cores)(delayed(ff_corr)(ux[i], uy, sx[i], sy, deltaX[:,:,i], deltaY, image[:,:,i], flat) for i in range(image.shape[2])))
    #arr = numpy.transpose(filtered, (1,2,0))
