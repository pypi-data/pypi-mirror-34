...
Parameters:
    ...
    group: str
        GMAG group of stations (eg. 'epo'). If specified, then stations is ignored.
    downloadonly: bool
...
Notes:
    For gmag station names and groups, see: 
        http://themis.ssl.berkeley.edu/gmag/gmag_list.php?selyear=4000&selmonth=13&smap=on&sinfo=on&saelist=on&ae=on
    ...
    for sdate in dates:
        ...
        for station in stations: 
            ...
            if station == 'idx':
                ...
                filename = probe + '_' + level + '_' + station + '_' + year + month + day + '_v0' + version + '.cdf'              
...
def gmag_load(dates, stations, group=None, downloadonly=False, varformat=None, get_support_data=False, prefix='', suffix=''):
    ...
    for remotef, localf in file_list:   
        ...
        if resp:
            ...
            if not downloadonly:
                try: 
                    testcdf = pyspedas.cdf_to_tplot(locafile, varformat, get_support_data, prefix, suffix, False, True)                
                    print(testcdf) 
