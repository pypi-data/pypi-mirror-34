    ...
    if CURR_ENTITIE is not None:
        ...
        if CURR_ENTITIE == "input_select.folder_name":
            if ais_local_audio_service.G_CURRENT_PATH != ais_local_audio_service.G_LOCAL_FILES_ROOT:
                ...
                _beep_it(hass, 28)
                k = ais_local_audio_service.G_CURRENT_PATH.rfind("/" + os.path.basename(ais_local_audio_service.G_CURRENT_PATH))
                _say_it(hass, os.path.basename(ais_local_audio_service.G_CURRENT_PATH[:k]), None)
                return
