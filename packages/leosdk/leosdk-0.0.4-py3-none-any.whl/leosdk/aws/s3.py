...
class S3(LeoStream):
    ...
    def __init__(self, config: Cfg, bot_id: str, queue_name: str):
        self.stream_name = config.value('STORAGE')
        if self.stream_name is None:
            raise AssertionError("Missing 'STORAGE' field for the current environment in leo_config.py")
        self.bot_id = bot_id
        self.queue_name = queue_name

        self.client = S3.__aws_session(config).client('s3')
        self.max_record_size = config.int_value_or_else('STORAGE_MAX_RECORD_SIZE', 1024 * 900)
        self.max_batch_size = config.int_value_or_else('STORAGE_MAX_BATCH_SIZE', 1024 * 900)
        self.max_batch_records = config.int_value_or_else('STORAGE_MAX_BATCH_RECORDS', 1000)
        self.max_batch_age = config.int_value_or_else('STORAGE_MAX_BATCH_AGE', 200)
        self.max_attempts = config.int_value_or_else('STORAGE_MAX_UPLOAD_ATTEMPTS', 10)
    ...
    def log_successes(self):
        ...
        logger.debug('Uploaded %d compressed payload%s to S3 with a total of %d bytes',
