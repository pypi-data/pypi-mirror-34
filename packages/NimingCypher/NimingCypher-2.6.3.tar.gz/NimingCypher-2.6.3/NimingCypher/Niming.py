...
class NCrypter:
    ...
    def key_hash(self):
        return '###NIMING_KEY_HASH###%s###END-NIMING-KEY-HASH###' % self.crypt_text(GenSum(self.textfromweb.encode()))

    def check_hash(self, encrypted_hash):
        return GenSum(self.textfromweb.encode()).encode() == self.decrypt_text(encrypted_hash[21:-25])
