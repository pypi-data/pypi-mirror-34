...
def list_servers(project, all, verbose):
    ...
    if verbose:
        if not servers_:
            ...
            headers = ('Server Name', 'Type', 'Machine Type', 'CPUs', 'RAM',
                       'Status', 'ID', 'Started')
            rows = []
            for server in servers_:
                if server.machine_type == 'custom':
                    ...
                    memory_gb = '-'
                rows.append((
                    server.name, server.type_, machine_type, cpus,
                    memory_gb, server.status, server.id_,
...
def new(
    project, cores, memory, type_, machine_type, version, name, environments,
    wait
):
