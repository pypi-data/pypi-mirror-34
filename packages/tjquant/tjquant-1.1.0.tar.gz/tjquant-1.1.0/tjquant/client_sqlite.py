...
def GetStkPrice(stkcode,starttime,endtime,ex=False,re=True,timeout=10):
    if ex==True or re==False:
        ...
        cursor.execute('select * from tb_st22047_parq where obj=\'%s\' order by timing' % (stkcode))
        tmp = cursor.fetchall()
        ...
        cursor.execute('select * from tb_st22047_ex where obj=\'%s\' order by timing' % (stkcode))
        tmp = cursor.fetchall()
        res = [list(i) for i in tmp]
    return res
