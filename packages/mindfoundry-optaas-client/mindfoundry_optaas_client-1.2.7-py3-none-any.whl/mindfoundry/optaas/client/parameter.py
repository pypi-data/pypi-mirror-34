...
class NumericParameter(Parameter, abc.ABC):
    """A Parameter which can be assigned any value between the `minimum` and `maximum` (inclusive).

    Args:
        minimum (Union[int, float]): Smallest allowed value.
        maximum (Union[int, float]): Largest allowed value.
...
class IntParameter(NumericParameter):
    """A NumericParameter that can only be assigned integer values."""
...
class FloatParameter(NumericParameter):
    """A NumericParameter that can be assigned float values."""

    def __init__(self, name: str, minimum: float, maximum: float,
                 id: str = None,  # pylint: disable=redefined-builtin
                 optional: bool = None, include_in_default: bool = None,
                 default: float = None, distribution: Distribution = None) -> None:
