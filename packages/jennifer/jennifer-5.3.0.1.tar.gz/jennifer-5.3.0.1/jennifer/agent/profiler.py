...
class TransactionProfiler(ProfileData):
    ...
    def db_execute(self, host='', port=3306, query=''):
    ...
    def service_error(self, exec):
        self.exception(exec, Error.SERVICE_ERROR)

    def not_found(self, exec):
        self.exception(exec, Error.HTTP_404_ERROR)

    def db_connection_error(self, exec):
        self.exception(exec, Error.DB_CONNECTION_FAIL)

    def sql_error(self, exec):
        self.exception(exec, Error.SQL_EXCEPTION)

    def exception(self, exec, error_type=None):
