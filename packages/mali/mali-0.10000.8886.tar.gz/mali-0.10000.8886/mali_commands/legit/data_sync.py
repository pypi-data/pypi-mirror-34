...
class DataSync(object):
    ...
    def __upload_file_for_processing(self, file_obj, file_description):
        ...
        with tqdm(total=file_obj.tell(), desc="Uploading {}".format(file_description), unit='B', ncols=80,
            ...
            do_upload(None, gcs_auth, None, data_object_name, file_obj, headers, head_url=None, put_url=put_url,
                      callback=update_bar)
    ...
    def download_all(self, query, root_folder, dest_pattern, batch_size, processes, save_meta=True):
        ...
        normalized_item = status_with_timing('Execute Query', lambda: get_next_results(), fp=sys.stderr)

        storage = DiskStorage(dest_pattern, save_meta)

        multi_process_control = get_multi_process_control(processes)
        with tqdm(total=download_iter.total_data_points_size, desc="Downloading", unit='B', unit_scale=True, ncols=80, disable=self.__no_progressbar) as bar:
            item_callback = create_callback(bar, len(download_iter))
    ...
    class DownloadIterResults(collections.Iterator):
        ...
        def __get_next_results(self):
            def do_query():
                ...
                return self.__repo.metadata.query(self.__query, is_async=self.__is_async, start_index=query_offset)

            if self.__is_async and not self.__silent:
            ...
            def iter_items():
                ...
                while True:
                    ...
                    if self.__is_async or not has_items:
                        break
        ...
        def __prepare_iter_if_needed(self):
            ...
            results_iter, self.__total_data_points, self.__total_data_points_size = self.__get_next_results()
    ...
    def __upload_full_index(self, index_entities_file, isolation_token):
        ...
        status_with_timing('Server process index', set_entries_operation)
