...
from ..tan.utils import check_tan
from ..password.browser import InvalidPassword

# Dieser Browser:
from .utils import makeActivationLink, no_password_fieldname
...
class Browser(BrowserView):
    ...
    def setConfigure(self):
        ...
        getAdapter('authorized')('Manage portal')
    ...
    def processNewUser(self, userid_field, afterwards=None,
        ...
        try:
            ...
            translate = _ = getAdapter('translate')
            message = getAdapter('message')
            ...
            message("Sorry; can't register while in read-only mode. "
    ...
    def checkUserName(self,
        ...
        if translate is None:
            translate = self.context.getAdapter('translate')
        try:
            if not username:
    ...
    def _redirect(self, request, portal, path='/login_form'):
        ...
        """
        return request.RESPONSE.redirect(portal.absolute_url() + path)
    ...
    def unlock(self):
        ...
        if getAdapter('isanon')():
            ...
            auth = getBrowser('auth')
            active_user = '%s (%s)' % (auth.getId(), auth.getFullname())
        ...
        try:
            ...
            if object_ is None:
                ...
                message(_('This account doesn\'t exist'
            ...
            if not object_.getLocked():
                ...
                object_.reindexObject()
                message(_('Thank you! Your account has been activated.'))
                ...
                if not getAdapter('isanon')():
                    auth = getAdapter('auth')
                    # dict_['by'] = '%s (%s)' % (auth.getId(), auth.getFullname())
