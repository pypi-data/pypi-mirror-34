...
def trimap(X, triplets, weights, n_dims, n_inliers, n_outliers, n_random, lr, n_iters, Yinit, weight_adj, verbose,
           return_seq):
    if verbose:
        t = time.time()
    n, dim = X.shape
    ...
    for itr in range(n_iters):
        ...
        if verbose:
            if (itr + 1) % 100 == 0:
                print(
                    'Iteration: %4d, Loss: %3.3f, Violated triplets: %0.4f' % (itr + 1, C, n_viol / n_triplets * 100.0))
    if verbose:
...
class TRIMAP(BaseEstimator):
    ...
    def fit(self, X, init=None):
        ...
        self.embedding_, self.triplets, self.weights = trimap(X, self.triplets,
                                                              self.weights, self.n_dims, self.n_inliers,
                                                              self.n_outliers, self.n_random,
                                                              self.lr, self.n_iters, init, self.weight_adj,
                                                              self.verbose, self.return_seq)
        return self
    ...
    def sample_triplets(self, X):
        ...
        X /= np.max(X)
        X -= np.mean(X, axis=0)
        self.triplets, self.weights = generate_triplets(X, self.n_inliers, self.n_outliers, self.n_random,
                                                        self.weight_adj, self.verbose)
        if self.verbose:
            print("sampled triplets")
