...
@click.option('--cert', '-c', type=str, help='CA cert file location', default='/etc/kubernetes/pki/ca.crt',
              show_default=True)
@click.pass_context
def bootstrap(ctx, cluster_name, region, max_pods, kubelet_opt, kubelet_exec, kubelet_svc, kubeconf, cert):
    ...
    # opts = [tuple(o.split('=', 1)) for o in kubelet_opt]
    kubelet = Kubelet(cluster_name=cluster_name, region=region, max_pods=max_pods, kubeconf_file=kubeconf,
...
@click.option('--no-user-data', cls=MutuallyExclusiveOption, mutex_group=['cp_only', 'no_user_data'],
              ...
              help='Not use the user data in NodeGroup LaunchConfiguration, instead ekscli-boostrap alike for node discovery.')
...
@click.option('--bootstrap-opt', '-b', type=str, multiple=True, help='Options for ekscli bootstrap. See ekscli bootstrap --help.')
