...
class TestThreadingHandler(unittest.TestCase):
    ...
    def _getAsync(self, *args):
        from kazoo_sasl.handlers.threading import AsyncResult
        return AsyncResult
    ...
    def test_huge_file_descriptor(self):
        from kazoo_sasl.handlers.threading import _HAS_EPOLL
        if not _HAS_EPOLL:
            self.skipTest('only run on systems with epoll()')
        import resource
        import socket
        from kazoo_sasl.handlers.utils import create_tcp_socket
        try:
...
class TestThreadingAsync(unittest.TestCase):
    ...
    def test_captured_exception(self):
        from kazoo_sasl.handlers.utils import capture_exceptions
    ...
    def test_wraps(self):
        from kazoo_sasl.handlers.utils import wrap
