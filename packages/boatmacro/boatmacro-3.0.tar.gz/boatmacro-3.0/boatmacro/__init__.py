...
def write(text):
    try:
        for i in text:
            print("",end="")
            win32api.keybd_event(keycode[i], 0,0,0)
            win32api.keybd_event(keycode[i],0 ,win32con.KEYEVENTF_KEYUP ,0)
    except:
        print("no upper and no korean")
        print("대문자랑 한국어는 지원을 하지 않습니다.")
