...
def _get_wiki_base(item, object_dict, cl, slot, char_api=False):
    ...
    stats = {'flat es': 0, 'flat armour': 0, 'flat evasion': 0,
             'inc es': 0, 'inc armour': 0, 'inc evasion': 0}
    if wiki_base.implicits:
        for stat in unescape_to_list(wiki_base.implicits):
            text = stat.lower().replace('{crafted}', '')
            if " to " in text:
                if 'armour' in text:
                    stats['flat armour'] += text.split(' ')[0][1:]
                elif 'evasion rating' in text:
                    stats['flat evasion'] += text.split(' ')[0][1:]
                elif 'maximum energy shield' in text:
                    stats['flat es'] += text.split(' ')[0][1:]
            elif " increased " in text:
                if "armour" in text:
                    stats['inc armour'] += text.split(' ')[0][:-1]
                if "evasion rating" in text:
                    stats['inc evasion'] += text.split(' ')[0][:-1]
                if "energy shield" in text:
                    stats['inc evasion'] += text.split(' ')[0][:-1]

    if wiki_base.explicits:
        for stat in unescape_to_list(wiki_base.explicits):
            text = stat.lower().replace('{crafted}', '')
            if " to " in text:
                if 'armour' in text:
                    stats['flat armour'] += text.split(' ')[0][1:]
    ...
    if wiki_base.armour:
        arm = int(wiki_base.armour)
        arm += stats['flat armour']
        arm += (stats['inc armour']/100) * arm
        wiki_base.armour = str(round(arm))
    if wiki_base.evasion:
        ev = int(wiki_base.evasion)
        ev += stats['flat evasion']
        ev += (stats['inc evasion']/100) * ev
        wiki_base.evasion = str(round(ev))
    if wiki_base.energy_shield:
        es = int(wiki_base.energy_shield)
        es += stats['flat es']
        es += (stats['inc es']/100) * es
        wiki_base.energy_shield = str(round(es))
