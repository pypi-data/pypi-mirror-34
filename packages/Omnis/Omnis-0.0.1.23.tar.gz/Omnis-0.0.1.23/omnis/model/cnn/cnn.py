...
class CNN(Model):
    ...
    def create_class_indices(self, unique_targets):
        class_indices = dict()
        for i in range(unique_targets.shape[0]):
            class_indices[unique_targets[i]] = i
    ...
    def train(self,
        ...
        (ex. number of unique targets(labels or classes) in the target_array or directory)
        ...
        if self.is_image_data == True:
            ...
            if type(self.model) == type(None):
                unique_targets = numpy.lib.arraysetops.unique(self.target_array)
                class_indices = self.create_class_indices(unique_targets)
    ...
    def train_with_images(self, train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator):
        if use_fit_generator == False:
            if self.get_image_from == 'directory':
                ...
                self.prepare_train_data(is_image_data = True, get_image_from='argument', data_array=images, target_array=labels)
            if type(self.model) == type(None):
                unique_targets = numpy.lib.arraysetops.unique(self.target_array)
                class_indices = self.create_class_indices(unique_targets)
                num_classes = len(class_indices)
