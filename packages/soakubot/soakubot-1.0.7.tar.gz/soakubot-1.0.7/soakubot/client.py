...
class Client:
    ...
    def get(self, url, *, referrer=False, protection=VIEW):

        return self.request(url, referrer=referrer, protection=protection)

    def post(self, url, data=None, *, referrer=False, protection=EDIT):

        x = self.request(url, data, "POST", referrer=referrer, protection=protection)
        return x

    def request(self, url, data=None, method="GET", *, referrer=False, protection=VIEW) \
        ...
        while True:
            ...
            # Set referer
            if referrer: h = {**h, "referer": referrer}
...
def get_client(func=None, client=None, *args, **kwargs) -> typing.Union[PreparedCall, Client]:
    ...
    if len(clients) == 1: return clients[0]
