...
def get_params(global_params):
    ...
    params['est_task_time'] = (global_params.get('iterations', 10) * 3) + 5
    params['priority'] = 7
...
def task_setup(global_params):
    ...
    logger.info("execute setup thread a")
...
def task_main(global_params):
    ...
    logger.info("execute task_main thread a")
    result = "Passed"
    ...
    for i in range(0, global_params['iterations']):
        logger.debug("thread_a loop %s", i)
...
def test_teardown(global_params):
    ...
    logger.info("execute teardown thread a")
