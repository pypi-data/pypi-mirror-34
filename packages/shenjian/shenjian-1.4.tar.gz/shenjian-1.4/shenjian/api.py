...
# under the License.
import requests, time, hashlib, base64, json
...
class Crawler(_Base):
    ...
    def config_robots(self, agree):
        if agree:
            return self._request(self.url + '/config/robots/agree')
        return self._request(self.url + '/config/robots/disagree')
    ...
    def config_custom(self, custom_param):
        for key in custom_param:
            ...
            if isinstance(custom_param[key], dict):
                custom_param[key] = json.dumps(custom_param[key])
        return self._request(self.url + "/config/custom", method='post', data=custom_param)

    def clear_data(self):
        return self._request(self.url + "/source/clear")
