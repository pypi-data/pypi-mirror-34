...
def large_circuit():
    ...
    for _ in xrange(11):
        ...
        circuit.append([cz(qubits[i], qubits[i + 1]) for i in xrange(9)])
    circuit.append([cg.XmonMeasurementGate(key=u'meas')(*qubits)])
    return circuit
...
def test_run(scheduler):
    ...
    circuit.append(
        [cg.XmonMeasurementGate(key=u'a')(Q1),
         cg.XmonMeasurementGate(key=u'b')(Q2)])
...
def test_run_no_sharing_few_qubits(scheduler):
    ...
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[True]])
    np.testing.assert_equal(result.measurements[u'b'], [[False]])
...
def test_param_resolver_exp_w_axis_half_turns():
    exp_w = cg.ExpWGate(
        half_turns=1.0, axis_half_turns=cirq.Symbol(u'a'))
    circuit = cirq.Circuit()
    circuit.append(exp_w(Q1))
    resolver = cirq.ParamResolver({u'a': 0.5})
    result = compute_gate(circuit, resolver)
...
def test_param_resolver_exp_w_multiple_params():
    exp_w = cg.ExpWGate(
        half_turns=cirq.Symbol(u'a'),
        axis_half_turns=cirq.Symbol(u'b'))
    circuit = cirq.Circuit()
    circuit.append(exp_w(Q1))
...
def test_param_resolver_exp_z_half_turns():
    exp_z = cg.ExpZGate(half_turns=cirq.Symbol(u'a'))
    circuit = cirq.Circuit()
...
def test_param_resolver_exp_11_half_turns():
    exp_11 = cg.Exp11Gate(half_turns=cirq.Symbol(u'a'))
    circuit = cirq.Circuit()
...
def test_run_circuit_sweep():
    circuit = cirq.Circuit.from_ops(
        cg.ExpWGate(half_turns=cirq.Symbol(u'a')).on(Q1),
        cg.XmonMeasurementGate(u'm').on(Q1),
    )
...
def test_measurement_qubit_order(scheduler):
    circuit = cirq.Circuit()
    meas = cg.XmonMeasurementGate(key=u'')
...
def test_inverted_measurement(scheduler):
    circuit = cirq.Circuit.from_ops(
        cg.XmonMeasurementGate(u'a', invert_mask=(False,))(Q1),
        cirq.X(Q1),
        cg.XmonMeasurementGate(u'b', invert_mask=(False,))(Q1),
        cg.XmonMeasurementGate(u'c', invert_mask=(True,))(Q1),
        cirq.X(Q1),
        cg.XmonMeasurementGate(u'd', invert_mask=(True,))(Q1))
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements,
...
def test_inverted_measurement_multiple_qubits(scheduler):
    circuit = cirq.Circuit.from_ops(
        cg.XmonMeasurementGate(u'a', invert_mask=(False, True))(Q1, Q2),
        cg.XmonMeasurementGate(u'b', invert_mask=(True, False))(Q1, Q2),
        cg.XmonMeasurementGate(u'c', invert_mask=(True, False))(Q2, Q1))
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
...
def test_measurement_multiple_measurements(scheduler):
    circuit = cirq.Circuit()
    measure_a = cg.XmonMeasurementGate(u'a')
    measure_b = cg.XmonMeasurementGate(u'b')
    circuit.append(cirq.X(Q1))
...
def test_measurement_keys_repeat(scheduler):
    circuit = cirq.Circuit()
    meas = cg.XmonMeasurementGate(u'a')
...
def test_handedness_of_basic_gates():
    circuit = cirq.Circuit.from_ops(
        ...
        cirq.Z(Q1)**-0.5,
        cirq.Y(Q1)**0.5,
        cg.XmonMeasurementGate(key=u'').on(Q1),
    )
...
def bit_flip_circuit(flip0, flip1):
    ...
    g1, g2 = cg.ExpWGate(half_turns=flip0)(q1), cg.ExpWGate(half_turns=flip1)(
        q2)
    m1, m2 = cg.XmonMeasurementGate(u'q1')(q1), cg.XmonMeasurementGate(u'q2')(q2)
    circuit = cirq.Circuit()
...
def test_simulator_simulate_trial_result_repr():
    ...
    assert repr(v) == (u"XmonSimulateTrialResult("
                       u"params=ParamResolver({'a': 2}), "
...
def test_supports_extensions_for_parameter_resolving():
    ...
    ext.add_cast(cirq.ParameterizableEffect,
                 DummyGate,
                 lambda _: cg.ExpWGate(half_turns=cirq.Symbol(u'x')))

    a = cirq.NamedQubit(u'a')
    circuit = cirq.Circuit.from_ops(
        DummyGate().on(a),
        cg.XmonMeasurementGate(u'a').on(a)
    )
    results = cirq.google.XmonSimulator().run(
        circuit=circuit,
        param_resolver=cirq.ParamResolver({u'x': 1}),
        extensions=ext)

    assert unicode(results) == u'a=1'
