...
class Cursebox():
    ...
    def format_time(self, time, max_in_context=0):
        """
    ...
    def build_current_playlist_tracks(self, current_track):
        ...
        tracks = [
            ' [{}] {} {}'.format(
                self.format_time(t.get('duration'), longest_track),
                '♪' if t.get('playlist index') == current_track else '·',
                t.get('title')
            ) for t in current_playlist_tracks if t
        ]
        return tracks
    ...
    def current_playlist_screen(self):
        ...
        self.window.addstr(
            next(line),
            0,
            '[D]          Remove highligted track'
        )
        self.window.addstr(next(line), 0, '[j/k]        Move highlight down/up')
        self.window.addstr(
            next(line),
            0,
            '[h/l]        Skip to previous/next track'
        )
        self.window.addstr(
            next(line),
            0,
            '[J/K]        Move highlighted track down/up'
        )
...
file in the default location).""".format(VERSION))
