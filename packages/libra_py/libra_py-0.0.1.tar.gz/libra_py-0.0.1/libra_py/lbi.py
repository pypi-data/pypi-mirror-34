...
def LB(X,y,ind_true=None,auc_compute=False,roc_plot=True,path_plot=True,family='gaussian',kappa=10,
     ...
     Parameters
     ----------
     X: array, the design matrix, with shape (n_samples,n_features)
     y: array, the label, with shape (n_samples,)
     ind_true: array, the true signal set (for auc computation), default None since it's unknown
     ...
     t_seq: array, the saved estimators in the path. 
     ...
     """
     class_dict = {'gaussian':LBI_Lasso,'binomial':LBI_Logistic}
...
def cv_LB(X,y,kfolds=5,random_seed=1,ind_true=None,auc_compute=False,roc_plot=True,path_plot=True,
    ...
    for (fold,(train_index, test_index)) in enumerate(skf.split(X, y_new)):
         ...
         if family == 'gaussian':
              predict_error_cv += predict_error_gaussian(X_test,y_test,obj.beta_,obj.intercept_)
              
         elif family == 'binomial':
              predict_error_cv += predict_error_binomial(X_test,y_test,obj.beta_,obj.intercept_)
              
    obj.predic_error_cv = predict_error_cv
