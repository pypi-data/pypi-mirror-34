    ...
    def fit(self,model,freepar={},fixpar={},mapNames={},method='leastsq',xslice=slice(None),condition=None,output=True,**kw):
        ...
        method : default 'leastsq', 'differential_evolution', ‘BFGS’, ‘Nelder-Mead’  or from scipy.optimize.minimize
            Type of solver for minimization, for options see scipy.optimize. See last example for a comparison.
             - Only 'leastsq', 'BFGS' and 'differential_evolution' return errors for the fit parameters.
             - 'leastsq' is fastest. 'leastsq' is a wrapper around MINPACK’s lmdif and lmder algorithms
               which are a modification of the Levenberg-Marquardt algorithm.
...
class dataList(dataListBase):
    ...
    def showlastErrPlot(self, title=None, modelValues=None, **kwargs):
        ...
        for mXX,mYY,XX,YY,eYY,xslice,c  in zip(modelValues.X,modelValues.Y,
            ...
            outlist = ''  # only first get nonempty outlist if np.all(mXX==XX):
            if showresiduals:
                # residuals type
...
class dataArrayBase(np.ndarray):
    ...
    def __new__(subtype, input=None,
        ...
        skiplines : boolean function, list of string or single string
             Skip line if line meets condition. Function gets the list of words in a line.
             Examples:
              - lambda words: any(w in words for w in ['',' ','NAN',''*****])
                -> with exact match.
              - lambda words: any(float(w)>3.1411 for w in words)
              - lambda words: len(words)==1
             If a list is given, the lambda function is generated automatically as in first example above.
        ...
        delimiter : string
            ...
            E.g. '\\t' tabulator
        takeline : string
            A single string as optional first word in a line with data.
        ...
        lines2parameter : list of integer
            Used to mark lines with only numbers which are actually only parameters.
            Given lines i are prepended with 'line_i' to be found as attribute line_i.
        encoding : None, 'utf-8', 'cp1252', 'ascii'
            The encoding of the files read. By default the system default encoding is used.
            Others python2.7='ascii', python3='utf-8',
    ...
    def fit(self,model,freepar={},fixpar={},mapNames={},xslice=slice(None),condition=None,output=True,**kw):
        ...
        for key, value in freepar.items():free[key]=(value if isinstance(value,(int,float,np.integer,np.float)) else value[0])
        for key, value in fixpar.items() : fix[key]=(value if isinstance(value,(int,float,np.integer,np.float)) else value[0])
        if 'debug' in kw:
...
# unfortunately both use the same names with small char at beginning
