...
class Situation:
    ...
    def generatePlan(self, frontier = None, k = 10, principle = None):
        if frontier == None:
            frontier = [[[],copy.deepcopy(self.init)]]
        for a in self.actions:
            if self.isApplicable(a, frontier[0][1]):
                fsucc = self.execute(a, copy.deepcopy(frontier[0][1]))
                for e in self.events:
                    fsucc = self.execute(e, fsucc)
                fsuccNode = [frontier[0][0] + [a], fsucc]
                if self.satisfiesGoal(fsucc):
                    plannew = Plan(fsuccNode[0])
                    if principle == None:
                        return plannew
                    else:
                        plan_backup = self.plan
                        self.plan = plannew
                        #t1 = time.time()
                        perm = principle().permissible(self)
                        #print("Time to Check Permissibility: ", time.time() - t1)
                        self.plan = plan_backup
                        if perm:
                            return perm
                frontier += [fsuccNode]
        frontiernew = frontier[1:]
        if k != None:
            frontiernew = self.onlyKeepSmallerThanK(frontiernew, k)
            #print("Frontier: ", len(frontiernew))
            if len(frontiernew) == 0:
                return False
        return self.generatePlan(frontiernew, k, principle)
        
    def onlyKeepSmallerThanK(self, frontier, k):
        fnew = []
        for f in frontier:
            if len(f[0]) < k:
                fnew += [f]
...
class DoNoHarm:
    def permissible(self, situation):
        sn = situation.simulate()
        for h in situation.getHarmfulConsequences():
            if situation.isSatisfied(h, sn):
...
class Utilitarianism:
    def permissible(self, situation):
        ...
        for a in situation.alternatives:
