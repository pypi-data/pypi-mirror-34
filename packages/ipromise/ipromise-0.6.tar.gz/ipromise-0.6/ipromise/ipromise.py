...
class AbstractBaseClass:
    ...
    def __init_subclass__(cls):
        ...
        interface_class_of_name = {}
        defined_in = {}
        for ancestor in cls.__mro__:
            for name, value in vars(ancestor).items():
                if name not in interface_class_of_name:
                    if not hasattr(value, "__implemented_from__"):
                        continue
                    interface_class = getattr(value, "__implemented_from__")
                    interface_class_of_name[name] = interface_class
                    defined_in[name] = ancestor
                elif ancestor is not interface_class_of_name[name]:
                    raise TypeError(
                        f"{name} was marked implemented from ancestor "
                        f"class {interface_class_of_name[name].__name__} "
                        f"in {defined_in[name].__name__}, but it was "
                        f"defined again in {ancestor.__name__}.")
                elif not getattr(value, "__isabstractmethod__", False):
                    raise TypeError(
                        f"{name} was marked implemented from "
                        f"ancestor class {ancestor.__name__}, "
                        f"but it is not marked abstract there.")
