...
class CliTools:
    ...
    def save_ssh_key(cls, ssh_key_data):
        ...
        config.general.default_public_key = cipher.bytes_to_b64str(cipher.export_public_key_bytes())
        config.general.default_private_key = cipher.bytes_to_b64str(cipher.export_private_key_bytes('PEM'))
        # todo: load old keys
    ...
    def save_mali_config(cls, config_prefix, config_data):
        ...
        config = get_active_config()
        config.general.mali_data = config_data
        config.general.mali_path = filename
        get_active_config().general.save()
