...
def convert_from_graph_def (train_dir, graph_def_file, input_arrays, output_arrays, quantize = False, quantized_input_stats = (128, 128), default_ranges_stats = (0, 1)):
    ...
    if quantize:
        tocol.append ("--inference_type=QUANTIZED_UINT8")
        tocol.append ("--inference_input_type=QUANTIZED_UINT8")
        tocol.append ("--mean_values={}".format (quantized_input_stats [0]))
        tocol.append ("--std_dev_values={}".format (quantized_input_stats [1]))
        tocol.append ("--default_ranges_min={}".format (default_ranges_stats [0]))
        tocol.append ("--default_ranges_max={}".format (default_ranges_stats [1]))    
    os.system (" ".join (cmd))
