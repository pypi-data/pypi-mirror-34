...
def factorize(number):
    ...
    square_root = commacalc.extra.isqrt(number) + 1
...
def factorize_str(number):
    return "{} = {}".format(number, " × ".join("{}{}".format(
        base, "" if exp == 1 else str(exp).translate(_sups_table))
        for base, exp in sorted(Counter(factorize(number)).items())))
