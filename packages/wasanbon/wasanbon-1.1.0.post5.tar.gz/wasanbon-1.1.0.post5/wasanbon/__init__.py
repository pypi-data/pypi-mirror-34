...
def xcode_check():
    ...
    if line.find('darwin14'):
        ...
        sys.stdout.write('ERROR. wasanbon.xcode_check() failed. Unknown gcc version (%s)\n' % line)
        raise UnsupportedPlatformException()


def platform(short=False):
    import platform as plt
    if sys.platform == 'darwin':
        if short: return 'osx'
        return 'osx' + plt.mac_ver()[0][0:plt.mac_ver()[0].rfind('.')] + '_' + xcode_check()

    elif sys.platform == 'win32':
        arch = 'x64' if os.environ['PROCESSOR_ARCHITEW6432'] == 'AMD64' else 'x86'
        if sys.getwindowsversion()[1] == 1:
            return 'windows7_' + arch
        elif sys.getwindowsversion()[1] == 2:
            return 'windows8_' + arch
            
    elif sys.platform == 'linux2':
        distri = plt.linux_distribution()
        arch = 'x86' if plt.architecture()[0] == '32bit' else 'x64'
        if distri[0] == 'Ubuntu':
            return 'ubuntu_' + arch
        ...
        sys.stdout.write('Error. Unknown Architecture(%s)\n' % sys.platform)
        return 'Unknown'
