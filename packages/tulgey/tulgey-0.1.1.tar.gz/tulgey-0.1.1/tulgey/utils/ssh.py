...
def paramikoExecuteRead(sshClient: paramiko.SSHClient, cmdStr: str, skipStdout: bool = False, _sync: bool = True) -> Optional[Tuple[bytes, bytes]]:
    # We can't rely on fabric (for god knows what reason), so we run our command in the background,
    ...
    _, stdout, stderr = sshClient.exec_command(screenCmd)
