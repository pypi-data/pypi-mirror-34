...
def get_latest_version():
    release_branches = do("git branch -r").split('\n')
    max_version = (0, 0, 0)

    log.warn("CURRENT BRANCH NAME: %s" % os.environ.get("BRANCH_NAME"))

    for branch_name in release_branches:
        if 'origin/release' not in branch_name:
            continue

        tokens = map(lambda x: int(x), branch_name.split('origin/release/')[1].split('.'))
        major, minor, patch = tokens

        if major > max_version[0] or (major == max_version[0] and minor > max_version[1]) or \
                (major == max_version[0] and minor == max_version[1] and patch > max_version[2]):
            max_version = tokens
