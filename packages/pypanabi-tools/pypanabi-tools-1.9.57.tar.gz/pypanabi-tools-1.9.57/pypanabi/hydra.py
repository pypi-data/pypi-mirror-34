...
class Hydra(object):
    ...
    def is_all_data_staged(self):
        """
        All data staged in RDL
        :return: true or false [Boolean]
        """
        all_data_staged = True
        try:
            if self._db_client:
                query = """
        ...
        return all_data_staged

    def is_all_data_loaded(self):
        """
        All data loaded in ODL
        :return: true or false [Boolean]
        """
    ...
    def transform(self):
        ...
        try:
            if self._db_client:
                ...
                # extract metrics
                metrics = self.get_extract_metrics()

                if metrics['PrefixesKO'] and 'load' not in self.get_commands():
                    self._logger.info('\r')
                    self._logger.error('Prefixes/Manifest with error:')

                    for prefix in metrics['PrefixesKO']:
                        self._logger.error('- {}'.format(prefix))
                    exit(1)
            else:
                raise Exception('No database connection established. Connect to database before to run any method.')
        except Exception as e:
                ...
                for row in regions_with_data:
                    ...
                    if row.region == 'unknown':
                        ...
                        if result['rowcount'] > 0:
                            ...
                            # update log table
                            if self._streaming or not self.is_all_data_loaded():
