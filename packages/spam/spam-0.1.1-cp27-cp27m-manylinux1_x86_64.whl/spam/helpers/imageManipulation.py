...
def stackToArray( prefix, sufix='.tif', stack=range(10), digits='05d', createMask=False, erosion=False, verbose=False ):
    ...
    WARNING
    -------
        createMask requires the the module ``cv2`` which is not part of the default dependencies.
        It can be install with the following command:
    ...
    if verbose:
        ...
        print('\tfrom: {p}{f:{d}}{s}'.format(p=prefix, s=sufix, f=stack[0], d=digits))
        print('\tto:   {p}{l:{d}}{s}'.format(p=prefix, s=sufix, l=stack[-1], d=digits))
...
def crop( im, boxSize, boxOrigin=None ):
    ...
    if boxOrigin is not None:
        ...
        (cz, cy, cx) = ( imCentre[0]-sz/2, imCentre[0]-sz/2, imCentre[0]-sz/2 )
        
    # test sizes
    if ( cz+sz > im.shape[0] or
         cy+sy > im.shape[1] or
         cx+sx > im.shape[2]):
        print("spam.imageManipulation.crop: box bigger than image.")
        print("exit function.")
        return -1

    return im[cz:cz+sz, cy:cy+sy, cx:cx+sx]

def rescale( im, nBytes=4, scale=(0, 1) ):
    ...
    ----------
        ...
        nBytes : int, default=4
            The number of bytes of the float interger output.
    ...
    if ((nBytes & (nBytes - 1)) != 0):
        print("spam.imageManipulation.rescaleToInteger: nBytes = {}. Should be a power of 2.".format(nBytes) )
        print("exit function.")
        return -1
    ...
    return ( min(scale)+( max(scale)-min(scale) )*((im.astype('<f4')-im_min)/(im_max-im_min)) ).astype('<f{}'.format(nBytes))
...
def convertUnsignedIntegers(im, nBytes=1):
    ...
    nBytes = int(nBytes)
    if ((nBytes & (nBytes - 1)) != 0):
        print( 'spam.imageManipulation.convertUnsignedIntegers: nBytes = {}. Should be a power of 2.'.format(nBytes) )
        print("exit function.")
        return -1
    ...
    if inputType == numpy.uint8:
        ...
        print( 'spam.imageManipulation.convertUnsignedIntegers: im.dytpe = {}. Input should be uint.'.format( inputType ) )
        print("exit function.")
        return -1
...
def singleShift(im, shift, axis, sub=0):
    ...
    if dim == 2:  # if 2D image
        if shift == 1 and axis == 0:
            ...
            print("spam.imageManipulation.singleShift: shift={} and axis={}".format( shift, axis ) )
            print("exit function.")
            return -1
        ...
        print("spam.imageManipulation.singleShift: dim={}. Should be 2 or 3.".format( dim ) )
        print("exit function.")
        return -1
...
def _mask2D(im, erosion=False, structure=None, ):
    ...
    if im.dtype != 'uint8':
        ...
        im = rescale_to_uint(im, nBytes=1)

    # Step 3: ...
    blur = cv2.GaussianBlur(im, (5, 5), 0)
    _, thresh = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    if parse_version(cv2.__version__) > parse_version('2.4.9'):
