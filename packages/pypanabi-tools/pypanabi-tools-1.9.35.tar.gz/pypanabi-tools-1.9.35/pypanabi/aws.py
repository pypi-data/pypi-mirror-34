...
from .extras import get_current_timestamp, DATETIME_FORMAT
...
class MySQL(object):
    def __init__(self, host, user, password, database, port, charset='UTF8', cursor_factory=pymysql.cursors.DictCursor, autocommit=True, level='INFO'):
        ...
        """
        self._logger = Logger(logger_name=__name__, level=level)
        settings = {"host": host,
                    "user": user,
                    "password": password,
                    "database": database,
...
class EMR(object):
    def __init__(self, name, aws_region, applications, logs, bootstrap, script, emr_version, subnet_id, security_group,
        ...
        self._cluster_name = name + '_' + get_current_timestamp().strftime(DATETIME_FORMAT)
        self._aws_region = aws_region
