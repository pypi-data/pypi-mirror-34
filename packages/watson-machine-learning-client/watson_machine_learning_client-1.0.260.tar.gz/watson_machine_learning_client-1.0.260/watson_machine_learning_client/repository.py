...
class Repository(WMLResource):
    ...
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def store_function(self, meta_props, function):
        """
...
""".format(code, function.__name__)
    ...
    def get_model_details(self, model_uid=None, limit=None):
        ...
        result = self._get_artifact_details(url, model_uid, limit, 'models')

        # TODO temporary - remove when published_models will be returning it by itself
        if model_uid is not None:
            try:
                ...
                runtime_url = d['entity']['runtime']['url']
                result['entity'].update({'runtime': {"url": runtime_url}})
            except Exception as e:
                pass
