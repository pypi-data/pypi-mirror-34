...
def LSTM_LatentVectorAsInput(
        latent_vectors: tf.Tensor,  # batch_size * latent_size
        real_indices: tf.Tensor,  # batch_size * maxlen
        lstm_input_dropout: float,
        lstm_output_dropout: float,
        lstm_state_dropout: float,
        state_size: int,
        embedding_table: np.ndarray,  # vocab_size * embedding_size
        assist: int = 0,
    ) -> Tuple[tf.Tensor, tf.Tensor]:

    batch_size = tf.shape(latent_vectors)[0]
    latent_size = latent_vectors.shape[1].value
    ...
    def _predict_next_word(
        ...
        _, next_state = tf.nn.dynamic_rnn(
            cell=lstm_cell,
            inputs=tf.concat([latent_as_input, input_vectors], axis=2),
