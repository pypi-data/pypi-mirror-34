...
def authorize_request(request,
    ...
    token = ""
    if 'x-token' in dict(request.headers):
        token = request.headers['x-token']
    query_dict = {"fr": origin, "op": op_dict[request.method], "to": provider_id, "token": token}
    r = requests.get(broker_url+"/security/authorization-requests", query=query_dict)
