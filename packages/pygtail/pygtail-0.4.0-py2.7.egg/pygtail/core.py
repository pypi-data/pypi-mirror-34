...
class Pygtail(object):
    ...
    def __init__(self, filename, offset_file=None, paranoid=False, copytruncate=True):
        ...
        if exists(self._offset_file) and getsize(self._offset_file):
            ...
            offset_fh.close()
            print("offset file inode: %d" % self._offset_file_inode)
            print("log file inode: %d" % stat(self.filename).st_ino)
            print("log file size: %d" % stat(self.filename).st_size)
            print("offset: %d" % self._offset)
