...
def _handle_async_api(config, http_session, http_method_name, method_url, data, retry, on_response=None):
    if data is not None:
        data['async'] = True
    else:
        method_url = add_url_params(method_url, is_async=True)

    retry = retry or default_api_retry()
    result = _handle_sync_api(config, http_session, http_method_name, method_url, data, retry).json()
    ...
    while True:
        r = _handle_sync_api(config, http_session, 'get', 'data_volumes/tasks/' + token)
...
def handle_api(ctx_or_config, http_session, http_method_name, method_url, data=None, retry=None, is_async=False, on_response=None):
    config = ctx_or_config if isinstance(ctx_or_config, Config) else ctx_or_config.config

    if is_async:
        return _handle_async_api(config, http_session, http_method_name, method_url, data, retry, on_response=on_response)

    r = _handle_sync_api(config, http_session, http_method_name, method_url, data, retry)
