...
def clusterSM(score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None):
    np.set_printoptions(precision=4,suppress=True)
    desktop = os.path.join(os.environ["HOMEPATH"],"Desktop")
    desktop = os.path.join(desktop,"vampire_analysis_visual_result")
    ...
    if BuildModel:
        ...
        cluster.hierarchy.set_link_color_palette(['m', 'c', 'y', 'k'])
        fig = plt.figure(289,figsize=(6,2),linewidth=2.0)
        ...
        fig.savefig(os.path.join(desktop,"dendrogram_with_cluster_number.png"))
    else:
    ...
    plt.axis('off')
    #plt.close()
    fig922.savefig(os.path.join(desktop,"cluster_shape.png"))
    #axis('ij')
    fig289=plt.figure(289)
    plt.axis('equal')
    plt.axis('off')
    fig289.savefig(os.path.join(desktop,"dendrogram_with_cluster_shape.png"))
    #axis('ij')
    ###
    crap=np.sort(IDX)
    sid=np.argsort(IDX)
...
# clusterSM.m:237
    ...
    #plt.close()
    fig22.savefig(os.path.join(desktop,'cluster_distribution.png'))
...
def cluster_main(score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None):
    ...
    IDX,bdsubtype,C,VamModel=clusterSM(score,pc,bdpc,clnum,pcnum,VamModel,BuildModel)
    end = time.time()
