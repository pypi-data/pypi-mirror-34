...
class Command:
    def __init__(self, host, port, instid, app_name, com_msg_queue):
        ...
        self.container_id = subprocess.check_output(
            "grep docker /proc/self/cgroup|grep -m 1 .scope|echo $(read s;s=${s#*docker-};s=${s%.scope*};echo $s)",
            shell=True).strip('\n')
    ...
    def _create_conn_msg(self):
        from constants import OP, COMMAND_CONN, PROBE, PYTHON, VER, ARF_VER, INSTID, PGM, PRMS
        from ca_apm_agent.utils import json_helpers

        l = [(OP, COMMAND_CONN), (PROBE, PYTHON), (VER, ARF_VER), (INSTID, self.instid), (PGM, self.app_name)]
        if self.container_id:
            from constants import CONTAINER_ID
    ...
    def worker(self):
        ...
        import time
        from ca_apm_agent.utils import json_helpers
        from constants import OP, CONFIG, COLLECTOR, VERSION, MESSAGE, SPEAK, PAUSE, CONTINUE, CMD, PRMS
        self.connect()
        while True:
