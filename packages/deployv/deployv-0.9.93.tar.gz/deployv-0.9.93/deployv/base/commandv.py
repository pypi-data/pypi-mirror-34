...
class CommandV(Extensible):
    ...
    def deactivate_required(self):
        ...
        if self.create_database() and instance_type in ['develop', 'test', 'updates']:
            return True
        return False
    ...
    def restore(self):
        ...
        try:
            ...
            error_msg = restore_res[1] if not restore_res[0] else False
        ...
        if error_msg:
            ...
            self.sender.send_message(msg=error_msg, body=res, log_type='ERROR')
            self.error_trigger()
            return res
    ...
    def _check_update_production_requirements(self):
        ...
        db_config = self.instance_manager.db_config
        # Update the db config of the container with the db config sent by orchest
        # so we can override the credentials set in the env vars if they are not correct
        orchest_db_config = self.__config.instance_config.get('config')
        db_config.update({'db_user': orchest_db_config.get('db_user'),
                          'db_password': orchest_db_config.get('db_password'),
                          'db_name': orchest_db_config.get('db_name')})
        db_config = utils.odoo2postgres(db_config)
        with postgresv.PostgresConnector(db_config) as conn:
