...
# Unitracc:
from ...permissions import (ManageGroups, ViewGroups,
        ManageUsers, ManageCourses,
        )

# Unitracc-Tools:
from ...tools.forms import back_to_referer
...
# Dieser Browser:
from .utils import profileInfoDict, make_condensed, oneliner, profileBrainInfo
...
class Browser(BrowserView):
    ...
    def autocomplete_search(self):
        ...
        Benutzersuche, die json-Daten zurückgibt.
        """
        result = [{"label": unicode('%s %s' % (x.getFirstname, x.getLastname)),
                   "value": unicode(x.getUserId)
                   }
    ...
    def canManageUsers(self):
        ...
        Kann der angemeldete Benutzer Benutzer administrieren?
        """
        return self.context.getAdapter('checkperm')(ManageUsers)
    ...
    def authManageUsers(self):
        ...
        administrieren kann
        """
        self.context.getAdapter('authorized')(ManageUsers)
    ...
    def canManageGroups(self):
        ...
        Kann der angemeldete Benutzer Gruppen administrieren?
        """
        return self.context.getAdapter('checkperm')(ManageGroups)
    ...
    def authManageGroups(self):
        ...
        administrieren kann
        """
        self.context.getAdapter('authorized')(ManageGroups)
    ...
    def deleteProfiles(self):
        ...
        if errors:
            message('Error in form data!', 'error')
        ...
        for user_id, user_uids in u2uids.items():
            ...
            if cnt > u2max[user_id]:
                message("Won't delete all ${cnt} profiles of user ${user_id}!",
                        'error',
                        mapping=locals())
                errors += 1
        ...
        for uid in uids:
            ...
            try:
                ...
                logger.error('Error deleting profile %(profile_id)r'
                             ...
                             locals())
                logger.exception()
                message('Error deleting profile ${profile_id}'
                        ' for user ${user_id}!',
                        'error',
                        mapping=locals())
        if deleted_profiles:
            message('${cnt} profile(s) deleted: ${info}',
    ...
    def deleteUser(self):
        ...
        for uid in form.get('uids', []):
            ...
            if form.get('account'):
                if not userId:
                    ...
                    message(_("Won't delete user %(userId)r (owner of profile %(uid)s)!"
                              ) % locals(),
                            'error')
                else:
        ...
        if deleted_users:
            message(_('%d user(s) deleted: %s'
                      ) % (len(deleted_users),
                           ', '.join(deleted_users),
                           ))
        if deleted_profiles:
            message(_('%d profile(s) deleted: %s'
    ...
    def _toggle_lock(self, lock):
        ...
        if 0:
            ...
            tup = (auth.getId(), auth.getFullname())
            admin_info = '%s (%s)' % tup
        ...
        for uid in sorted(set(uids)):
            ...
            if o is None:
                message('User profile $uid not found!',
                        'error',
                        mapping=locals())
                continue
    ...
    def addGroup(self):
        ...
        portal = getAdapter('portal')()
        _ = getAdapter('translate')
        referrer = context.REQUEST.form['referrer']

        if not form.get('title'):
            message(_('Please fill all required fields.'), 'error')
        ...
        message(_('added group %(group_id)s'
                  ) % locals())

        if referrer == "course":
            return context.REQUEST.RESPONSE.redirect(portal.absolute_url() + '/manage_course_view')
        return context.REQUEST.RESPONSE.redirect(portal.absolute_url() + '/manage_groups_view')
    ...
    def reset_date(self):
        ...
        if context.portal_type == 'UnitraccAuthor':
            ...
            if user:
                ...
                message(_('user profile %r reset successfully'
                          ) % (userId,
                               ))
            else:
                message(_('member %r not found'
                          ) % (userId,
                               ))
        else:
            message(_('This can only be applied to user profiles (%r)'
                      ) % (context.portal_type,
                           ),
                    'error')
        return context.REQUEST.RESPONSE.redirect(portal.absolute_url() + '/manage_users_view')
    ...
    def set_group_manager(self):
        ...
        groupbrowser = getBrowser('groups')

        form = context.REQUEST.form
        group = groupbrowser.getById(form.get('group_id'))

        if not form.get('ids'):
            msgstr = _('No user selected.')
            ...
            if not acl.getUser(user_id):
                msgstr = _('The selected entry is no user.')
                message(msgstr, 'error')
            else:
                group.set_group_manager(user_id)
                msgstr = _('Group manager set.')
                message(msgstr)
    ...
    def remove_group_manager(self):
        ...
        msgstr = _('Group manager removed.')
        message(msgstr)
