...
## :Copyright: © 2008, 2009, 2010, 2013, 2014, 2016, 2018 Lele Gaifax
##
...
% if entity.agreedprivacy and entity.portrait:
...
<table class="ui small definition table">
  ...
  <%
  def omissis(value):
      return value if entity.agreedprivacy else value[:2] + '…'
  %>
  <tr>
    <td class="right aligned">${_('First name')}</td>
    <td>${omissis(entity.firstname)}</td>
  </tr>
  <tr>
    <td class="right aligned">${_('Last name')}</td>
    <td>${omissis(entity.lastname)}</td>
  </tr>
  % if not entity.shouldOmitNickName():
    <tr>
      <td class="right aligned">${_('Nickname')}</td>
      <td>${omissis(entity.nickname)}</td>
    </tr>
  % endif
  % if entity.agreedprivacy and entity.sex:
    <tr>
...
<%def name="partecipations_header()">
  <thead>
    <tr>
      ...
      <th class="center aligned sortedby date-header" rowspan="2">${_('Date')}</th>
      % if team_events:
      <th class="center aligned player-header" rowspan="2">${_('In team with')}</th>
      % endif
      <th class="center aligned event-header" rowspan="2">${_('Points')}</th>
      <th class="center aligned event-header" rowspan="2">${_('Bucholz')}</th>
      <th class="center aligned center aligned event-header" colspan="2">${_('Scores')}</th>
      <th class="center aligned total-header" rowspan="2">${_('Final prize')}</th>
      <th class="center aligned total-header" rowspan="2">${_('Rank')}</th>
    </tr>
    <tr>
      <th class="center aligned event-header">${_('Net')}</th>
      <th class="center aligned event-header">${_('Total')}</th>
    </tr>
  </thead>
</%def>
