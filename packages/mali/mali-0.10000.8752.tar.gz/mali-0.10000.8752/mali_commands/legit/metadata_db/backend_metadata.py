...
class BackendMetadataDB(BackendMixin, BaseMetadataDB):
    ...
    def query(self, query_text, **kwargs):
        ...
        result = self._handle_api(
            self._config, self._session, 'get', url, retry=default_api_retry(stop_max_attempt_number=self.max_query_retry), is_async=is_async)

        is_live_async_response = is_async and isinstance(result, (list, tuple))
