...
# Installierte Module:
from xml.etree.cElementTree import Element, SubElement, tostring as etree_tostring

# Unitracc:
from ...permissions import Manage_Ads as PERM_MANAGE

# Unitracc-Tools
from visaplan.plone.tools.cfg import get_raw_config
from ...tools.misc import getOption
from visaplan.tools.minifuncs import NoneOrInt, makeBool
from visaplan.tools.sequences import matrixify, next_of
from visaplan.tools.minifuncs import gimmeConst__factory
...
from ...tools.debug import log_result
...
def make_basekeyfunc(key, quot=CACHE_TIMEOUT):
    ...
    def basekey(method, self, relationship, lang=None, *args):
        if lang is None:
            lang = self.context.getAdapter('langcode')()
        return (lang,
                key,
                int(time()) // quot % 32000,
                relationship,
...
class Browser(BrowserView):
    ...
    def canAdd(self):
        ...
        if not self.configure_here(context):
            return False
        return context.getAdapter('checkperm')(PERM_MANAGE)
