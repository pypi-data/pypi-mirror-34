...
class Column(object):
	...
	def eval_types(self, column_value):
		...
		self.dt_eval = False
		self.rowcount += 1
		if column_value is None or (not conf.empty_strings and isinstance(column_value, basestring) and len(column_value.strip()) == 0):
			self.nullrows += 1
			return
		...
		def load_line(line):
			...
			if not conf.empty_strings:
				...
				for i in range(len(line)):
					if line[i] is not None and isinstance(line[i], basestring) and line[i].strip() == u'':
						line[i] = None
...
class CounterVars(object):
	...
	def substitute(self, command_str):
		...
		if m:
			...
			return command_str.replace(u'!!$'+m.group(1)+u'!!', unicode(new_count)), True
...
class MetacommandStmt(object):
	...
	def run(self, localvars=None, commit=False):
		...
		# 'localvars' must be a SubVarSet object.
		# The "commit" argument is to allow an isomorphic interface with 
		# SqlStmt.run() and is not used.
...
class CommandList(object):
	...
	def run_next(self):
		...
		cmditem.command.run(self.paramvals, not status.batch.in_batch())
...
class CompareUI(object):
	def __init__(self, title, message, button_list, headers1, rows1, 
		...
		def make_table(column_headers, rowset, treeview, tableframe):
			...
			# Fill the Treeview table widget with data
			for i in range(len(column_headers)):
				treeview.column(column_headers[i], width=pixwidths[i])
		...
		def find_match(from_table, from_headers, to_table, to_headers):
			# Highlight the first row in to_table that matches the selected row in
			...
			if sel_item is not None and sel_item != '':
				...
				if found:
					...
					to_table.selection_set(to_item)
				else:
					to_table.selection_remove(to_table.focus())
		def match2to1(event):
...
def x_prompt_compare(**kwargs):
	...
	msg = kwargs["msg"]
	db = dbs.current()
	sq_name1 = db.schema_qualified_table_name(schema1, table1)
	sq_name2 = db.schema_qualified_table_name(schema2, table2)
	sql1 = u"select * from %s;" % sq_name1
	sql2 = u"select * from %s;" % sq_name2
	hdrs1, rows1 = db.select_data(sql1)
	hdrs2, rows2 = db.select_data(sql2)
...
metacommands.append(MetaCommand(
	ins_table_rxs(r'^\s*PROMPT\s+COMPARE\s+', ins_table_rxs(r'\s+(?P<orient>AND|BESIDE)\s+', r'\s+PK\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_compare))
...
def x_sub_empty(**kwargs):
	subvars.add_substitution(kwargs['match'], u'')
	return None
...
metacommands.append(MetaCommand(r'^\s*CONFIG\s+EMPTY_STRINGS\s+(?P<yesno>YES|NO)\s*$', x_empty_strings))
...
metacommands.append(MetaCommand(r'^\s*CONFIG\s+BOOLEAN_INT\s+(?P<yesno>YES|NO)\s*$', x_boolean_int))
...
metacommands.append(MetaCommand(r'^\s*CONFIG\s+BOOLEAN_WORDS\s+(?P<yesno>YES|NO)\s*$', x_boolean_words))
...
metacommands.append(MetaCommand(r'^\s*IMPORT_ONLY_COMMON_COLUMNS\s+(?P<yesno>YES|NO)\s*$', x_import_common_cols_only))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+IMPORT_COMMON_COLUMNS_ONLY\s+(?P<yesno>YES|NO)\s*$', x_import_common_cols_only))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+IMPORT_ONLY_COMMON_COLUMNS\s+(?P<yesno>YES|NO)\s*$', x_import_common_cols_only))
...
def x_sub_append(**kwargs):
	subvars.append_substitution(kwargs['match'], kwargs['repl'])
	return None
...
def xf_sub_defined(**kwargs):
	invert = kwargs["invert"]
	invert = False if invert is None else True
	s_exists = subvars.sub_exists(kwargs["match_str"])
	if invert:
...
def read_sqlfile(sql_file_name):
	...
	scriptfile_obj = ScriptFile(sql_file_name, conf.script_encoding)
	sqllist = []
	subscript_stack = []
	currcmd = ''
			...
			if not is_comment_line:
				if metacommand_match:
					...
					if begs:
						...
						cmd = ScriptCmd(sql_file_name, scriptfile_obj.lno, 'cmd', MetacommandStmt(metacommand_match.group('cmd').strip()))
						if len(subscript_stack) == 0:
							sqllist.append(cmd)
						else:
