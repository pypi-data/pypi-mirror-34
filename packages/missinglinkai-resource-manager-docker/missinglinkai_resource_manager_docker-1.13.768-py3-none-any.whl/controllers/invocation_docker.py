...
class InvocationDocker:
    ...
    def git_clone_image(self):
        return self.active_config.general.get('git_image', 'missinglinkai/git-lfs:latest')

    @property
    def shell_image(self):
        return self.active_config.general.get('shell_image', 'library/bash:latest')
    ...
    async def run_mali_command(self, mounts, *args, update_mali=True):
        ...
        logger.info(f"Run mali command {args}: {mounts}")
        return await  self.run_with_callbacks_and_validate_result(
            f'mali_command: {args}',
            image=MALI_IMG,
    ...
    async def setup_root(self):
        ...
        mounts = {self.root_mount[1]: {'bind': "/root"}}
        await  self.pull_image(self.shell_image, force=False)
        return await  self.run_with_callbacks_and_validate_result(
