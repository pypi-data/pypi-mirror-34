...
class fleader():
    ...
    @staticmethod    
    def rwcache(hash,res_text='',suffix='',path='cache'):
        res_text=str(res_text)
        if res_text=='':
            import os
            for root, dirs, fs in os.walk(r'%s/%s'%(path,fleader.time(2))): #目录路径,所有子目录,非目录子文件   
                for f in fs:  
                    if f==hash+suffix:#os.path.splitext()
                        res_text=fleader.rw(os.path.join(root,f))
                        return True,res_text
            return False,''
        else:
            fleader.rw(r'%s/%s/%s%s'%(path,fleader.time(2),hash,suffix), res_text,aw='w')

    @staticmethod
    def cache(suffix='.txt',path='cache'):#类似functools.lru_cache()
        def deco(func):
            def wrapper(*arg, **kw):
                hash=fleader.md5(func.__name__,arg,kw)
                state,res=fleader.rwcache(hash,suffix=suffix,path=path)
                if not state:
                    res=func(*arg, **kw)
                    fleader.rwcache(hash,res,suffix=suffix,path=path)
                return res
            return wrapper
        return deco
