...
def test_minimal_sequence_search_traverses_grid():
    ...
    #         |
    #         *
    assert search._choose_next_qubit(q42, set([q42])) == q41
    assert search._choose_next_qubit(q41, set([q42, q41])) == q40
    assert search._choose_next_qubit(q40, set([q42, q41, q40])) == q30
    assert search._choose_next_qubit(q30, set([q42, q41, q40, q30])) == q20
    assert search._choose_next_qubit(q20, set([q42, q41, q40, q30, q20])) == q10
    assert search._choose_next_qubit(q10,
                                     set([q42, q41, q40, q30, q20, q10])) == q11
    assert search._choose_next_qubit(q11, set([q42, q41, q40, q30, q20, q10,
                                           q11])) is None
...
def test_largest_collect_stops_on_used():
    ...
    start = q02
    search = LargestAreaGreedySequenceSearch(_create_device(qubits), start)
    assert search._collect_unused(start, set([start, q04])) == set([q00, q01, q11, q02,
                                                           q03])
...
def test_greedy_search_method_fails_when_unknown():
    ...
    method = GreedySequenceSearchStrategy(u'fail')
    with pytest.raises(ValueError):
        method.place_line(_create_device(qubits), length)


@mock.patch(u'cirq.line.placement.greedy.LargestAreaGreedySequenceSearch')
@mock.patch(u'cirq.line.placement.greedy.'
            u'MinimalConnectivityGreedySequenceSearch')
def test_greedy_search_method_calls_largest_only(minimal, largest):
    ...
    method = GreedySequenceSearchStrategy(u'largest_area')
...
def test_greedy_search_method_calls_minimal_only(minimal, largest):
    ...
    method = GreedySequenceSearchStrategy(u'minimal_connectivity')
    assert method.place_line(device, length) == GridQubitLineTuple(sequence)
