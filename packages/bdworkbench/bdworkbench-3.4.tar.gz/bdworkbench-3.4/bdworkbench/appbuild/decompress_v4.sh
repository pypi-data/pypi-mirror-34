...
TAR_ENTRY_IMAGE=(@@@@TARENTRYIMAGES@@@@)
...
for image in ${TAR_ENTRY_IMAGE[@]};
do
    if [[ -z ${ENTRY_IMAGE_PATH} ]]; then
        ENTRY_IMAGE_PATH="\"${image}\""
    else
        ENTRY_IMAGE_PATH="${ENTRY_IMAGE_PATH},\"${image}\""
    fi
done
...
print_feed_json() {
    ...
    echo -n         "\"feed_entry_version\":${FEED_ENTRY_VERSION},"
...
print_file_json() {
    ...
    then
        echo -n ",\"images\":[${ENTRY_IMAGE_PATH}]"
    fi
    echo -n "}"
}
...
then
    ...
    echo "       Image OS Types : @@@IMAGEOSTYPES@@@"
