...
class TANEAlgorithm(object):
    ...
    def transformToLists(self, stats):
        ...
        for key in suggested_keys:
            suggested_list.append(list(map(lambda x: int(x),key.split('_'))))
        ...
        for fd in FDs:
            if fd[0] != '':
                FD_lists.append(list(map(lambda x:int(x), fd[0].split('_') + [fd[1]])))
            else:
                FD_lists.append(list(map(lambda x:int(x), [fd[1]])))
