...
from ..functions  import (CHUNKSIZE, FM_THRESHOLD, RTOL, ATOL,
                          FREE_MEMORY, parse_indices, _numpy_allclose,
...
"Can't collapse: Incorrect weights shape {!r}".format(numpy_shape(weight)))
        ...
        if (not _preserve_partitions and
            d._pmndim and d.fits_in_one_chunk_in_memory(d.dtype.itemsize)):
            d.varray
...
#        save = not new.fits_in_memory(new.dtype.itemsize)
       ...
       # p  = new.partitions.matrix.item((0,)*new._pmdim) # "first" partition of new
       # data = d[p.indices[:n_non_collapse_axes] + c_slice]
       # n_data = data.partitions.matrix.size
       # n_new = new.partitions.matrix.size
#
#        if n_new > n_data:
#            "turn off" paralliem in _collapse_subspace
        ...
        for partition in new.partitions.matrix.flat:
            ...
            partition.subarray = d._collapse_subspace(
                ...
                Nmax, mtol, _preserve_partitions=_preserve_partitions, **kwargs)
    ...
    def _collapse_subspace(self, func, fpartial, ffinalise, indices,
                           ...
                           mtol, weights=None,
                           _preserve_partitions=False, **kwargs):
        '''
...
# Turn off?
        if (not _preserve_partitions and 
            data._pmndim and data.fits_in_memory(data.dtype.itemsize)):
            data.varray
