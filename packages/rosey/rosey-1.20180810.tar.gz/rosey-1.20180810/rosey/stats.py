...
def p_value_from_test_dist(model: stats.norm, alpha=0.05):
    """
    Computes the p-value from arbitrary distributions

    >>> samples = 100000
    >>> a_se, b_se = stats.norm(100, 1.5).rvs(samples), stats.norm(101, 3.2).rvs(samples)
    >>> diff = a_se - b_se
    >>> test_dist = stats.norm(*stats.norm.fit(diff))
    >>> pvalue, is_sig = p_value_from_test_dist(test_dist)
    >>> is_sig
    False
    >>> pvalue > 0.05
    True
    >>> a_se, b_se = stats.norm(100, 1.5).rvs(samples), stats.norm(200, 3.2).rvs(samples)
    >>> diff = a_se - b_se
    >>> test_dist = stats.norm(*stats.norm.fit(diff))
    >>> pvalue, is_sig = p_value_from_test_dist(test_dist, alpha=0.01)
    >>> is_sig
    True
    >>> pvalue <= 0.01
    True
    ...
    :param model: Scipy distribution
    :param alpha: Defaults to 0.05
    :return:
    """
    p = min(model.cdf(0), model.sf(0)) * 2
    a = 1 - alpha

    assert 0 <= p <= 1, 'Something went wrong with p-value computation'

    return p, ~(model.ppf(0.5 - a/2) <= 0 <= model.ppf(0.5 + a/2))
