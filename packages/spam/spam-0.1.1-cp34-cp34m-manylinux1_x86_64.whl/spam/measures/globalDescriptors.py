...
def volume(segmented, phase=1, aspectRatio=(1.0, 1.0, 1.0), specific=False, ROI=None):
    ...
    if phase == 0:
        print("spam.measure.globalDescriptors.volume: phase={}. Should be > 0.".format(phase))
...
def eulerCharacteristic(segmented, phase=1, ROI=None, verbose=False):
    ...
    if phase == 0:
        print("spam.measure.globalDescriptors.eulerCharacteristic: phase={}. Should be > 0.".format(phase))
    ...
    if dim == 1:  # DIMENSION 1
        ...
        # Count the edges
        s_x = iman.singleShift(counter, 1, 0, sub=0)
        counter_x = numpy.logical_and(counter, s_x)
        s_y = iman.singleShift(counter, 1, 1, sub=0)
        counter_y = numpy.logical_and(counter, s_y)
        s_z = iman.singleShift(counter, 1, 2, sub=0)
        counter_z = numpy.logical_and(counter, s_z)
        E = counter_x.sum() + counter_y.sum() + counter_z.sum()

        # Count the faces
        s_xy = iman.singleShift(s_x, 1, 1, sub=0)
        f_xy = numpy.logical_and(numpy.logical_and(counter_x, s_xy), counter_y)
        s_xz = iman.singleShift(s_x, 1, 2, sub=0)
        f_xz = numpy.logical_and(numpy.logical_and(counter_x, s_xz), counter_z)
        s_yz = iman.singleShift(s_y, 1, 2, sub=0)
        f_yz = numpy.logical_and(numpy.logical_and(counter_y, s_yz), counter_z)
        F = f_xy.sum() + f_xz.sum() + f_yz.sum()

        # Count the polyhedra
        s_xyz = iman.singleShift(s_xy, 1, 2, sub=0)
...
def totalCurvatureCPP(field, level=0.0, aspectRatio=(1.0, 1.0, 1.0), fileName=None):
    """
    Computes the total curvature of a continuous field over a given level set.
    ...
    # import
    import skimage.measure
    import spam.measures.measuresToolkit as mtk
    ...
    # compute curvature
    gaussCurvatures = numpy.zeros(verts.shape[0]).astype('<f8')
    meanCurvatures = numpy.zeros(verts.shape[0]).astype('<f8')
    areas = numpy.zeros(verts.shape[0]).astype('<f8')

    # call CPP function
    mean_gauss_area = mtk.computeCurvatures( faces.astype(numpy.int), verts.astype('<f8')) 
    mean_gauss_area=numpy.array(mean_gauss_area)

    totalCurvature = numpy.multiply(mean_gauss_area[:,0], mean_gauss_area[:,2]).sum()
    

    if fileName is not None:
        pointData={ "meanCurvature": mean_gauss_area[:,0],
                     "gaussCurvature": mean_gauss_area[:,1],
                     "area": mean_gauss_area[:,2]
        }

        import spam.helpers
        spam.helpers.writeUnstructuredVTK(verts, faces, pointData=pointData, fileName=fileName)

    return totalCurvature


def perimeter(segmented, phase=1, aspectRatio=(1.0, 1.0), ROI=None):
    """Computes the perimetre of a given phase.
    ...
    """
    import spam.morphologicalOperations.binary as mbin

    if phase == 0:
        print("spam.measure.globalDescriptors.perimeter: phase={}. Should be > 0.".format(phase))
    ...
    # take perimeters
    peri = mbin.morphologicalGradient(im_l)

    # compute number of each voxel types
    late = peri & iman.multipleShifts(peri, shifts=[+1, 0])
    late = late + (peri & iman.multipleShifts(peri, shifts=[0, +1]))
    late = late + (peri & iman.multipleShifts(peri, shifts=[-1, 0]))
    late = late + (peri & iman.multipleShifts(peri, shifts=[0, -1]))
    diag = peri & iman.multipleShifts(peri, shifts=[+1, +1])
    diag = diag + (peri & iman.multipleShifts(peri, shifts=[+1, -1]))
    diag = diag + (peri & iman.multipleShifts(peri, shifts=[-1, +1]))
    diag = diag + (peri & iman.multipleShifts(peri, shifts=[-1, -1]))
    inte = diag & late
...
def generic(segmented, n, phase=1, level=0.0, aspectRatio=(1.0, 1.0, 1.0), specific=False, ROI=None, verbose=False):
    ...
    print('spam.measures.globalDescriptors.generic: dim={} (should be between 1 and 3) and n={} (should be between 0 and dim). Return NaN'.format(dim, n))
    return numpy.nan
