...
class ApolloProp(object):
    ...
    def __init__(self, client, key, default=None, namespace='application', fetch_on_cache_miss=False):
        self.client = client
        self.key = key
    ...
    def value(self):
        return self.client.get_value(key=self.key, default=self.default, namespace=self.namespace, fetch_on_cache_miss=self.fetch_on_cache_miss)
...
if __name__ == '__main__':
    client = ApolloClient(app_id=1000, config_server_url='http://127.0.0.1:8080/', file_cache_dir='/Users/sunguangran/tmp/cache').start()
    while True:
        ...
        prop = ApolloProp(client=client, namespace='application', key=key, default="")
