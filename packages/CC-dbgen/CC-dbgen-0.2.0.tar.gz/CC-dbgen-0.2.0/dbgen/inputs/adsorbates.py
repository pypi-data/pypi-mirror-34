...
from dbgen.support.datatypes.rule      import Rule,Plan,PureSQL,PyBlock,SqlBlock
...
pop_struct_ads = Rule('pop_struct_ads'
    ...
    ,plan= Plan([PyBlock(json_to_graph,args=[Arg('geo_graph')])
                ,PyBlock(identify_ads, 'ads', [Arg('json_to_graph')])
                ,SqlBlock(name = 'adsorbate_id'
                       ,text = 'SELECT A.adsorbate_id FROM adsorbate AS A WHERE A.name=%s'
                       ,args = [Arg('ads',4)])
                ,SqlBlock(name='adsorbate_id_many'
                       ,text ='SELECT A.adsorbate_id FROM adsorbate AS A WHERE A.name=%s'
                       ,args = [Arg('ads',2)])
                ,SqlBlock(name = 'insert_struct_ads'
                       ,text = mkInsCmd('struct_adsorbate',struct_ads_cols,sqlite=False)
                       ,args = [Arg('struct_id')
                              ,Arg('adsorbate_id')
                              ,Arg('ads',3)
                              ,Arg('ads',5)])
                ,SqlBlock(name = 'update_atom_fk'
