...
class Sample:
    ...
    def print_history(self):
        ...
        for index, event in enumerate(history):
            print('-'*int((30-len(event['date']))/2) + event['date'] + '-'*int((30-len(event['date']))/2))
            for key in event:
                if str(key) != 'date':
    ...
    def get_measurements(self):
        if len(self.measurements) == 0 and len(self.get_history()) > 0:
            raise ImportWarning('No measurements downloaded. Download measurements by calling "sample.load_measurements_from_db(db)".')
