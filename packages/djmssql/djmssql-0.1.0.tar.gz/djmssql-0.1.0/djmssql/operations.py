...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    def date_trunc_sql(self, lookup_type, field_name):
        ...
        if lookup_type == 'week':
            return "CONVERT(datetime2, DATEADD(day, CASE DATEPART(dw, %s) WHEN 1 THEN -6 ELSE -(DATEPART(dw, %s)-2) END, CAST(%s as date)))" % (field_name, field_name, field_name)
        if lookup_type == 'day':
    ...
    def limit_offset_sql(self, low_mark, high_mark):
        ...
        return '%s%s' % (
            ' OFFSET %d ROWS' % (0 if offset is None else offset,),
            ' FETCH NEXT %d ROWS ONLY' % (999999999 if fetch is None else fetch,),
        )
    ...
    def savepoint_commit_sql(self, sid):
        ...
        """
        return self.savepoint_create_sql(sid)
    ...
    def adapt_datetimefield_value(self, value):
        ...
        if timezone.is_aware(value):
            if settings.USE_TZ:
                ...
                raise ValueError("djmssql backend does not support timezone-aware datetimes when USE_TZ is False.")
        return str(value)
    ...
    def adapt_timefield_value(self, value):
        ...
        if timezone.is_aware(value):
            raise ValueError("djmssql backend does not support timezone-aware times.")
    ...
    def check_expression_support(self, expression):
        ...
        from django.db.models.functions import ATan2, Ln, Log, Mod, Substr, Repeat, Chr
        if isinstance(expression, ATan2):
            expression.function = 'ATN2'
        elif isinstance(expression, Ln):
            expression.function = 'LOG'
        elif isinstance(expression, Substr):
