...
from kazoo_sasl.exceptions import (
    AuthFailedError,
    ConfigurationError,
...
from kazoo_sasl.handlers.threading import SequentialThreadingHandler
from kazoo_sasl.handlers.utils import capture_exceptions, wrap
from kazoo_sasl.hosts import collect_hosts
from kazoo_sasl.loggingsupport import BLATHER
from kazoo_sasl.protocol.connection import ConnectionHandler
from kazoo_sasl.protocol.paths import _prefix_root, normpath
from kazoo_sasl.protocol.serialization import (
    Auth,
    CheckVersion,
    CloseInstance,
    Create,
...
from kazoo_sasl.protocol.states import (
    Callback,
    EventType,
    KazooState,
    KeeperState,
...
from kazoo_sasl.security import ACL, OPEN_ACL_UNSAFE

# convenience API
from kazoo_sasl.recipe.barrier import Barrier, DoubleBarrier
from kazoo_sasl.recipe.counter import Counter
from kazoo_sasl.recipe.election import Election
from kazoo_sasl.recipe.lease import NonBlockingLease, MultiNonBlockingLease
from kazoo_sasl.recipe.lock import Lock, ReadLock, WriteLock, Semaphore
from kazoo_sasl.recipe.partitioner import SetPartitioner
from kazoo_sasl.recipe.party import Party, ShallowParty
from kazoo_sasl.recipe.queue import Queue, LockingQueue
from kazoo_sasl.recipe.watchers import ChildrenWatch, DataWatch
...
class KazooClient(object):
    ...
    def __init__(self, hosts='127.0.0.1:2181',
        ...
        :param handler: An instance of a class implementing the
                        :class:`~kazoo_sasl.interfaces.IHandler` interface
        ...
        :param connection_retry:
            A :class:`kazoo_sasl.retry.KazooRetry` object to use for
    ...
    def add_listener(self, listener):
        ...
        :class:`~kazoo_sasl.protocol.states.KazooState` instance indicating
        ...
        .. warning::
            ...
            than the :meth:`~kazoo_sasl.interfaces.IHandler.spawn` method
    ...
    def start(self, timeout=15):
        ...
        :param timeout: Time in seconds to wait for connection to
                        succeed.
        :raises: :attr:`~kazoo_sasl.interfaces.IHandler.timeout_exception`
    ...
    def add_auth(self, scheme, credential):
        ...
        :raises:
            :exc:`~kazoo_sasl.exceptions.AuthFailedError` if it failed though
    ...
    def add_auth_async(self, scheme, credential):
        ...
        :rtype: :class:`~kazoo_sasl.interfaces.IAsyncResult`
    ...
    def sync(self, path):
        ...
        :raises:
            :exc:`~kazoo_sasl.exceptions.ZookeeperError` if the server
    ...
    def create(self, path, value=b"", acl=None, ephemeral=False,
        ...
        :param acl: :class:`~kazoo_sasl.security.ACL` list.
        ...
        :raises:
            :exc:`~kazoo_sasl.exceptions.NodeExistsError` if the node
            already exists.

            :exc:`~kazoo_sasl.exceptions.NoNodeError` if parent nodes are
            missing.

            :exc:`~kazoo_sasl.exceptions.NoChildrenForEphemeralsError` if
            the parent node is an ephemeral node.

            :exc:`~kazoo_sasl.exceptions.ZookeeperError` if the provided
            value is too large.
    ...
    def exists(self, path, watch=None):
        ...
        :returns: ZnodeStat of the node if it exists, else None if the
                  node does not exist.
        :rtype: :class:`~kazoo_sasl.protocol.states.ZnodeStat` or `None`.
    ...
    def get(self, path, watch=None):
        ...
        :param watch: Optional watch callback to set for future changes
                      to this path.
        :returns:
            Tuple (value, :class:`~kazoo_sasl.protocol.states.ZnodeStat`) of
            node.
        :rtype: tuple

        :raises:
            :exc:`~kazoo_sasl.exceptions.NoNodeError` if the node doesn't
            exist
    ...
    def get_children(self, path, watch=None, include_data=False):
        ...
        :param watch: Optional watch callback to set for future changes
                      to this path.
        :param include_data:
            Include the :class:`~kazoo_sasl.protocol.states.ZnodeStat` of
    ...
    def get_acls(self, path):
        ...
        :rtype: tuple of (:class:`~kazoo_sasl.security.ACL` list,
                :class:`~kazoo_sasl.protocol.states.ZnodeStat`)
        :raises:
            :exc:`~kazoo_sasl.exceptions.NoNodeError` if the node doesn't
            exist.
    ...
    def set_acls(self, path, acls, version=-1):
        ...
        :param acls: List of :class:`~kazoo_sasl.security.ACL` objects to
                     set.
        :param version: The expected node version that must match.
        :returns: The stat of the node.
        :raises:
            :exc:`~kazoo_sasl.exceptions.BadVersionError` if version doesn't
            match.

            :exc:`~kazoo_sasl.exceptions.NoNodeError` if the node doesn't
            exist.

            :exc:`~kazoo_sasl.exceptions.InvalidACLError` if the ACL is
            invalid.
    ...
    def set(self, path, value, version=-1):
        ...
        :returns: Updated :class:`~kazoo_sasl.protocol.states.ZnodeStat` of
                  the node.
    ...
    def delete(self, path, version=-1, recursive=False):
        ...
        :raises:
            ...
            :exc:`~kazoo_sasl.exceptions.NotEmptyError` if the node has
            children.
    ...
    def reconfig(self, joining, leaving, new_members, from_config=-1):
        ...
        :raises:
            :exc:`~kazoo_sasl.exceptions.UnimplementedError` if not supported.

            :exc:`~kazoo_sasl.exceptions.NewConfigNoQuorumError` if no quorum of new
            ...
            :exc:`~kazoo_sasl.exceptions.ReconfigInProcessError` if another
            reconfiguration is in progress.

            :exc:`~kazoo_sasl.exceptions.BadVersionError` if version doesn't
            match.

            :exc:`~kazoo_sasl.exceptions.BadArgumentsError` if any of the given
