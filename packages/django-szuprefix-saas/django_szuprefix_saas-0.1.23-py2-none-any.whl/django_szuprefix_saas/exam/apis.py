...
class PaperViewSet(PartyMixin, UserApiMixin, viewsets.ModelViewSet):
    ...
    def for_course(self, request):
        ...
        papers = models.Paper.objects.filter(course_chapters__course_id=course_id)
        page = self.paginate_queryset(papers)
        serializer = serializers.PaperSerializer(page, many=True)
    ...
    def for_student(self, request):
        ...
        performances = dict([(p.id, model_to_dict(p, ['score', 'detail'])) for p in
        ...
        for c in courses:
            ...
            for cpt in c['chapters']:
                for p in cpt['exam_papers']:
                    ...
                    if unlock == False and performance is None:
                        unlock = True
                        p['unlock'] = True
