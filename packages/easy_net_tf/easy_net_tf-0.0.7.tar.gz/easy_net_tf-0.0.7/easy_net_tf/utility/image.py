...
class UtilityImage:
    ...
    def normalize(image_in, low, high, mode=NORMALIZE_DEFAULT):
        ...
        # normalize each channel separately to [-1, 1]
            ...
            for channel in range(channels):
                ...
                image_out[:, :, channel] = ((image_out[:, :, channel] - minimum) / (
                        maximum - minimum)) * (high - low) + low
        ...
        # normalize all channels together to [-1, 1]
            ...
            image_out = ((image_out - minimum) / (maximum - minimum)) * (high - low) + low
    ...
    @staticmethod
    def add_coordinate(images_in,
                       add_r=False,
                       normalize=True,
                       debug=False):
        """

        :param images_in: a batch of images
        ...
        prepare
        """
        image_out = numpy.copy(images_in)
