...
class Repository(WMLResource):
    ...
    def store_experiment(self, meta_props):
        ...
        if any(u'training_definition_url' not in x for x in meta_props[self.ExperimentMetaNames.TRAINING_REFERENCES]):
            raise MissingMetaProp(u'training_references.training_definition_url')

        for ref in meta_props[self.ExperimentMetaNames.TRAINING_REFERENCES]:
        ...
        # TODO workaround to make things works always
        payload = self.ExperimentMetaNames._generate_resource_metadata(meta_props)
        if 'author' not in payload['settings']:
            payload['settings']['author'] = {}
    ...
    def store_definition(self, training_definition, meta_props):
        ...
        response_definition_post = requests.post(
            ...
            json=self.DefinitionMetaNames._generate_resource_metadata(meta_props, with_validation=True),
            headers=self._client._get_headers()
        )
    ...
    def _publish_from_training(self, model_uid, meta_props, training_data=None, training_target=None):
        """
        ...
        if re.match('^training-[0-9a-zA-Z_-]{9}$', model_uid) is not None:
            prefix = model_uid
        else:
            res = re.search('^(training-[0-9a-zA-Z_-]{9})_([0-9]+)$', model_uid)
        ...
        # Is useful while debuging
        # bucket = s3.Bucket(bucket_name)
        # for object in bucket.objects.filter(Prefix=prefix):
        #     if object.key[-1:] is not '/':
        #         if re.match('[^\.]+\.h(df)?5', object.key) is not None:
        #             is_keras = True
        #         print('key', object.key)

        if self.ModelMetaNames.FRAMEWORK_NAME not in meta_props:
            meta_props[self.ModelMetaNames.FRAMEWORK_NAME] = details['entity']['model_definition']['framework']['name']

        if self.ModelMetaNames.FRAMEWORK_VERSION not in meta_props:
            meta_props[self.ModelMetaNames.FRAMEWORK_VERSION] = details['entity']['model_definition']['framework']['version']

        if is_keras:
            if meta_props[self.ModelMetaNames.FRAMEWORK_NAME] is not 'tensorflow':
                raise WMLClientError('Detected \'keras\' model while framework name is set to \'{}\'.'.format(meta_props[self.ModelMetaNames.FRAMEWORK_NAME]))

            # TODO find better solution with Niall
            if self.ModelMetaNames.FRAMEWORK_VERSION is '1.2' or self.ModelMetaNames.FRAMEWORK_VERSION is '1.3':
                keras_version = '2.0.8'
            elif self.ModelMetaNames.FRAMEWORK_VERSION is '1.4':
                keras_version = '2.1.2'
            elif self.ModelMetaNames.FRAMEWORK_VERSION is '1.5':
                keras_version = '2.1.3'
            elif self.ModelMetaNames.FRAMEWORK_VERSION is '1.6':
                keras_version = '2.1.4'
            else:
                keras_version = '2.1.5'
            ...
            if meta_props[self.ModelMetaNames.FRAMEWORK_LIBRARIES] is None:
                meta_props[self.ModelMetaNames.FRAMEWORK_LIBRARIES] = [keras_entry]
            else:
                meta_props[self.ModelMetaNames.FRAMEWORK_LIBRARIES].append(keras_entry)

        model_meta = self.ModelMetaNames._generate_resource_metadata(
            meta_props,
            client=self._client,
        ...
        creation_response = requests.post(
            self._wml_credentials['url'] + '/v3/ml_assets/models', # todo
            headers=self._client._get_headers(),
            json=model_meta
        )
    ...
    def store_model(self, model, meta_props=None, training_data=None, training_target=None, pipeline=None):
        ...
        if not isinstance(model, STR_TYPE):
            ...
            if (os.path.sep in model) or os.path.isfile(model) or os.path.isdir(model):
                ...
                saved_model = self._publish_from_training(model_uid=model, meta_props=meta_props, training_data=training_data, training_target=training_target)
...
""".format(code, function.__name__)
        ...
        try:
            if self.FunctionMetaNames.RUNTIME_UID not in meta_props:
                ...
                meta_props[self.FunctionMetaNames.RUNTIME_UID] = runtime_uid

            function_metadata = self.FunctionMetaNames._generate_resource_metadata(meta_props, client=self._client)
