...
class CNN:
    ...
    def __init__(self, model_path = None, class_dictionary_path = None):
        """Initializes CNN model with data type specification.

        If you want to load a model you've already trained then just give a path of model file.
        Otherwise, you should call self.prepare_train_data() before training your model.
    ...
    def prepare_train_data(self, is_image_data = True, get_image_from = 'directory', data_path = None, data_array = None, label_array = None):
        ...
        Keyword Arguments:
            ...
            data_array {ndarray} --
                ...
                (default: {None})
            label_array {ndarray} -- ndarray. An array of labels. (default: {None})
        """
    ...
    def get_num_of_classes(self, label_array):    
        unique, counts = numpy.lib.arraysetops.unique(label_array, return_counts=True)
        return len(counts)

    def create_class_indices(self, unique_labels):
        class_indices = dict()
        for i in range(unique_labels.shape[0]):
            class_indices[unique_labels[i]] = i
        return class_indices

    def prepare_y(self, label_array):
        """Prepares y for a keras model.(ex. y_train)
        
        Arguments:
            label_array {ndarray} -- An array of labels.
        
        Returns:
            [ndarray] -- A binary matrix representation of the input.
        """
        unique_labels = numpy.lib.arraysetops.unique(label_array)
        class_indices = self.create_class_indices(unique_labels)
        self.class_dictionary = reverse_dict(class_indices)
        
        y_list = list()
        for i in range(label_array.shape[0]):
            label_class = class_indices[label_array[i]]
            y_list.append(label_class)
        
        y = keras.utils.to_categorical(numpy.array(y_list))
        return y
    ...
    def train(self,
        ...
        if self.is_image_data == True:
            ...
            if self.get_image_from == 'directory':
                self.train_with_directory_images(train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle)
            elif self.get_image_from == 'argument':
                self.train_with_argument_images(train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator)
        else:
            num_classes = self.get_num_of_classes(self.label_array)
            y_train = self.prepare_y(self.label_array)
            if type(self.model) == type(None):
    ...
    def train_with_directory_images(self, train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle):
        train_generator = train_datagen.flow_from_directory(self.data_path, target_size = self.input_shape[:2], class_mode = 'categorical')
        self.class_dictionary = reverse_dict(train_generator.class_indices)
        if type(self.model) == type(None):
    ...
    def train_with_argument_images(self, train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator):
        num_classes = self.get_num_of_classes(self.label_array)
        y_train = self.prepare_y(self.label_array)
        if type(self.model) == type(None):
            self.model = self.create_model( num_classes )
    ...
    def save(self, model_path, class_dictionary_path):
        ...
        Raises:
            e -- Raises error if a model or path is wrong.
        """
        if type(self.model) == type(None):
            print('you should train your model before save it')
        try:
    ...
    def save_class_dictionary(self, class_dictionary_path):
        if type(self.class_dictionary) == type({}): # check class_dictionary type
            ...
            if type(class_dictionary_path) == type(''):
                raise TypeError('wrong class_dictionary type. maybe you should train a model with image data')  

    def load(self, model_path, class_dictionary_path = None):
        """Loads a model and a class_dictionary(optional) from a file system.
        If you don't specify a class_dictionary path, you may get weird integer labels as a prediction result.  
        
        Arguments:
            model_path {str} -- Path of saved model.
        
        Keyword Arguments:
            class_dictionary_path {str or None} -- Path of saved class_dictionary (default: {None})
    ...
    def predict(self, data, is_image_data=True):
        ...
        Keyword Arguments:
            is_image_data {bool} -- Check a type of data. (default: {True})
        ...
        try:
            if data.shape[1:] != self.input_shape:
                reshaped_data = self.reshape_data(data, is_image_data)
            else:
                reshaped_data = data
            probs = self.model.predict(reshaped_data)
            predicted_classes = probs.argmax(axis=-1)
            if type(self.class_dictionary) == type({}):
    ...
    def evaluate(self, data_array, label_array, is_image_data=True):
        """Returns test results of data_array.
        This function only works when self.class_dictionary exists.
        ...
        Arguments:
            data_array {ndarray} --
                ...
                (default: {None})
            label_array {ndarray} --
                ndarray. Actual labels of data_array's data.
                (default: {None})
        ...
        Raises:
            e -- Raise exception when evaluation failed.
        ...
        try:
            if data_array.shape[1:] != self.input_shape:
                reshaped_data = self.reshape_data(data_array, is_image_data)
            else:
                reshaped_data = data_array

            class_indices = reverse_dict(self.class_dictionary)
            y_list = list()
            for i in range(label_array.shape[0]):
                label_class = class_indices[label_array[i]]
                y_list.append(label_class)
            ...
            return self.model.evaluate(reshaped_data, y)
        except Exception as e:
            raise e