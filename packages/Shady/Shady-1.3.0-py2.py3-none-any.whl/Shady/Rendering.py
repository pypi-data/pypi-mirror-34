...
__all__ = [
	...
	'SINEWAVE_SIG', 'SINEWAVE_MOD', 'HANN_WIN',
	'SIGFUNC', 'MODFUNC', 'WINFUNC',
	'DRAWMODE',
	'LoadImage',
	'Require',
]
...
class World( LinkGL ):
	...
	_doc_cut = """
		...
		profile:
			...
			exact profiler: `'profile'`, `'cProfile'`, `'pyinstrument'` or 
			`'hotshot'` (those last two require the correspondingly-named third-
			party packages to be installed).  Examine `self.profile` after
			running the world.
		window:
	...
	def _Construct( self, width, height, window, left, top, screen, canvas, frame, fullScreenMode, visible, openglContextVersion, debugTiming, profile, **kwargs ):
		...
		if self.logger: self.logger.Log( world_size=list( self.size ) )
	...
	lookupTableTextureSize = ManagedProperty( [ -1, -1, -1 ] )     # also placeholder properties for sharing with canvas
...
class Stimulus( LinkGL ):
	...
	def SwitchTo( self, key ):
		...
		bundle of properties that determine texture, enevelope size and
...
class LookupTable( LinkGL ):
	...
	length = property( lambda self: max( 0, self.lookupTableTextureSize[ -1 ] ) )
	...
	def LoadValues( self, values ):
		...
		self.values = values
		textureData = values.reshape( [ width, height, values.shape[ 2 ] ] ).transpose( [ 1, 0, 2 ] )
		
		self.lookupTableTextureSize = [ textureData.shape[ 1 ], textureData.shape[ 0 ], length ]
		if self._accel:
