...
def dict_recursive_mapper(d, fn, copy=True, include_private=False, level=tuple()):
    if copy:
        d = deepcopy(d)
    ...
    if isinstance(d, dict):
        for attr, value in d.items():
            if not include_private or not attr.startswith('_'):
                ...
                d[attr] = dict_recursive_mapper(value, fn, copy, level=new_level)

        value = d
    elif isinstance(d, (list, tuple, set)):
        value = d.__class__(
            dict_recursive_mapper(value, fn, copy, level=(level + (idx,)))
            for idx, value
            in enumerate(d)
        )
    else:
        if 'level' in inspect.signature(fn).parameters.keys():
...
def create_result_csv_streamer(results):
    """
