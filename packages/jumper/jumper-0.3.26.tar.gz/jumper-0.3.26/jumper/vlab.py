...
class Vlab  (object):
    ...
    def __init__(
        ...
        self._uart = JemuUart(self) if platform is None else JemuUart(self, mcu_name=platform)
    ...
    def _build_components_methods(self):
        ...
        default_stm32l4_json = os.path.join(os.path.dirname(__file__), "stm32l4_default_board.json")
        ...
        if os.path.isfile(board_json):
            ...
            if self._platform == "stm32f4":
                components_list, self._platform = self._parse_bsp_json(default_stm32f4_json)
            elif self._platform == "stm32l4":
                components_list, self._platform = self._parse_bsp_json(default_stm32l4_json)
            elif self._platform == "nrf52832":
    ...
    def load(self, file_path):
        ...
        if not self._local_jemu:
            self._web_api.upload_file(self._firmware)
