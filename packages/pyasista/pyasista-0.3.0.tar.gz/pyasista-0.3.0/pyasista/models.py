...
# Enumeration of supported languages (TR, EN, AR)
class Language(Enum):
    """A case-sensitive enumeration of supported languages.
    ...
    # A static method returning the related enum for the given language string
    # For unrecognized languages, fallbacks to Turkish.
    @staticmethod
    def get(language):
        """A static method to parse an object to Language instance.
        If the object is not a valid language, fallback to Language.TURKISH.

        :param language: An object, the key for the Language.
        :rtype: Language
        ...
        Usage::
            ...
            >>> from pyasista import Language
            >>> language = 'ILLEGAL'
            >>> lang = Language.get(language)  # Fallback: TURKISH
            <Language.TURKISH: 'TR'>
        """
...
# Enumeration of supported environments (DEVELOPMENT, TESTING, PROD)
...
class AsistaToken:
    ...
    def is_valid(self):
        ...
        if (isinstance(self.__access_token, str) and
                ...
                isinstance(self.__asista_refresh_token_uri, str)):
            if re.match(AsistaToken.__URL_PATTERN,
...
class AsistaInput:
    ...
    Usage::
        ...
        ...     print(asista_input.language)  # Language.TURKISH
        ...     print(asista_input.serial_number)  # 'QNUBQWERTY123'
        ...     return dict()
    """
    def __init__(self, event, required_parameters=list()):
        def __parse_slot_values(slot_values):
            parameters = dict()
            if isinstance(slot_values, list):
                for slot_value in slot_values:
                    key, value = __parse_slot_value(slot_value)
        ...
        self.__language = Language.get(event['language'])
        self.__serial_number = event.get('serial_number', None)
        self.__slot_values = __parse_slot_values(
    ...
    def __init__(self, command_type, payload=None):
        ...
        if (self.__command == CommandType.NOACTION or
            self.__command == CommandType.STOP or
                payload is None):
            self.__payload = dict()
        elif self.__command == CommandType.PLAY_ANNOUNCE:
            self.__payload = {'data': payload}
        elif self.__command == CommandType.PLAY_STREAM:
            self.__payload = {'data': payload}
