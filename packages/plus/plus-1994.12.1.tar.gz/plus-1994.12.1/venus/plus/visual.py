...
from venus.utils import get_layer, get_para
...
from venus.logger.visdomlogger import VisdomPlotLogger
...
class BasePlotter(object):
    ...
    def update_plot(self):
        for ele in self.plotters.values():
            ele.plot()

    def regis_plot(self, name, plotter):
        # TODO: Add frequency utility, plot each ele in different frequency
        assert name not in self.plotters
...
class VisdomPlotter(BasePlotter):
    ...
    def __init__(self, env, server='localhost', port=8097):
    ...
    def regis_visdom_logger(self, plot_type, opts):
        assert opts['title'] not in self.plotters, "the title already exists"
        title = opts['title']


        plotter = VisdomPlotLogger(plot_type, port=self.port, env=self.env, server=self.server, opts=opts)
        self.plotters[title] = plotter
...
class VisualManager(object):
    ...
    def create_visdom_viz(self, name, env, port, server):
        viz = VisdomPlotter(env, server, port)
        assert name not in self.viz_dict, "Visual Manager: the name in viz_dict already exist"
        self.viz_dict[name] = viz
...
def create_viz(name, env='debug', port=8097, server='http://localhost', backend='visdom'):
    name = name if name else 'root'
    if backend == 'visdom':
        viz_manager.create_visdom_viz(name, env, port, server)
