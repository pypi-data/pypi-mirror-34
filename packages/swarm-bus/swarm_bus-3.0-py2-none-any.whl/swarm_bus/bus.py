...
class AMQPConnector(object):
    ...
    def connect(self):
        ...
        logger.info(
            "[AMQP] Connected to %s, using '%s' prefix",
    ...
    def normalize_transport(self):
        ...
        if 'priorities' not in self.transport:
            self.transport['priorities'] = ['low', 'medium', 'hight']
...
class AMQPConsumer(object):
    ...
    def consume(self, queue_name, callbacks=None, error_handler=None):
        ...
        with self._connection.Consumer(queues=queues, callbacks=callbacks):
            logger.debug('[AMQP] Consuming on %s', queue_name)
            while True:
                if self.can_consume:
                    ...
                    logger.debug(
                        '[AMQP] Consuming is on hold. Next fetch in 60s'
                    )
                    sleeping_time = 60
        ...
        def exception_catcher(body, message):
            ...
            try:
                ...
                if error_handler:
                    ...
                    logger.debug('[AMQP] Error handler called')
