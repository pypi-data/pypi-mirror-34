...
u"""Ways to wrap values so that they match desired interfaces."""
...
T_ACTUAL = TypeVar(u'T_ACTUAL')
T_DESIRED = TypeVar(u'T_DESIRED')
CASTER = Callable[[u'Extensions', Any], Optional[Any]]


class Extensions(object):
    u"""Specifies ways to wrap values so that they match desired interfaces."""

    def __init__(
            self,
            desired_to_actual_to_caster=None
            ):
        u"""Specifies extensions.
    ...
    def add_recursive_cast(
        ...
        u"""Adds a multi-step way to turn one type of thing into another.
        ...
        if not overwrite_existing:
            for t in all_desired_types:
                if self._have(t, actual_type):
                    raise ValueError(
                        u'Already have a way to cast {} into {}.'.format(
                            actual_type, t))
    ...
    def add_cast(self,
                 ...
                 also_add_inherited_conversions = True,
                 overwrite_existing = False):
        u"""Adds a way to turn one type of thing into another.
    ...
    def can_cast(self,
                 desired_type,
                 actual_value):
        u"""Is it possible to turn the given value into the desired type?
    ...
    def try_cast(self,
                 desired_type,
                 actual_value):
        u"""Represents the given value as the desired type, if possible.
    ...
    def cast(self,
        ...
        if result is None:
            raise TypeError(u'Expected a {} but got {}'.format(
                desired_type,
                repr(actual_value)))
        return result
