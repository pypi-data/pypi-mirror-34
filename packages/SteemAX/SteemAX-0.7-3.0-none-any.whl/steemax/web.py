...
class Web:
    ...
    def info_page(self, account):
        ...
        for value in axlist:
            ...
            if int(value[7]) == -1:
                buttoncode = '''
                <div class="button" onClick="command('{}', '{}', '{}')">Start</div>'''.format(
                                value[0], 
                                otheraccount,
                                "start")
            if int(value[7]) == 0 and invitee == 1:
                buttoncode = '''
                <div class="button" onClick="command('{}', '{}', '{}')">Accept</div>'''.format(
                                value[0],
                                otheraccount,
                                "accept")
            if (int(value[7]) == 0 and invitee == 0) or int(value[7]) > 1:
                buttoncode = buttoncode + '''
                <div class="button" onClick="barter_window('{}');">Barter</div>'''.format(value[0])
            if int(value[7]) == 1:
                buttoncode = '''
