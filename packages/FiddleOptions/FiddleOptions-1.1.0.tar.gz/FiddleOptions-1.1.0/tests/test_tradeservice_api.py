...
from mercury.client.model import OptionSingle, VerticalSpread
...
class TestTradeserviceApi(unittest.TestCase):
    ...
    def _create_condor_position(self, type=OptionContractType.PUT):
    ...
    def _generate_test_positions(self):
        return [self._create_iron_condor_position(),
    ...
    def test_iron_condor_strike_exception(self):
        with self.assertRaises(CondorValueException) as ctx:
            ic = IronCondor(self.equity, self.open_date, 10,
                            out_leg_call=OptionLeg(strike=1, quantity=1),
                            in_leg_call=OptionLeg(strike=2, quantity=-1),
                            in_leg_put=OptionLeg(strike=2, quantity=-1),
                            out_leg_put=OptionLeg(strike=1, quantity=1))
