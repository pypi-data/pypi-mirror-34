    ...
    def build(self):
        ...
        for button in menu_in['buttons']:
            ...
            if (button['name']=='Export Grid'):
                name = button['name'].lower().replace(' ', '-')+'-'+str(self.obj._id)
                if 'flex_css' in button:
                    button['container_css']=FlexboxCSS(button['flex_css'], kind='item').build_css('container-'+name, opt_dic={})
                    button['css'] = FlexboxCSS({}, kind='item').build_css(name, opt_dic=options)
                else:
                    button['container_css']=menu_in['button_default_flex_css'].build_css('container-'+name, opt_dic={})
                    button['css'] = FlexboxCSS({}, kind='item').build_css(name, opt_dic=options)
            else:
                name = button['name'].lower().replace(' ', '-')+'-'+str(self.obj._id)
                if 'flex_css' in button:
                    button['css']=FlexboxCSS(button['flex_css'], kind='item').build_css(name, opt_dic=options)
                else:
        ...
        if not ('input_div_css' in menu_in):
            menu_in['input_div_css']=FlexboxCSS({'align-items':'baseline'}, kind='container')
        else:
