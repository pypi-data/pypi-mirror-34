...
HTTP_STATUS_CLIENT_ERROR_UNAUTHORIZED = 401

# 403 Forbidden
# The request was valid, but the server is refusing action.
# The user might not have the necessary permissions for a resource,
# or may need an account of some sort.
HTTP_STATUS_CLIENT_ERROR_FORBIDDEN = 403

# 500 Internal Server Error
# A generic error message, given when an unexpected condition was encountered
# and no more specific message is suitable.
HTTP_STATUS_SERVER_ERROR_INTERNAL_SERVER_ERROR = 500
...
def yamlizable(d):
    r"""Normalize a nested datastructure to be serialized by yaml.

    Arguments:
        d: a nested dict, list, tuple, or namedtuple.

    Returns:
        Normalized form of the input datastructure, that uses only dicts,
        lists, and builtin types.
    """
    if hasattr(d, '_asdict') and hasattr(d, '_make') and not isinstance(d, type):  # namedtuple
        return {k: yamlizable(v) for (k, v) in d._asdict().items()}
    elif isinstance(d, APISpec):
        return {k: yamlizable(v) for (k, v) in d.to_dict().items()}
        ...
        return {k: yamlizable(v) for (k, v) in d.items()}
    elif isinstance(d, (list, tuple)):
        return list(yamlizable(x) for x in d)
    else:
        return d
...
def dict_print(n, d):
    ...
    print('%s:' % n)
    print(json.dumps(yamlizable(d), sort_keys=True, indent=4, separators=(', ', ': ')))
    print(' ')
...
def resource_deploy(
        ...
        user_id,
        resource_type,
# TODO:  ADD LIMIT USING A CUSTOM, HACKED-UP VALIDATION METHOD
        name,
# TODO:  ADD LIMIT USING A CUSTOM, HACKED-UP VALIDATION METHOD
        tag,
        path, # TODO: THIS IS THE LOCAL path OF THE USER'S HOME LAPTOP FILE SYSTEM
              # TODO: THIS IS THE MOST IMPORTANT ITEM HERE
        framework_type,
        runtime,
        chip,
        template=None, # THIS IS THE 2nd MOST IMPORTANT ITEM HERE
        verify=False,
        cert=None,
        overwrite=True,
        ...
        # TODO: WHAT HAPPENED TO THE FANCY resource_deploy() METHOD?  ARE WE NOT USING THAT?
        ...
        status_code = HTTP_STATUS_SUCCESS_CREATED
        name = _validate_and_prep_model_name(name)
        tag = _validate_and_prep_model_tag(tag)

        # NOTE: THIS IS LOCAL path ON A USER'S HOME LAPTOP FILE SYSTEM
        ...
        # NOTE:  THIS IS LOCAL absolute_path ON A USER'S HOME LAPTOP FILE SYSTEM
        ...
        print('Compressing generated source code into tar archive.')

        # NOTE:  THIS IS LOCAL archive_path ON A USER'S HOME LAPTOP FILE SYSTEM
        ...
        print('LOCAL LAPTOP FILE SYSTEM absolute_path: %s' % absolute_path)
        print('LOCAL LAPTOP FILE SYSTEM archive_path: %s' % archive_path)

        # TODO THIS IS THE PIPELINE.AI server_path ON THE PipelineAI SERVER FILE SYSTEM
        #      THIS PATH REPRESENTS WHERE THE FILE WILL LIVE ON PipelineAI SERVER FILE SYSTEM
        #      THIS VALUE IS RELATIVE TO /root/pipelineai/models/...
        # TODO: WE WILL REMOVE THIS FROM THE CLI, BUT WE ARE KEEPING IT FOR NOW.
        #       AT THE MOMENT, THIS IS ARTIFICIALLY GENERATED 
        # 
        server_path = '%s/%s-%s/' % (framework_type, name, tag)
        
        # *********** resource_source_init ********************************
        print('''
        Initializing resource
        Creating required directories and generating deployment and service resource definitions.''')
        endpoint = '/resource-source-init'

        url = api_url + endpoint

        body = {
# TODO:  WE NEED user_id EVERYWHERE - EITHER EXPLICITLY (FOR MONDAY WEBINAR)
#        OR IMPLICTLY (WITH AUTH0 LOGIN AFTER MONDAY WEBINARS)
            # 'user_id': user_id,
...
# TODO:  Why is this being passed from the cli-local/ cli => api-private/ REST API?
#        This should only affect the timeout for the call from the (local) cli => (remote) REST API
        ...
        # *********** resource_archive_upload ********************************
        print('Uploading source code archive...')

        endpoint = '/resource-archive-upload'

        url = api_url + endpoint
        print(url)

        # THIS IS THE MAGIC OF requests LIBRARY.
        # WE DON'T HAVE TO DO ANYTHING MORE THAN THIS ON THE CLIENT/CALLING SIDE
...
#        OR IMPLICTLY (WITH AUTH0 LOGIN AFTER MONDAY WEBINARS)
            ...
            'tag': tag,
            'framework_type': framework_type,
# TODO:  IS THIS THE server_path WHERE THE MODEL WILL LIVE ON THE PipelineAI SERVER FILE SYSTEM
#        THIS IS RELATIVE TO /root/pipelineai/models/... ON THE PipelineAI SERVER FILE SYSTEM
            'path': server_path,
            'overwrite': overwrite,
# TODO:  Why is this being passed from the cli-local/ cli => api-private/ REST API?
#        This should only affect the timeout for the call from the (local) cli => (remote) REST API
        ...
        # print('response.json: %s' % yamlizable(response.json))
        status_code = response.status_code
        if status_code > HTTP_STATUS_SUCCESS_CREATED:
            return_dict['error_message'] = 'resource_archive_upload %s' % status_code
            # response.raise_for_status()
            return return_dict, status_code
        else:
            print('SUCCESS')
            return_dict['resource_archive_upload'] = response.json()
            print('response.text: %s' % response.text)
            print('response.json: %s' % yamlizable(response.json))

        # *********** resource_source_add ********************************
        print('Initializing training resource')
...
# TODO:  IS THIS THE server_path WHERE THE MODEL WILL LIVE ON THE PipelineAI SERVER FILE SYSTEM
#        THIS IS RELATIVE TO /root/pipelineai/models/... ON THE PipelineAI SERVER FILE SYSTEM
# TODO:  AT SOME POINT, WE SHOULDN'T NEED TO PASS THIS, BUT WE ARE FOR NOW
            'path': server_path,
            'timeout': timeout_seconds,
        }
        response = _requests.post(
        ...
        response.raise_for_status()
        return_dict['resource_source_add'] = response.json()

        # *********** resource_server_build ********************************
        print('Build resource docker images')
...
#        THIS IS RELATIVE TO /root/pipelineai/models/... ON THE PipelineAI SERVER FILE SYSTEM 
        ...
        # *********** resource_kube_start ********************************
        print('Deploying and starting resource service')
        endpoint = '/resource-kube-start'
        url = api_url + endpoint
        body = {
            'resource_type': resource_type,
            'name': name,
        ...
        response.raise_for_status()
        resource_kube_start_dict = response.json()
        return_dict['resource_kube_start'] = resource_kube_start_dict
        ...
        Use PipelineAI to route live traffic to the service.
        
            %s
        
        ''' % resource_kube_start_dict.get('endpoint_url', ''))
