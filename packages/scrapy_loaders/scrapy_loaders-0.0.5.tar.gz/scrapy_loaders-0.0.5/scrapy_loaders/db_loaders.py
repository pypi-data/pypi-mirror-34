...
class DBLoader(object):
    ...
    def __init__(self, item, spider):
        ...
        if self.hash_fields:
            self.item[self.hash_field_name] = self.hash
    ...
    def has_changed(self):
        if self.hash_fields:
            return self.item[self.hash_field_name] != getattr(self.item_model, self.hash_field_name)

    def get_db_item(self):
        try:
            return self.session.query(self.model).filter(getattr(
                self.model, self.primary_key_field) == self.item[self.primary_key_field]).first()
        except AttributeError:
            raise ImproperlyConfigured('{} Has no attribute id. You can specify your primary key field name at '
                                       '{} overriding its attribute primary_key_field'.format(
                                            self.model().__class__.__name__, self.__class__.__name__))
        except KeyError:
            raise ImproperlyConfigured('{} Has no attribute id. You can specify your primary key field name at'
                                       '{} overriding its attribute primary_key_field'.format(
                                            self.item.__class__.__name__, self.__class__.__name__))
    ...
    def hash(self):
        ...
        for field in self.hash_fields:
            seed += '{}'.format(self.item.get(field))
    ...
    def __str__(self):
        return "{action} {item_id}: {success}".format(
            action=self.action, item_id=self.item[self.primary_key_field], success=self.success)
