...
class CNN:
    ...
    def train(self,
        ...
        if self.is_image_data == True:
            ...
            self.class_dictionary = train_datagen.class_indices
    ...
    def train_with_directory_images(self, train_datagen, steps_per_epoch, epochs, verbose, callbacks, shuffle):
        ...
        self.class_dictionary = train_generator.class_indices
        if type(self.model) == type(None):
    ...
    def save(self, model_path, class_dictionary_path = None):
        """This function saves a model as h5 format and a class_dictionary as pickel format.
        
        Arguments:
            model_path {str} -- Path to save model.
        
        Keyword Arguments:
            class_dictionary_path {str or None} --
                If a model is trained with image from directory and class_dictionary_path is specified,
                this function will check whether class_dictionary exists and save the dictionary if it exists.
                (default: {None})
        
        Raises:
            e -- Raises error if model is None or path is wrong.
        """
        if type(self.model) == type(None):
