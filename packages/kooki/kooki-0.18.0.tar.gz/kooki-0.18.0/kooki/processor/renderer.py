...
class KookiInlineGrammar(InlineGrammar):
    ...
    criticmarkup_substitution = re.compile(r'^{~~(.*)~>(.*)~~}')
...
class KookiRenderer(Renderer):
    ...
    def criticmarkup_addition(self, text):
        return self.extensions.__criticmarkup_addition__(renderer=self, text=text)

    @catch_errors
    def criticmarkup_deletion(self, text):
        return self.extensions.__criticmarkup_deletion__(renderer=self, text=text)

    @catch_errors
    def criticmarkup_substitution(self, text, substitution):
        return self.extensions.__criticmarkup_substitution__(renderer=self, text=text, substitution=substitution)

    @catch_errors
    def criticmarkup_comment(self, text):
        return self.extensions.__criticmarkup_comment__(renderer=self, text=text)

    @catch_errors
    def criticmarkup_highlight(self, text, comment):
        return self.extensions.__criticmarkup_highlight__(renderer=self, text=text, comment=comment)
