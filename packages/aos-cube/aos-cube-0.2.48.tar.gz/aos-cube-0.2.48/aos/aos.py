...
def scons_upload(args):
    make_args = ['scons -f ucube.py COMMAND=upload']
    target_find = False
    ...
    for arg in args:
        if '@' in arg and not target_find:
            ...
            target_find = True
        #else:
        #    ret = make_args.append(arg)

    ret = popen(' '.join(make_args), shell=True, cwd=os.getcwd())
#    ret = subprocess.call(make_args, stdout=sys.stdout, stderr=sys.stderr)
    return ret
