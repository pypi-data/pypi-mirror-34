# -*- coding: utf-8 -*- äöü
# Kopie von quintagroup.transmogrifier.sitewalker;
# erweitert um Debugging-Optionen

# ---------------------- [ quintagroup.transmogrifier.sitewalker ... [
...
logger = logging.getLogger('visaplan.transmogrifier.sitewalker')

class SiteWalkerSection(BaseSection):
    ...
    def walk(self, obj):
        if IFolderish.providedBy(obj) or IBaseFolder.providedBy(obj):
            ...
            yield obj, tuple([(k, v.getPortalTypeName())
    ...
    def __iter__(self):
        for item in self.previous:
            yield item
        tracefunc = make_countdown_function(self.trace1st)
        ...
        if self.start_path:
            ...
            for cur_start_path in self.start_path:
                ...
                for item in self.walker(start_obj, tracefunc):
                    yield item
        else:
