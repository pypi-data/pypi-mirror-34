...
class LDM:
    ...
    def compare2dir(imagedir1,imagedir2,max_compare_num=30,score_threshold=0.8):
        ...
        sameper_num=0
        score_ave=0
        for imf1 in os.listdir(imagedir1):
            #print imf1
            ...
            for imf2 in os.listdir(imagedir2):
                ...
                rd=ldmer.face_rec(img1,img2,0.0)
                ...
                if len(rd['scorel'])>1:
                    break
                compare_num+=1
                for score in rc['scorel']:
                       if score>score_threshold:
                           sameper_num+=1
                           break
                       score_ave+=score
                #print "%d,%d,%d/%d"%(cc1,cc2,sameper_num,compare_num)
                if compare_num>max_compare_num:
                    break
        ...
        return float(sameper_num)/(float(compare_num)+0.0000001),score_ave,compare_num,imagedir1.split('/')[-1],imagedir2.split('/')[-1],"http://clt.management.vipkid.com.cn/operation/classroom/classroom/" 
