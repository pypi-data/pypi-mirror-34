...
def augment (y, sr, time_stretch = False, pitch_shift = False, random_laps = False, add_noise = False):    
    if add_noise:
        ...
        if y.shape [0] % 2 == 1:
            y = y [:-1] # make even
        noise = gen_noise(color, y.shape [0]) / 4
        gain = np.random.uniform(0.1, 0.4)
        y = y * np.random.uniform(0.6, 0.9) + ((gain * noise).astype ("float32"))
    ...
    if pitch_shift:
        n_step = random.choice (range (-3, 4))
        y = librosa.effects.pitch_shift (y, sr, n_step + random.random ())
    ...
    if random_laps and librosa.get_duration(y, sr = sr) > 1.6:
       ...
       y = np.roll (y, random.randrange (len (y) // 10))
    
    return y   
    
def load (wavfile, sample_rate = SAMPLE_RATE, time_lap = 0.2, due_limit = (1, 5), *args, **karg):
