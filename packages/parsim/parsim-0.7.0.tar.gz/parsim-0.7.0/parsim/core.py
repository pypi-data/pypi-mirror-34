...
def create_caselist_file(output_path, case_names, column_dict=None, case_dicts=None, parameter_names=None, csv=None):
    ...
    if column_dict:
        ...
        for row_dict in caselist_rows:
            missing = column_key_set - row_dict.viewkeys()
            if missing:
...
class Study(ParsimObject):
    ...
    def collect(self, **kwargs):
        ...
        for case in self:
            ...
            try:
                ...
                if not output_keys_set: # First case is reference...
                    ...
                    missing = output_keys_set - case_data.viewkeys()
                    if missing:
                        case.logger.error('Missing output fields: %s' % missing)
                    extra = case_data.viewkeys() - output_keys_set
                    if extra:
        ...
        column_dict = dict(response_list=output.keys(), param_list=param_list)
...
class Case(ParsimObject):
    ...
    def create(self, template_path, user_parameters=None, caselist_parameters=None, default_parameters=None,
        ...
        if not default_parameters:
            ...
            default_parameters = self.project.parse_parameter_file(default_parameter_path)
        # Trace parameter sources, no overlap! (Use try clause, for compatibility between Python 2 and 3)
        try:
            default_view = default_parameters.viewkeys()
            user_view = user_parameters.viewkeys()
            caselist_view = caselist_parameters.viewkeys()
        except AttributeError:
        ...
        param_keys = {}
        param_keys['caselist'] = list(caselist_view)
        param_keys['user'] = list(src_keys_user)
        param_keys['default'] = list(src_keys_default)
