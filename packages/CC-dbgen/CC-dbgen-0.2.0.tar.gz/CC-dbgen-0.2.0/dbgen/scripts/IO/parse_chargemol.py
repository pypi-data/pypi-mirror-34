...
def parse_chargemol(stordir : str
                   ,atoms   : Atoms
                   ) -> Tuple[List[int]
                                 ,List[int]
                                 ,List[int]
    ...
    #----------------
    try:
        ase_sort = get_file(join(stordir,'ase-sort.dat'))
        loglines = map(lambda x: map(int,x.split()),ase_sort.split('\n'))
    ...
    bondlines = get_file(filepath).split('\n')
    ...
    chg_lines = get_file(fpath).split('\n')
...
if __name__=='__main__':
    ...
    import sys
    pth = sys.argv[1]
    print(parse_chargemol(pth,anytraj(pth)))
