...
class CNN:
    ...
    def prepare_y(self, label_array):
        """This function prepares y for a keras model.(ex. y_train)
    ...
    def train(self,
        ...
        if self.is_image_data == True:
            ...
            if self.get_image_from == 'directory':
                self.train_with_directory_images(train_datagen, steps_per_epoch, epochs, verbose, callbacks, shuffle)
            elif self.get_image_from == 'argument':
                self.train_with_argument_images(train_datagen, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator)
        else:
            ...
            self.model.fit(self.x_train, y_train, steps_per_epoch=steps_per_epoch, epochs=epochs, verbose=verbose, callbacks=callbacks, shuffle=shuffle)
    
    def train_with_directory_images(self, train_datagen, steps_per_epoch, epochs, verbose, callbacks, shuffle):
    ...
    def train_with_argument_images(self, train_datagen, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator):
        num_classes = self.get_num_of_classes(self.label_array)
        y_train = self.prepare_y(self.label_array)
        if type(self.model) == type(None):
            self.model = self.create_model( num_classes )
    ...
    def save(self, model_path, class_dictionary_path):
        """This function saves a model as h5 format and a class_dictionary as pickle format.
    ...
    def load(self, model_path, class_dictionary_path = None):
        """This function load a model and a class_dictionary(optional) from a file system.
