...
class AXverify:
    ...
    def verify_post (self, account1, account2):
        ...
        '''
        permlink = self.steem.util.permlink(
                self.steem.recent_post(account1))
        if not permlink[1]:
            self.msg.error_message("No post for " + account1)
            return False
        else:
            print("Checking "+permlink[1])
    ...
    def eligible_votes(self, account1, account2, 
        ...
        self.vote_cut = v3 * 100
        v4 = self.get_vote_value(account2, self.vote_cut, vpow)
