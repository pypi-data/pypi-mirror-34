...
def test_ccz_repr():
    assert repr(Rot111Gate(half_turns=1)) == u'CCZ'
    assert repr(Rot111Gate(half_turns=0.5)) == u'CCZ**0.5'
...
@pytest.mark.parametrize(u'half_turns', [1.0, 0.5, 0.25, 0.1, 0.0, -0.5])
...
def test_cxxyy_repr():
    assert repr(ControlledXXYYGate(half_turns=1)) == u'CXXYY'
    assert repr(ControlledXXYYGate(half_turns=0.5)) == u'CXXYY**0.5'
...
def test_cyxxy_repr():
    assert repr(ControlledYXXYGate(half_turns=1)) == u'CYXXY'
    assert repr(ControlledYXXYGate(half_turns=0.5)) == u'CYXXY**0.5'


@pytest.mark.parametrize(
        u'gate, half_turns, initial_state, correct_state, atol', [
            (CCZ, 0.5,
