...
from ..type import (
    GraphQLArgument, GraphQLDirective, GraphQLEnumType, GraphQLEnumValue,
    GraphQLField, GraphQLInputField, GraphQLInputObjectType, GraphQLInputType,
    GraphQLInterfaceType, GraphQLList, GraphQLNamedType, GraphQLNonNull,
    GraphQLObjectType, GraphQLOutputType, GraphQLScalarType, GraphQLSchema,
    GraphQLType, GraphQLUnionType, TypeKind, assert_interface_type,
    assert_nullable_type, assert_object_type, introspection_types,
    is_input_type, is_output_type, specified_scalar_types)
...
def build_client_schema(
        introspection: Dict, assume_valid: bool=False) -> GraphQLSchema:
    """Build a GraphQLSchema for use by client tools.

    Given the result of a client running the introspection query, creates and
    returns a GraphQLSchema instance which can be then used with all GraphQL-core-next
    ...
    def build_interface_def(
            interface_introspection: Dict) -> GraphQLInterfaceType:
    ...
    def build_input_object_def(
            input_object_introspection: Dict) -> GraphQLInputObjectType:
    ...
    def build_field_def_map(
            type_introspection: Dict) -> Dict[str, GraphQLField]:
        if type_introspection.get('fields') is None:
            raise TypeError(
                'Introspection result missing fields:'
                f' {type_introspection!r}')
        return {field_introspection['name']: build_field(field_introspection)
                for field_introspection in type_introspection['fields']}

    def build_arg_value(
            arg_introspection: Dict) -> GraphQLArgument:
        type_ = get_input_type(arg_introspection['type'])
        default_value = arg_introspection.get('defaultValue')
        default_value = INVALID if default_value is None else value_from_ast(
            parse_value(default_value), type_)
        return GraphQLArgument(
            type_, default_value=default_value,
            description=arg_introspection.get('description'))

    def build_arg_value_def_map(
            arg_introspections: Dict) -> Dict[str, GraphQLArgument]:
        return {input_value_introspection['name']:
                build_arg_value(input_value_introspection)
                for input_value_introspection in arg_introspections}

    def build_input_value(
            input_value_introspection: Dict) -> GraphQLInputField:
    ...
    def build_input_value_def_map(
            input_value_introspections: Dict) -> Dict[str, GraphQLInputField]:
        return {input_value_introspection['name']:
    ...
    def build_directive(directive_introspection: Dict) -> GraphQLDirective:
        ...
        return GraphQLDirective(
            ...
            locations=list(cast(Sequence[DirectiveLocation],
    ...
    mutation_type = get_object_type(
        mutation_type_ref) if mutation_type_ref else None
    subscription_type_ref = schema_introspection.get('subscriptionType')
    subscription_type = get_object_type(
        subscription_type_ref) if subscription_type_ref else None

    # Get the directives supported by Introspection, assuming empty-set if
    # directives were not queried for.
    directive_introspections = schema_introspection.get('directives')
    directives = [build_directive(directive_introspection)
                  for directive_introspection in directive_introspections
                  ] if directive_introspections else []
