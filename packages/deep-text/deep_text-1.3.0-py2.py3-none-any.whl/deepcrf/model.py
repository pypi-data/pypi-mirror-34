...
from .default_custom import DeepCRFTransform


class DeepCRFModel(BaseModel):
    def __init__(self, session, transformer, config):
        super(DeepCRFModel, self).__init__(session, transformer, config)
        self.input = None
    ...
    def predict(self, multi_parts):
        ...
        feed_dict = {self.input: part_ids,
                     ...
                     self.weights: np.ones([part_ids.shape[0], part_ids.shape[1], self.transformer.num_tags])}
...
class GraphDeepCRFModel(DeepCRFModel):
    def __init__(self, model_path, config, TransformClass):
        if TransformClass is None:
            super(GraphDeepCRFModel, self).__init__(tf.Session(graph=utils.load_graph_file(model_path)), DeepCRFTransform(config), config)
        else:
            super(GraphDeepCRFModel, self).__init__(tf.Session(graph=utils.load_graph_file(model_path)), TransformClass(config), config)
        ...
        self.weights = self.session.graph.get_tensor_by_name("weights:0")
        if utils.tf_version_uper_than("1.3.0"):
...
class Bi_LSTM_CRF(DeepCRFModel):
    def __init__(self, session, transformer, config):
        super(Bi_LSTM_CRF, self).__init__(session, transformer, config)
        ...
        # weights shape: [batch_size, max_seq_len, num_tags]
        self.weights = tf.placeholder(tf.float32, shape=[None, None, None], name="weights")
        ...
        with tf.variable_scope("logits"):
            W = tf.get_variable("W", shape=[2 * config.hidden_size, transformer.num_tags],
                                initializer=tf.contrib.layers.xavier_initializer(),
                                dtype=tf.float32)

            b = tf.get_variable("b", shape=[transformer.num_tags],
            ...
            self.logits = tf.reshape(prob, [-1, s[1], transformer.num_tags], name="logits")
        ...
        if utils.tf_version_uper_than("1.3.0"):
            with tf.variable_scope("viterbi"):
                viterbi_sequence, viterbi_score = \
                    tf.contrib.crf.crf_decode(self.logits * self.weights, transition_params, self.sequence_lengths)
