...
dim_dis_add_cmnd(PyObject* /* self */, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "s#s#O|l",
...
dim_dis_update_service(PyObject* /* self */, PyObject* args) {
  ...
  if (!svc_args) {
    ...
    /*
    PyArg_ParseTuple with an O object does NOT increase the reference count of the variable
    bound to it
    */
    Py_INCREF(svc_args);
  }
...
dim_callbackCommandFunc(void* uTag, void* address, int* size) {
  ...
  if (args) {
	...
	if (pycall->pyTag) {
		...
		PyTuple_SET_ITEM(funargs, 1, PyLong_FromLong(pycall->pyTag));
	} else {
...
dim_dic_info_service(PyObject* /* self */, PyObject* args) {
  ...
  /*Parse arguments if the format of data is passed
  to the dic_info_service python function
  */
  if (!PyArg_ParseTuple(args, "s#s#O|iiiO",
        ...
        &default_value)
      || !PyCallable_Check(pyFunc)){
    /*As PyArg_ParseTuple(...) raises an exception when it fails,
    we need to flush the exception system in python by PyErr_Clear().
    If not, the time.sleep(x) called in the Python client program will
    raise the exception and make it crash
    */
    PyErr_Clear();
    /*Parse arguments if the format of data is NOT passed to the dic_info_service
    python function
    */
    if(!PyArg_ParseTuple(args, "s#O|iiiO",
          &name, &name_size,
          &pyFunc,
          &service_type,
          &timeout,
  ...
  if(format != NULL){
    /* The format has been passed in parameters*/
    svc->format = (char*)malloc(sizeof(char)*format_size+1);
    if(!svc->format){
      goto no_memory;
    }
...
PyObject *deactivate_command_format_cache(PyObject *,PyObject *args){
  deactivate_cmnd_format_cache();
  Py_INCREF(Py_None);
  return Py_None;
}

PyObject *activate_command_format_cache(PyObject *,PyObject *args){
  activate_cmnd_format_cache();
  Py_INCREF(Py_None);
  return Py_None;
}
...
dim_dic_cmnd_service(PyObject* /* self */, PyObject* args) {
   ...
   */
  char *service_name, *format = NULL, *buffer;
  unsigned int buffer_size;
  int res;
  PyObject *pySeq;

  res = PyArg_ParseTuple(args, "sO|s", &service_name, &pySeq, &format);
  if (!res){
    goto invalid_arguments;
  }

  if(format == NULL){
    /*Format not provided by the user, retrieve it from the cache or from the DNS*/
    get_cmnd_format(service_name,&format);
    if(format == NULL)
        /*Command not found by the DNS, return 0 because command has not been requested*/
  ...
  invalid_arguments:
    PyErr_SetString(PyExc_TypeError,
        ...
        "update_data (tuple or list), format (DIM format string) (optional)");
    return NULL;
...
dim_dic_cmnd_callback(PyObject* /* self */, PyObject* args) {
      ...
      || !PyCallable_Check(pyFunc))
  {
    ...
    */
    PyErr_Clear();
    if(!PyArg_ParseTuple(args,"sOOl",
      &service_name,&pySeq,&pyFunc,&pyTag)
      || !PyCallable_Check(pyFunc))
    {
      goto invalid_arguments;
    }
 ...
 invalid_arguments:
  PyErr_SetString(PyExc_TypeError,
      ...
      "             format (a DIM format string) (optional), \n"\
...
void _dic_info_service_dummy (void* tag, void* buffer, int* size) {
  ...
  if(svc->format == NULL){
    svc->format = dic_get_format(0);
  }

  if (!(*size) || svc->format == NULL) {
  ...
  /*Testing if the returned args are a tuple to avoid a TypeError with dic_sync_info_service*/
  if (args) {
//    Py_INCREF(args);  pydim.dis_update_service(svc3)
    if(args == Py_None){
      /*Can't call PyEval_CallObject with args = PyNone, it must be NULL*/
      args = NULL;
    }
...
static PyMethodDef DimMethods[] = {
  ...
  {
    "deactivate_command_format_cache",
    deactivate_command_format_cache,
    METH_VARARGS,
    "Deactivate the command format cache"
  },
  {
    "activate_command_format_cache",
    activate_command_format_cache,
    METH_VARARGS,
    "Deactivate the command format cache"
  },
...
static pthread_mutex_t pydimlock;
#endif
...
{
  PyObject *m;
  pthread_mutex_init(&pydimlock,NULL);
  PyEval_InitThreads();
