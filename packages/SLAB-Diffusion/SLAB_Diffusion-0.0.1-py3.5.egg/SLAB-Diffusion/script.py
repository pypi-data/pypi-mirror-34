...
#__all__=[''] #redefinition of '*' for import *
...
if already_run==False:
    ...
    plotter_3D.plot_u_latest_time(u,space_z,time,time_Units='tD',output_file=params.path_outputs,show_plots=True)
    ...
    plotter_3D.plot_cuts_2D_temp(temp,spacez,time,time_Units='tD',output_file=params.path_outputs,dilution_factor=dilution_factor,show_plots=True)
    plotter_3D.plot_temp_latest_time(temp,spacez,time,time_Units=None,output_file=params.path_outputs,show_plots=False)
...
time_observed=[]
dilution_factor=params.dillution_factor
    ...
    for t, s in enumerate(time_diluted):
        ...
        for i,l in enumerate(spacex):
            for j,k in enumerate(spacey):
                ...
                bb_flux_spectrum=black_body_flux_density.black_body_flux_density(temp[int(index_dilute[t]),i,j],np.linspace(1e-10,1e-6,num=100),type='P',verbose=False,distance_pc=1,Radius=None,Ebv=None)[2]#les luminosites seront celle de bb 1 solar radius a 1 pc!
        ...
        #pylab.savefig(params.path_outputs+'/cells_bb_spectra/cell_bb_' + str(t).zfill(5) + '.png', bbox_inches='tight')
        np.savetxt(params.path_outputs+'/Sum_BB_time_{0}.txt'.format(s), Sum_BB_ind[:])
...
np.savetxt(params.path_outputs+'/time_observed.txt',time_observed_x)

distance_modulus=37.77
distance_pc=distances_conversions.DM_to_pc(distance_modulus)

time_observed_fromfile=np.genfromtxt(params.path_outputs+'/time_observed.txt')
#Sum_BB_fromfile=np.genfromtxt(params.path_outputs+'/Sum_BB.txt',skip_header=1)
for t, s in enumerate(time_diluted): #10s per step 1days~6 steps (60 s per day), 50 days en 50 min
    Sum_BB_fromfile[t,:]=np.genfromtxt(params.path_outputs+'/Sum_BB_time_{0}.txt'.format(s))
...
pylab.savefig(params.path_outputs+'/cells_bb_spectra/sum_cells_bb.png', bbox_inches='tight')
...
if already_fit==False:
    ...
    np.savetxt(params.path_outputs+'/Sum_BB_properties.txt',BB_evo,header='time,best-fit temperature,best-fit radius,best-fit luminosity')


BB_evo_fromfile=np.genfromtxt(params.path_outputs+'/Sum_BB_properties.txt',skip_header=1)
...
pylab.savefig(params.path_outputs+'/edge_spectrum_BB_T.png', bbox_inches='tight')
...
pylab.grid()
pylab.savefig(params.path_outputs+'/edge_spectrum_BB_R.png', bbox_inches='tight')

temperatures=np.genfromtxt(params.path_outputs+'/Sum_BB_properties.txt',skip_header=1)[:,1]
Luminosities=np.genfromtxt(params.path_outputs+'/Luminosities_Dcst_2.txt')
...
pylab.savefig(params.path_outputs+'/edge_spectrum_BB_R_2.png',bbox_inches='tight')
pylab.show()
...
#plt.savefig(params.path_outputs/edge_spectrum_BB_R_2.png',bbox_inches='tight')
plt.yticks(fontsize=16)
plt.xticks(fontsize=16)
plt.xlim(0.01,1)
#plt.ylim(0,16e15)
...
plt.savefig(params.path_outputs+'/edge_spectrum_BB_R_T_sameaxis.png',bbox_inches='tight')
