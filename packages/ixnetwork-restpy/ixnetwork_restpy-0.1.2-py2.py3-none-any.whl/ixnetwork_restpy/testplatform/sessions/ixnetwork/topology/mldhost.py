...
class MldHost(Base):
	...
	def MldMcastIPv6GroupList(self):
		"""Returns the one and only one MldMcastIPv6GroupList object from the server.
		...
		return self._read(MldMcastIPv6GroupList(self), None)
	...
	def remove(self):
		"""Deletes a child instance of MldHost on the server.
	...
	def MldStartHost(self, Arg1):
		"""Executes the mldStartHost operation on the server.

		Start MLD Host

		Args:
			Arg1 (list(str[None|/api/v1/sessions/1/ixnetwork/topology])): This parameter requires a list of /topology/./mldHost object references

		Raises:
			NotFoundError: The requested resource does not exist on the server
			ServerError: The server has encountered an uncategorized error condition
		"""
		return self._execute('mldStartHost', payload=locals(), response_object=None)

	def MldStartHost(self, Arg1, SessionIndices):
		"""Executes the mldStartHost operation on the server.
	...
	def MldStopHost(self, Arg1):
		...
		Raises:
			...
			ServerError: The server has encountered an uncategorized error condition
		"""
		return self._execute('mldStopHost', payload=locals(), response_object=None)

	def MldStopHost(self, Arg1, SessionIndices):
		"""Executes the mldStopHost operation on the server.
