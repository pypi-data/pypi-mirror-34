...
class DNN:
    ...
    def log (self, name, val, family = "net"):
        if isinstance (val, Tensor):
            tf.summary.scalar (name, val, family = self.name and self.name + "-" + family or family)
        else:    
    ...
    def trainable (
            self, initial_learning_rate=0.00001, decay_step=3000, decay_rate=0.99, 
            overfit_threshold = 40, accuracy_threshold = 0.4, 
            monitor = "cost"
        ):
        ...
        with self.graph.as_default ():
            ...
            self.log ("cost-total", self.cost)
            self.log ("cost-average", self.add_average (self.cost))
    ...
    def fit (self, x, y, ops = None, **kargs):
        ...
        if self.summary_op is not None:
            self.writers ["net"].add_summary (r [-(trailers + 3)], self.eval (self.global_step))        
        
        if trailers:
            logit, cost = r [:-trailers][-2:]
        else:
            logit, cost = r [-2:]
    ...
    def optimizer (self, name = 'adam', cost = None, learning_rate = None, **karg):
        ...
        return getattr (optimizers, name) (cost, learning_rate, self.global_step, **karg)
