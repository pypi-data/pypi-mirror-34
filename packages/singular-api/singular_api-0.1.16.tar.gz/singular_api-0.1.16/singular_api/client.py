        ...
        try:
            ...
            if response.status_code in [200, 201, 202, 204]:
                ...
                if self.token_managed is True:
                    # TODO(kuba): fix
                    if response.text == '' and response.status_code == 503:
                        db_token = getattr(self._model, self._token_str)
                        if db_token != self._singular_token:
                            # allready refreshed
                            self.singular_token = db_token
                            request = try_request()
                            if response.status_code in [200, 201, 202]:
                                pass
                        db_refresh_token = getattr(self._model,
