...
class ScheduleImage:
    ...
    def __init__(self, schedule_items, config, logger):
        ...
        if 60 % self._group_by != 0:
            self._logger.error("ScheduleImage - 60 must be a multiple of group-by. (60 % '{}' != 0)".
                               format(self._group_by))
            raise ValueError("ScheduleImage - 60 must be a multiple of group-by. (60 % '{}' != 0)".
                             format(self._group_by))
        groups_per_hour = int(60 / self._group_by)
        pixel_per_hour = int(self._width / 24)
        if pixel_per_hour % groups_per_hour != 0:
            self._logger.error("ScheduleImage - width ({}) and group-by ({}) not fulfilling constraint "
                               "'(width / 24) % (60 / group-by) == 0'".format(self._width, self._group_by))
            raise ValueError("ScheduleImage - width ({}) and group-by ({}) not fulfilling constraint "
                             "'(width / 24) % (60 / group-by) == 0'".format(self._width, self._group_by))
