...
class Pos(Base):
	...
	@property
	def Dcc(self):
		"""Returns the one and only one Dcc object from the server.
		...
		return self._read(Dcc(self), None)

	@property
	def Ppp(self):
		"""Returns the one and only one Ppp object from the server.
		...
		return self._read(Ppp(self), None)
