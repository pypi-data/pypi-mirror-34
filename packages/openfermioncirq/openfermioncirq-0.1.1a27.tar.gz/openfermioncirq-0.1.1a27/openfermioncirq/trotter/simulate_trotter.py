...
u"""Perform Hamiltonian simulation via a Trotter-Suzuki product formula."""
...
def simulate_trotter(qubits,
    ...
    u"""Simulate Hamiltonian evolution using a Trotter-Suzuki product formula.
    ...
    if order < 0:
        raise ValueError(u'The order of the Trotter formula must be at least 0.')
    ...
    if not isinstance(hamiltonian, tuple(algorithm.supported_types)):
        raise TypeError(
                u'The input Hamiltonian was a {} but the chosen Trotter step '
                u'algorithm only supports Hamiltonians of type {}'.format(
                    type(hamiltonian).__name__,
                    set(cls.__name__ for cls in algorithm.supported_types)))
...
def _perform_trotter_step(qubits,
                          time,
                          order,
                          trotter_step,
                          control_qubit
                          ):
    u"""Perform a Trotter step."""
    if order <= 1:
...
def _select_trotter_algorithm(hamiltonian):
    if isinstance(hamiltonian, DiagonalCoulombHamiltonian):
        ...
        raise TypeError(u'Failed to select a default Trotter algorithm '
                        u'for Hamiltonian of type {}.'.format(
                            type(hamiltonian).__name__))
...
def _select_trotter_step(hamiltonian,
                         ...
                         algorithm,
                         controlled):
    u"""Select a particular Trotter step from a Trotter step algorithm."""
    if controlled:
        if order == 0:
            trotter_step = algorithm.controlled_asymmetric(hamiltonian)
            if trotter_step is None:
                raise ValueError(u'The chosen Trotter step algorithm does not '
                                 u'support the order 0 (asymmetric) formula '
                                 u'with a control qubit.')
        else:
            trotter_step = algorithm.controlled_symmetric(hamiltonian)
            if trotter_step is None:
                raise ValueError(u'The chosen Trotter step algorithm does not '
                                 u'support higher (> 0) order formulas '
                                 u'with a control qubit.')
    else:
        if order == 0:
