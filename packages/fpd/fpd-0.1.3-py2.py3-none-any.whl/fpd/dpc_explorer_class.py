...
class DPC_Explorer:
    def __init__(self, d, r_min=0, r_max=None, r_min_pct=0, r_max_pct=95,
                 ...
                 ransac=False, ransac_dict=None, nbins=None, hist_lims=None, 
                 flip_y=False, flip_x=False, origin='top'):
        '''        
    ...
    def _init_cmap(self, cmap_in):
        ...
        try:
            ...
            self._cmaps['Uniform'] = mycmap
        except IOError as e:
            print("WARNING: Perceptually uniform cmap not found.")
            pass
    ...
    def _calc_data(self, calc_ransac=False):
        # median
        if self._median:
            medx = np.median(self._x_orig, 1)
            medx -= np.median(medx)
            medy = np.median(self._y_orig, 1)
            medy -= np.median(medy)
            
            self.x = self._x_orig - medx[:, None]
            self.y = self._y_orig - medy[:, None]
        else:
    ...
    def _plot_hist(self):
        ...
        self._hist_title = plt.title(self._hist_tit_str(), fontsize=12) 
        plt.xlabel('x')
        plt.ylabel('y')
        ...
        rax = plt.axes([0.05, 0.9-h, 0.15, h])
    ...
    def get_image_sequence(self, im_types='tr_im', images=False):
        '''
        Get rendered images of sequence.
        ...
        if images:
            ...
            for i, imt in enumerate(im_types):
                if imt in raw:
                    ...
                    print('Images of Unknown im_type %s wil be autoscaled' %(imt))
                    mins[i], maxs[i] = np.percentile(ims[i], [0, 100], axis=(-3, -2, -1))
    ...
    def _on_save(self, event, post_tag=None, working_dir=None):       
        ...
        data = np.column_stack((dnames+['pct', 'median', 'ransac', 'ransac_dict', 'flip_y', 'flip_x', 'origin'],
    ...
    def _on_print(self, event):
        ...
        cmd_str = "r_min=%0.4f, r_max=%0.4f, descan=[%0.4f, %0.4f, %0.4f, %0.4f], cyx=[%0.4f, %0.4f], vectrot=%0.2f, gaus=%0.2f, pct=[[%0.3f, %0.3f], [%0.3f, %0.3f]], dt=%0.2f, median=%s, ransac=%s, ransac_dict=%s, flip_y=%s, flip_x=%s, origin=%s" %(self._r_min, self._r_max, self._descanY_factY*1000, self._descanY_factX*1000, self._descanX_factY*1000, self._descanX_factX*1000, self._yc, self._xc, self._vectrot, self._gaus, self._pct[0][0], self._pct[0][1], self._pct[1][0], self._pct[1][1], self._dt, str(self._median), str(self._ransac), str(self._ransac_dict), str(self._flip_y), str(self._flip_x), self._origin)
