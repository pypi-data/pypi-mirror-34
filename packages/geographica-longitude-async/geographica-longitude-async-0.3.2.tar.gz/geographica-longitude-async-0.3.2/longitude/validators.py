...
def validate_obj_attr_existence(model, field_name, oid, extra_filters={}):
    if not model.exists(oid=oid, sync=True, **extra_filters):
...
def validate_has_keys(keys, dict):
    ...
    if missing_keys:
        raise ValidationError('the dictionary is missing the following keys: {}', missing_keys)
...
def validate_elems(elem_type: Field):
    def validate_elems(values):
        ...
        for idx, value in values:
            try:
                elem_type._validate(value)
            except ValidationError as e:
                e.message = 'Error in element {}: {}'.format(
                    idx,
                    e.message
                )
