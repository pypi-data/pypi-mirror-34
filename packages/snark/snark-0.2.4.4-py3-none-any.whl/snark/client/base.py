...
from snark.cli.utils import get_cli_version

from snark.exceptions import (AuthenticationException, AuthorizationException,
                              BadGatewayException, BadRequestException,
                              GatewayTimeoutException, NotFoundException,
                              OverLimitException, ServerException,
                              LockedException, SnarkException)
...
class SnarkHttpClient(object):
    ...
    def request(self, method, relative_url, params={}, data={}, files={}, json={},
            timeout=config.DEFAULT_TIMEOUT, headers={}):
        request_url = urljoin(config.SNARK_REST_ENDPOINT, relative_url)
        headers['snark-cli-version'] = get_cli_version()
        if not 'Authorization' in headers or headers['Authorization'] != self.auth_header:
        ...
        try:
           ...
           logger.debug("Exception: {}".format(e, exc_info=True))
           sys.exit("Connection timeout. Please retry or check your internet connection")
    ...
    def check_response_status(self, response):
        ...
        if code < 200 or code >= 300:
            ...
            logger.debug('Error received: status code: {}, message: "{}"'.format(response.status_code,
                                                                                 message))
            if code == 400:
