...
def clone(url):
    if iou.isdir(JOURNAL_ROOT):
        log.warning(Fore.YELLOW + 'WARNING: This will reset the local journal !' + Fore.RESET)
        ask_confirmation()
        shutil.rmtree(JOURNAL_ROOT)

    os.chdir(iou.dirname(JOURNAL_ROOT))
    log.success('Cloning journal')
    git_call(['clone', url, '.nikki'])
