...
class LoggingConsumer(object):
    ...
    class AsyncFlushThread(threading.Thread):
        ...
        def run(self):
            while True:
                ...
                self._consumer.need_flush.wait()
                # 进行发送，如果成功则清除标志位
    ...
    """
    def __init__(self, log_directory,queue_size=5000,max_batch_size=1000,log_size=1024):
        self._queue_size = queue_size
        self._max_batch_size = max_batch_size #每次刷新最大的数量
        if self._queue_size <= self._max_batch_size:
            self._max_batch_size = int(self._queue_size*0.2)
        self.log_directory = log_directory #log文件保存的目录
        self.flush_max_time = 20
        ...
        self.handler = self.__initLogger(self.log_directory+"log."+self.suffix)
        ...
        self._flushing_thread = LoggingConsumer.AsyncFlushThread(self)
    ...
    def send(self, msg):
        ...
        try:
            ...
            while self._queue.qsize() > self._queue_size * 0.9:
    ...
    def fileSizeOut(self,filePath):
        ...
        if fsize >= self._fileSize:
            ...
            os.rename(filePath, filePath +"_" + str(self._split_log_suffix))
            self._split_log_suffix += 1
            return True
        return False
    def sync_flush(self, throw_exception=True):
        ...
        if self.suffix != date_suffix :
            self.handler = self.__initLogger(self.log_directory+"log."+date_suffix)
            self.suffix = date_suffix
            self._split_log_suffix = 1
        self.fileSizeOut(self.log_directory + "log." + date_suffix)
