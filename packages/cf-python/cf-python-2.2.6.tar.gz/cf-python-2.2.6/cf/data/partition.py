...
from .filearray import FileArray, _TempFileArray #, SharedMemoryArray, _shared_memory_array
...
False
        '''
        return isinstance(self._subarray, _TempFileArray)
    #--- End: def
...
one.
        ...
        size = long(reduce(mul, self.shape, 1))
...
>>> p.close()
        ...
        if config is None:
            return
#        print 'close:', hasattr(self, 'config'),
        if kwargs:
        ...
        if config['serial']:
                ...
                if originally_on_disk:
                    ...
                    if config.get('to_disk', False):
                        ...
                        if (config['unique_subarray'] and 
                            isinstance(original_subarray, _TempFileArray)):
            ...
            if _debug: print 'Partition.close: parallel'
            # --------------------------------------------------------
            # PARALLEL
            # --------------------------------------------------------
            pass
...
'''
        ...
        if dtype is not None and dtype != p_dtype:
            try:
                ...
                raise ValueError(
"Can't recast partition array from {} to {}".format(p_dtype.name, dtype.name))
            else:
                # We've just copied p_data, so in place changes are
                # not possible
                copy = False
...
>>> p.to_disk(reopen=False)
        ...
        '''
        try:
            tfa = _TempFileArray(self.array)
        except:
            return False

        self.subarray = tfa
        _temporary_files.add(tfa._partition_file)
