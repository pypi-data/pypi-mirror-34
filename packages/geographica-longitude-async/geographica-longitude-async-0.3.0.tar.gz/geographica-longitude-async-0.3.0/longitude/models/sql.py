...
class SQLCRUDModel(metaclass=SQLCRUDModelMetaclass):
    ...
    @allow_sync
    async def list(self, format=None, **kwargs):
        res = await self.list_unformatted(format=format, **kwargs)

        return await self.transform_objects(res, format=format)
    ...
    async def count(self, **filters):
        return (await self.get(columns=(('count', 'count(*)'),), **filters))['count']
    ...
    @allow_sync
    async def upsert(self, values, pk=('id',)):
        ...
        return (await self._fetch(sql, *params))[0]['id']
    ...
    def select_sql(self, params=None, columns=None, joins=None, group_by=None, order_by=None, **filters):
        ...
        columns = self.columns_sql(columns)
    ...
    def where_sql(self, params, **filters):
        ...
        # Check if filters are used, and add them
        snippet = []
