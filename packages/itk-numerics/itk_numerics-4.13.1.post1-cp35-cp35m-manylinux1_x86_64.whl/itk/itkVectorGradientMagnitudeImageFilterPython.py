...
class itkVectorGradientMagnitudeImageFilterIVF22F(itkImageToImageFilterBPython.itkImageToImageFilterIVF22IF2):
    ...
    largest two eigenvalues in a principle component analysis of the
    partial derivatives [1]. The gradient is then based on the direction
    of maximal change, and is a characterization of how "elongated" the
    point-spread of the analysis is found to be.  The second, more
    heuristic, calculation mode finds gradient magnitude as the square-
    root of the sum of the * individual weighted vector component
    derivative sums squared. That is, * $ \\mathbf{magnitude} =
    ...
    output image will be of type itk::Image<itk::Vector<TRealType, M>, N>.
