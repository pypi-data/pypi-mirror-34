...
class WriterPython(Writer):
    ...
    def write_class(self, cls, flags):
        if not cls.is_changed:
            return {flags: self.parser.hash.get_class_body(cls)}
        ...
        self.parser.hash.fix_class_body(cls, out)
        return {flags: out}
...
{2}
    ...
    def create_serialization_function(self, cls, serialize_type):
        if cls.is_test or cls.name.startswith('ITest'):
            return
        function = Function()
