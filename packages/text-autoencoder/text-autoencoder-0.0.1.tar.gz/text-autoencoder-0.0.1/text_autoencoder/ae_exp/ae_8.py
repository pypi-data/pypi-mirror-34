...
from autoencoders.encoders import DynamicLSTM
from autoencoders.decoders import LSTM_LatentVec_InitState_AttentiveHistory
...
def build_model(maxlen: int, embedding_table: np.array):
    ...
    with tf.variable_scope("encoder"):
        latent, output_state = DynamicLSTM(
            input_=input_X,
            seqlen=seqlen,
            input_dropout=0.1,
            state_size=256,
    ...
    with tf.variable_scope("decoder"):
        output_logits, output_word_indices = LSTM_LatentVec_InitState_AttentiveHistory(
            latent_input=latent,
            real_indices=input_X,
            embedding_table=embedding_table,
            history_size=256,
            attention_hidden_size=512,
            attention_hops=5,
            init_state=output_state,
            assist=assist_place,
        )
