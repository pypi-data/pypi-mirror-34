...
class XPS:
    ...
    def get_energy_survey(self,range=None):
        if range:
            start_index = self._find_index_of_nearest(array = self.survey_data.energy,value=range[0])
            end_index = self._find_index_of_nearest(array = self.survey_data.energy, value=range[1])
