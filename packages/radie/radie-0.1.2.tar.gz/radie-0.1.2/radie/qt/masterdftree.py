...
class DataStructureTree(IndexableDict):
    """The main DataStructure tree Construct

    This Class is the Root of the Tree of DataFrames presented in the GUI viewer.  Essentially, it is an Ordered
    Dict whose keys are the DataStructure classes, and whose values are the record of the available instances of those
    classes
    ...
    def __setitem__(self, key, value):
        if not issubclass(key, DataStructure):
            raise TypeError("key must be a subclass of DataStructure")
...
class DataStructureNode(IndexableDict):
    """A foldable node on the TreeView, one for each class of DataStructure

    A dict whose keys are a uuid string generated by the DataStructure instance, and the values are a reference to
    the DataStructure of the class specified by the cls attribute
    ...
    cls : typing.Type(DataStructure)
        The subclass of DataStructure that all members of this node belong to
    """
    def __init__(self, cls, tree):
        self.cls = cls  # type: DataStructure
...
class DFReference(QtCore.QObject):
    ...
    def __init__(self, df: DataStructure, node: DataStructureNode):
        if not isinstance(df, DataStructure):
            raise TypeError("df must be a DataStructure")
        self._df = df
        self.node = node
...
class DFTreeModel(QtCore.QAbstractItemModel):
    ...
    def deleteDataFrame(self, index: QtCore.QModelIndex):
        ...
        if not isinstance(df, DFReference):
            raise TypeError("index does not contain a DataStructure")
        row = index.row()
...
class DFTreeView(QtWidgets.QTreeView):
    ...
    def rightClicked(self, pos: QtCore.QPoint):
        ...
        if num_dataframes > 0:
            if num_dataframes == 1:
                deleteItem = QtWidgets.QAction("Delete DataStructure", menu)
                viewItem = QtWidgets.QAction("View DataStructure", menu)
                viewItem.triggered.connect(self.viewSelectedDataFrame)
                menu.addAction(viewItem)
                copyItem = QtWidgets.QAction("Copy DataStructure", menu)
