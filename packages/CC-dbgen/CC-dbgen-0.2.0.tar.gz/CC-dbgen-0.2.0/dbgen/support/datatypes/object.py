...
class Object(object):
    ...
    """
    _universe    = {}    # type: Dict[str,Object]
    _todo        = set() # type: Set[str]
    _parents    = []    # type: List[Object]
    _components = []    # type: List[Object]
    _name       = ''
    _desc       = ''
    _pks        = []    # type: List[Attr]
    _fks        = []    # type: List[FK]
    _attrs      = {}    # type: Dict[str,Attr]
    _many_to_one= False
    ...
    @classmethod
    def _process_parent(cls,par:'Object')->None:
        new_pks = [Attr(par._name+'_'+k.name,pk=True,obj=cls._name)
                        for k in par._pks]
        cls._pks.extend(new_pks)
        cls._fks.append(FK(new_pks,par,par._pks))
        for p in new_pks: cls._addAttr(p)
    @classmethod
    def _process_component(cls,comp:'Object')->None:
        new_fks = [Attr(comp._name+'_'+k.name,obj=cls._name) for k in comp._pks]
        for f in new_fks:
            cls._addAttr(f)

        cls._fks.append(FK(new_fks,comp,comp._pks))
    ...
    def _init_vars(cls)->None:
        ...
        cls._parents     = cd.get('_parents',[])
        cls._components  = cd.get('_components',[])
        cls._many_to_one = cd.get('_many_to_one',False)
        cls._desc        = cls.__dict__.get('__doc__','(No description)')

        cls._pks,cls._fks,cls._attrs = [],[],{}

        new_attrs  = [cls._mkAttr(k,v)
                      for k,v in cd.items()
                      if k[0]!='_'] + [Attr('deleted',obj=name,default=0)]

        has_indep_pk = (not cls._parents) or cls._many_to_one
        if has_indep_pk:
    ...
    def _addAttr(cls,attr:Attr)->None:
        if attr.name in cls._attrs:
            print('Cannot re-add '+str(attr))
        else:
            assert(attr.obj == cls._name)
            cls._attrs[attr.name] = attr
    ...
    def _mkAttr(cls
        ...
        if isinstance(data,tuple):
            ...
            return Attr(name,dtype=data,obj=cls._name)
    ...
    def _mkView(cls
        ...
        return View(cls._universe,attr,const,nn)

    @classmethod
    def _get_unsafe(cls,colname:str)->Attr:
        return cls._attrs[colname]

    @classmethod
    def insert(cls,cols:Optional[List[str]] = None)->InsertUpdate:
        if cols is None:
            return Insert(cls._universe,cls._pks)
        else:
            try:
                return Insert(cls._universe,[cls._get_unsafe(x) for x in cols])
    ...
    def update(cls,attrs : List[str])->Update:
        return Update(cls._universe,[cls._get_unsafe(x) for x in attrs])
    ...
    def update_component(cls
        ...
        """
        return InsertFK(cls._universe,attrs,cls)

    @classmethod
    def _mkTable(cls)->Table:
        return Table(name = cls._name,desc = cls._desc
                    ,cols = [c._mkCol() for c in cls._attrs.values()]
                    ,fks  = [fk.to_old_style() for fk in cls._fks])
    @classmethod
...
def new_model(name:str)->Object:
    ...
    class New(Object):
        ...
        def _init_subclass(cls) -> None:
            ...
            if name in uni:
                warn = 'WARNING: %s was already defined for this universe'
                print(warn % name)
            ...
            for p in cls._parents:
                ...
                if isinstance(p,type) or p.lower() in uni:
                    ...
                    #------------------------------------------------------------
                    if all([isinstance(pp,type) for pp in parent._parents]):
...
class From(object):
    ...
    #--------------
    def __str__(self) -> str:
        tab = ' %s AS %s '%(self.obj._name,self.obj._alias)
...
class View(object):
        ...
        for o1 in self.objects:
            if not o1._many_to_one:
                for o2 in o1._parents:
                    G.add_edge(o2._name,o1._name)
            for o2 in o1._parents + o1._components:
                    G.add_edge(o1._name,o2._name)
        return G
    ...
    def get_fk(self,o1:Object,o2:Object)->FK:
        ...
        """
        fks = [fk for fk in o1._fks if fk._to is o2]  + [fk for fk in o2._fks if fk._to is o1]
        if len(fks) != 1:
