...
class JemuWebApi(object):
    ...
    def upload_file(self, file_path, analytics):
        t = threading.Thread(target=self._upload_file, args=[file_path, analytics])
        t.start()
        self._threads.append(t)

    def _upload_file(self, filepath, analytics):
        ...
        for try_number in range(3):
            try:
                ...
                res.raise_for_status()
                analytics.set_filename(os.path.basename(res.url))
                signed_url = res.text
