...
def test_override_order():
    e = extension.Extensions({
        DesiredType: {
            object: lambda _, _2: u'obj',
            UnrelatedType: lambda _, _2: u'unrelated',
        }
    })
    assert e.cast(DesiredType, u'') == u'obj'
    assert e.cast(DesiredType, None) == u'obj'
    assert e.cast(DesiredType, UnrelatedType()) == u'unrelated'
    assert e.cast(DesiredType, ChildType()) == u'obj'
...
def test_add_cast_include_subtypes():
    ...
    assert e.try_cast(Aunt, c) == u'boo'
    assert e.try_cast(Cousin, c) == u'boo'
    ...
    assert e2.try_cast(Cousin, c) == u'boo'
