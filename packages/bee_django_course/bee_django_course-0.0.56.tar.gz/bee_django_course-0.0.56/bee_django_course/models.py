...
class UserCourseSection(models.Model):
    ...
    def open_ucs(self):
        # 如果课件为自动通过或者TIPS，则自动通过，开启下一课
        if self.section.auto_pass or self.section.type == 1:
            self.status = 2
    ...
    def update_work_time(self, minutes):
        ...
        if self.section.has_videowork and not self.section.has_imagework and not self.section.has_textwork:
            if self.work_time >= self.section.video_length_req or (not self.section.video_length_req):
                self.get_pass()
