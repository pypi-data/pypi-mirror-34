...
from ch_frb_wview.chlog_database import LogMessage
from ch_frb_wview.chime_frb_operations import UserAccounts, Acquisition, SignUpForMonitoring, Base
...
def get_rpc_client():
    ...
    if _rpc_client is None:
        from ch_frb_wview.rpc_client import RpcClient
...
def get_cnc_client(user="l1operator"):
    ...
    if False:
        ...
        from ch_frb_wview.cnc_client import CncClient
        client = CncClient(ctx=app.zmq)
        return client
    from ch_frb_wview.cnc_ssh import CncSsh
...
def parse_config(app):
    ...
    config_filename = os.path.join(here, 'config/l1_production_8beam_webapp.yaml')
...
def node_service():
    ...
    for node in app.cnc_nodes+app.L2_L3_node:
        try:
...
def l1_logs_recent():
    ...
    datecut = datetime.now() - timedelta(0, 600)
    print (datecut)
    logs = log_session.query(LogMessage).filter(LogMessage.date >= str(datecut))
    #logs = log_session.query(LogMessage).all()
    logs = logs[::-1]
