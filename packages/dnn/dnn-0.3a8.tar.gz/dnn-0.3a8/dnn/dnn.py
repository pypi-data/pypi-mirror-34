...
class DNN:
    ...
    def trainable (
            self, initial_learning_rate=0.001, decay_step=100, decay_rate=0.99, 
            overfit_threshold = 40, accuracy_threshold = 0.0, improve_metric = "cost"
        ):
        assert improve_metric in ("cost", "accuracy"), "improve_metric is one of cost|accuracy"
        ...
        with self.graph.as_default ():
            ...
            self.log ("opt/default/cost", self.cost)
            self.log ("opt/default/cost/regularized", self.cost + tf.losses.get_regularization_loss ())
    ...
    def optimizer (self, name = 'adam', cost = None, learning_rate = None, var_names = [], **karg):
        ...
        if not var_names:
            ...
            for name_ in var_names:
                reg_losses += tf.losses.get_regularization_loss (name_)
            ...
            for var in t_vars:                
                for name_ in var_names:
                    if name_ in var.name:
                        ...
                        break
            karg ["var_list"] = list (collected)
            ident = "/" + "+".join (var_names)

        if cost is None:
            cost = self.cost
        reg_cost = tf.add (cost, reg_losses)
