...
class ML_Logger:
    ...
    def log_image(self, namespace="images", fmt="04d", ext="png", step=None, **kwargs):
        ...
        if self.step is None:
            ...
            for key, image in kwargs.items():
                filename = os.path.join(self.prefix or "", namespace, key, f"{self.step:{fmt}}.{ext}")
                self.logger.send_image(key=filename, data=image)

    def log_video(self, frame_stack, key, namespace='videos', fmt="04d", format="mp4", step=None,
                  fps=20, macro_block_size=None, **imageio_kwargs):
        """
        ...
        if self.step is None:
            filename = os.path.join(self.prefix or "", namespace, key + f".{format}")
        else:
            filename = os.path.join(self.prefix or "", namespace, key, f"{self.step:{fmt}}.{format}")

        import tempfile, imageio
        with tempfile.NamedTemporaryFile(suffix=f'.{format}') as ntp:
            imageio.mimwrite(ntp.name, frame_stack, format='mp4', fps=fps,
                             macro_block_size=macro_block_size, **imageio_kwargs)
            self.logger.log_buffer(key=filename, buf=frame_stack)
