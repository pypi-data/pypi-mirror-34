...
**Derivative Work:** Additions and Modifications to NADCON5 code are released explicitly under Public Domain. Additional modules within this project are released under MIT License. See: [Licensing](#s-license)

As a product of the United States Government NADCON5 Source Code is considered a work under public domain in the United States, the legal status abroad is not clear at this time to `nc5ng.org`
...
Modifications to NGS `nadcon5` source code, as well as elements of the build system
contained in the file `Makefile` as well as documentation generated directly from source code are released under public domain as it applies in the United States of America. Elsewhere, the US Government reserves the right to copyright and intelectual property and the legal status is not clear to `nc5ng.org`. However, in those cases, if the US License allows, the modification should be considered public domain if it exists in the relevant jurisdiction. Please see [LICENSE-0](LICENSE-0) for a formal license statement for the public domain portion of this work/


Code examples, novel documentation,  the `nc5ng-core` python package and python processing pipeline, and all elements of  python library `nc5ng` including subpackages `nc5ng.nc5data` and `nc5ng.nc5core` are novel derivative works and are released under MIT License. Due to the tightly coupled nature with the source material, they are provided in the same distribution under a dual license. See [LICENSE-MIT](LICENSE-MIT) for more information. 