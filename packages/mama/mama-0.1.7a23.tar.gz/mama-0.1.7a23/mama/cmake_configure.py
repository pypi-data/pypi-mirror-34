...
def add_flag(cxxflags:dict, flag):
    cxxflags[flag] = ''

def get_flags_string(cxxflags:dict):
    flags = ''
...
def cmake_default_options(target):
    ...
    if config.windows:
        add_flag(cxxflags, '/EHsc')
        add_flag(cxxflags, '-D_HAS_EXCEPTIONS=1' if exceptions else '-D_HAS_EXCEPTIONS=0')
        add_flag(cxxflags, '-DWIN32=1') # so yeah, only _WIN32 is defined by default, but opencv wants to see WIN32
        add_flag(cxxflags, '/MP')
    else:
        if not exceptions: add_flag(cxxflags, '-fno-exceptions')
    
    if config.android and config.android_ndk_stl == 'c++_shared':
        add_flag(cxxflags, f'-I"{config.ndk_path}/sources/cxx-stl/llvm-libc++/include"')
    elif config.linux:
        add_flag(cxxflags, '-march=native')
        if config.clang:
            add_flag(cxxflags, '-stdlib=libc++')
    elif config.macos:
        add_flag(cxxflags, '-march=native')
        add_flag(cxxflags, '-stdlib=libc++')
    elif config.ios:
        add_flag(cxxflags, '-arch arm64')
        add_flag(cxxflags, '-stdlib=libc++')
        add_flag(cxxflags, f'-miphoneos-version-min={config.ios_version}')

    if config.flags:
        add_flag(cxxflags, config.flags)
