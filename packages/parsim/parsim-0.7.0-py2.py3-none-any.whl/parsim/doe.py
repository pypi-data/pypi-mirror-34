...
class fullfact(FactorLevelScheme):
    ...
    def _calc_norm(self):
        ...
        # Compute design matrix
        self.norm_matrix = 2*pd.fullfact(self.levels) / (np.array(self.levels)-1) - 1
...
class fracfact(FactorLevelScheme):
    ...
    def _calc_norm(self):
        ...
        # Compute design matrix
        self.norm_matrix = pd.fracfact(' '.join(self.gen_list))

        # Optional folding
        if self.fold == 'all':
            self.norm_matrix = pd.fold(self.norm_matrix)
        elif self.fold:
            self.norm_matrix = pd.fold(self.norm_matrix, columns=list(self.fold))
...
class ccdesign(FactorLevelScheme):
    ...
    Keyword arguments:
        mapping (str): Selection of method for mapping high/low factor levels to actual values in the distribution.
                   ...
                   given by parameter 'beta' (default: 0.9545)
        beta (float): Width of confidence interval for 'int' method (see above). Default: 0.9545 (+/- 2*sigma)
        face (str): Specifies combination of corner and facila points: 'cci' for inscribed (defualt), 'ccc'
    ...
    def _calc_norm(self):
        ...
        # Compute design matrix
        self.norm_matrix = pd.ccdesign(self.get('npar'), (0,1), self.get('alpha'), self.get('face'))
