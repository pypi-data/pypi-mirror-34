...
class TestWMLClientWithSpark(unittest.TestCase):
    ...
    def test_04_create_deployment(self):
        ...
        deployment = self.client.deployments.create(asset_uid=self.model_uid, name="Test deployment", asynchronous=False)
    ...
    def test_09_score(self):
        ...
        predictions = self.client.deployments.score(TestWMLClientWithSpark.scoring_url, scoring_data, transaction_id='payload_logging_test')
    ...
    def test_10_check_db_content(self):
        ...
        rows = cur.fetchall()
        for row in rows:
            self.assertTrue('payload_logging_test' in row[0])
            print("ROW = ", row, "\n")
