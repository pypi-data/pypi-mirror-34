...
def prop2qid(prop, value, endpoint='https://query.wikidata.org/sparql', value_type='str'):
    """
    ...
    :type value: str
    :param value_type: can be one of 'str', 'item', 'uri'
    :type value_type: str
    :return: wdid as string or None
    """
    assert value_type in {'str', 'item', 'uri'}
    if value_type == 'str':
        value = '"{}"'.format(value)
    elif value_type == 'item':
        assert value[0] in {'Q', 'P'}, "if value_type is 'item', value must start with 'Q' or 'P'"
        value = 'wd:{}'.format(value)
    elif value_type == "uri":
        value = '<{}>'.format(value)
