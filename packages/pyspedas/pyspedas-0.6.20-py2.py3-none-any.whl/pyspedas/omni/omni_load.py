...
Parameters:
    ...
    downloadonly: bool (True/False)
        If True then CDF files are downloaded only, if False then they are also loaded into pytplot using cdf_to_tplot.
    varformat : str
...
def omni_filename(dates, level):
    ...
    for sdate in dates:
        ...
        filename = r'omni_hro_' + level + '_' + year + month + '01' + '_v0' + version + '.cdf'
...
def omni_load(dates, level, downloadonly=False, varformat=None, get_support_data=False, prefix='', suffix=''):
    ...
    if not downloadonly:
        try: 
            downloaded_vars = pyspedas.cdf_to_tplot(downloaded_files, varformat, get_support_data, prefix, suffix, False, True)                
