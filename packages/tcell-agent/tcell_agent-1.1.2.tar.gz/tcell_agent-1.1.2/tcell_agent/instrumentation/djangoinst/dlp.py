...
def check_max_database_rows_reached(request, total_records, dataloss_policy, appfirewall_enabled):
    if request:
        safe_wrap_function(
            "Add number of DB records to appsensor_meta_event",
            appsensor_add_db_rows_to_meta,
            request,
            total_records,
            appfirewall_enabled)
...
def dlp_instrumentation():
    ...
    def _tcell_execute_sql(_tcell_original, self, *args, **kwargs):
        ...
        try:
            ...
            if (dataloss_policy and dataloss_policy.enabled) or rust_policies.instrument_database_queries:
                try:
                    ...
                    if self.select:
                        try:
                            ...
                            if result_type == MULTI:
                                ans = multicallback(results, track_idxes, request, dataloss_policy, rust_policies.appfirewall_enabled)
                                if ans:
                                    return ans

                            if results and request and len(track_idxes.keys()) > 0 and result_type == SINGLE:
                                check_max_database_rows_reached(request, 1, dataloss_policy, rust_policies.appfirewall_enabled)
                                return singlecallback(results, track_idxes, request, 0)

                        except Exception as b:
                            if rust_policies.instrument_database_queries:
