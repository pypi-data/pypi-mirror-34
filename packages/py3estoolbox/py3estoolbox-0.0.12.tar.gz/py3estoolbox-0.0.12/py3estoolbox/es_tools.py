...
class ES():
  ...
  def test_analyzers(self, index=None, alias=None, text=None):
    if index is None: index = self.get_index_by_alias(alias=alias)
    response_list = {}
    for analyzer in  self.get_analyzer(index=index):
      response_list[analyzer] = self.test_analyzer(index=index,analyzer=analyzer,text=text)
...
if __name__ == "__main__": 
  ...
  #print (es.test_analyzers(alias='products',text='Journalism'))
