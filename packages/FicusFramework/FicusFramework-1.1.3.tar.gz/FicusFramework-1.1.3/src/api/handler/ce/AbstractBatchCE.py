...
class AbstractBatchCE(ITaskHandler, IBatchOutputer, ICacheAbleHandler):
    ...
    def __action(self, data_compute_execution, output_stream, params, is_finished, messages):
        ...
        try:
            ...
            self.do_compute(BatchOutputPipe(output_stream, len(data_compute_execution.outputFdCodes)),
                            FdInputPipe(data_compute_execution.outputFdCodes), params)
        finally:
    ...
    def execute(self, params):
        ...
        try:
            ...
            self.__tasks_local_host.tasks.append(threading.Thread(target=self.__action, args=(
                data_compute_execution, output_stream,
                params, is_finished,
                messages)))
            # 增加发送结果的线程
            self.__tasks_local_host.tasks.append(threading.Thread(target=self.batch_output, args=(
                params["code_"],
                data_compute_execution.outputFdCodes,
                is_finished,
                output_stream)))
            ...
            self.__tasks_local_host.tasks = None
