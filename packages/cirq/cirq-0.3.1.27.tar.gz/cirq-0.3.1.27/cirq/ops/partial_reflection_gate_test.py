...
class DummyGate(PartialReflectionGate):
    ...
    def _with_half_turns(self, half_turns = 1.0):
...
def test_partial_reflection_gate_eq():
    ...
    eq.make_equality_group(lambda: DummyGate(half_turns=Symbol(u'a')))
    eq.make_equality_group(lambda: DummyGate(half_turns=Symbol(u'b')))
...
def test_partial_reflection_as_self_inverse():
    ...
    ha = DummyGate(half_turns=cirq.Symbol(u'a'))
...
def test_partial_reflection_gate_trace_bound():
    ...
    assert DummyGate(half_turns=cirq.Symbol(u'a')).trace_distance_bound() >= 1


def test_partial_reflection_gate_with_parameters_resolved_by():
    gate = DummyGate(half_turns=Symbol(u'a'))
