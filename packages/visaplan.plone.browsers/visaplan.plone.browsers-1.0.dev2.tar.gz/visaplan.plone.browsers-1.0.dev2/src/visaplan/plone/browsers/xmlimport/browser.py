...
class Browser(BrowserView):
    ...
    def canImport(self, portal_type):
        ...
        if not checkperm('unitracc: Import XML'):
            return False
        if not checkperm('unitracc: Import %s' % portal_type):
            return False
        if not checkperm('unitracc: Add %s' % portal_type):
            return False
        return True
