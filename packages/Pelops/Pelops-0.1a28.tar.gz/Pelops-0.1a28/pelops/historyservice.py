...
class HistoryService:
    ...
    def __init__(self, config, max_length, topic_sub, register_topic_handler, update_available, mqtt_client, logger):
        ...
        self._logger = pelops.mylogger.get_child(logger, __name__)
        self._logger.info("HistoryService.__init__")

        self._group_by = int(self._config["group-by"])
        if self._group_by <= 0:
            self._logger.error("HistoryService.__init__ - 'group-by' must be > 0. ({})".format(self._group_by))
            raise ValueError("HistoryService.__init__ - 'group-by' must be > 0. ({})".format(self._group_by))
        ...
        if self._config["aggregator"] == "avg":
            ...
            self._logger.error("HistoryService.__init__ - unknown aggregator '{}'.".format(self._aggregator))
            raise ValueError("HistoryService.__init__ - unknown aggregator '{}'.".format(self._aggregator))
    ...
    def start(self):
        ...
        """
        self._logger.debug("HistoryService.start - acquiring history_lock")
        with self.history_lock:
            self._logger.debug("HistoryService.start - acquired history_lock")
            self.history.clear()
        self._logger.debug("HistoryService.start - released history_lock")
    ...
    def _topic_sub_handler(self, value):
        ...
        """
        value = float(value)
        self._logger.info("HistoryService._topic_sub_handler - received value '{}'.".format(value))
    ...
    def add_value(self, value, timestamp=None):
        ...
        self._logger.info("HistoryService.add_value - value: {}, timestamp: {}.".format(value, timestamp))

        self._logger.debug("HistoryService.add_value - acquiering _add_lock")
        with self._add_lock:
            self._logger.debug("HistoryService.add_value - acquired _add_lock")
            self._add_value(value, timestamp)
        self._logger.debug("HistoryService.add_value - released _add_lock")
        self._request_dataservice_response()
    ...
    def _add_value(self, value, timestamp, ignore_history_lock = False):
        ...
        self._logger.info("HistoryService._add_value - append value {}.".format(value))
    ...
    def _topic_dataservice_response(self, message):
        ...
        self._logger.info("HistoryService._topic_dataservice_response - received message with len={} on topic '{}'.".
                          format(len(message), self._dataservice_response_topic))
        if len(self.history) == self._max_length:
            self._logger.info("HistoryService._topic_dataservice_response - skipped message / history is fully filled.")
        else:
            response = json.loads(message.decode("utf-8"))
            self._logger.debug("HistoryService._topic_dataservice_response - received data: {}".format(response))
    ...
    def _merge_import(self, response_history, history_entries):
        ...
        self._logger.debug("HistoryService._merge_import - start")
        self._logger.debug("HistoryService._merge_import - merging {} old and {} new entries.".
        ...
        self._logger.debug("HistoryService._merge_import - response from {} to {} / history from {} to {}.".
        ...
        self._logger.debug("HistoryService._merge_import - added {} entries from response.".
        ...
        self._logger.debug("HistoryService._merge_import - added {} gap entries.".
                           format(len(new_history)-previous_new_history_len))
        previous_new_history_len = len(new_history)
        ...
        self._logger.debug("HistoryService._merge_import - added {} history entries.".
                           format(len(new_history)-previous_new_history_len))
        ...
        self._logger.debug("HistoryService._merge_import - copied {} entries into history.".format(len(self.history)))
        self._logger.debug("HistoryService._merge_import - finished")
    ...
    def _direct_import(self, response_entries):
        ...
        self._logger.debug("HistoryService._direct_import - start")
        ...
        self._logger.debug("HistoryService._direct_import - added {} entries with a shift of {}.".
                           format(len(response_history), shift))

        self._logger.debug("HistoryService._direct_import - finished")
    ...
    def _aggregated_import(self, response_entries):
        ...
        self._logger.debug("HistoryService._aggregated_import - start")
        ...
        self._logger.debug("HistoryService._aggregated_import - added {} entries.".format(len(response_history)))

        # restore aggregation status
        self._aggregation_timestamp = stored_aggregation_timestamp
        self._aggregation = stored_aggregation

        self._logger.debug("HistoryService._aggregated_import - finished")
        return response_history
    ...
    def _gap_in_group_by(self, history_first, response_last):
        ...
        self._logger.debug("HistoryService._gap_in_group_by - history_first: {}, response_last: {}, result: {}."
    ...
    def _dataservice_import(self, response):
        ...
        self._logger.info("HistoryService._dataservice_direct_import - start")
        self._logger.debug("HistoryService._dataservice_direct_import - acquiering _add_lock")
        with self._add_lock:
            self._logger.debug("HistoryService._dataservice_import - acquired _add_lock")
            self._logger.debug("HistoryService._dataservice_direct_import - acquiering history_lock")
            with self.history_lock:
                self._logger.debug("HistoryService._dataservice_import - acquired history_lock")
                ...
                self._logger.debug("HistoryService._dataservice_direct_import - history: {}".format(history_entries))
                self._logger.debug("HistoryService._dataservice_direct_import - response: {}".format(response_entries))
                self._logger.debug("HistoryService._dataservice_direct_import - history_first: {}, response_last: {}, "
                ...
                self._logger.debug("HistoryService._dataservice_direct_import - new history: {}".format(self.history))

            self._logger.debug("HistoryService._dataservice_direct_import - released history_lock")

        self._logger.debug("HistoryService.add_value - released _add_lock")

        self._logger.info("HistoryService._dataservice_direct_import - finished. merged history ({}) with response ({}) to "
    ...
    def _check_if_response_message_is_valid(self, response):
        ...
        if response["version"] != self._ACCEPTED_VERSION:
            self._logger.warning("HistoryService._check_if_response_message_is_valid - skipped message / expected version '{}'"
            ...
            self._logger.info("HistoryService._check_if_response_message_is_valid - skipped message / expected values for "
                              "group-by are {} and < {} but received {} instead.".
                              format(self._group_by, self._group_by / 2, group_by))
        elif response["len"] == 0:
            self._logger.info("HistoryService._check_if_response_message_is_valid - skipped message / received empty data set")
        elif len(self.history) > 0 and first >= datetime.fromtimestamp(self.history[0]["time"]):
            self._logger.info("HistoryService._check_if_response_message_is_valid - skipped message / no new data available")
        else:
            self._logger.info("HistoryService._topic_dataservice_response - message accepted")
            result = True
    ...
    def _request_dataservice_response(self):
        ...
        if self._use_dataservice:
            if self._max_length > len(self.history):
                ...
                message = {
                    "first": first.strftime(self._TIME_FORMAT),
                    "last": datetime.now().strftime(self._TIME_FORMAT),
                    "group-by": self._group_by
                }
                self._logger.info("HistoryService._request_dataservice_response - send message '{}' to topic '{}'.".
                ...
                self._logger.debug("HistoryService._request_dataservice_response - skipped / history is fully filled.")
    ...
    def _aggregation_to_history(self, timestamp, ignore_history_lock = False):
        ...
        if ignore_history_lock:
            self._logger.debug("HistoryService._aggregation_to_history - ignoring history_lock")
            self.history.append(entry)
        else:
            self._logger.debug("HistoryService._aggregation_to_history - acquiering history_lock")
            with self.history_lock:
                self._logger.debug("HistoryService._aggregation_to_history - acquired history_lock")
                self.history.append(entry)
            self._logger.debug("HistoryService._aggregation_to_history - released history_lock")

        self._logger.info("HistoryService._aggregation_to_history - add value: {}, time: {}, len history: {}".
