...
class GPSInfo(object):
    ...
    def __init__(self, coord, alt=0, timeStamp=None):
...
class GPSPhoto(object):
        ...
        if not self.__gpsRawDict is None:
            ...
            for tag in list(tags.keys()):
                if tag == 'GPS GPSTimeStamp':
                    ...
                    self.__gpsDict['UTC-Time'] = str(str(t[0]) + ":" + str(t[1])
                            + ":" + str(t[2]))
    ...
    def modGPSData(self, gpsInfo, newFileName):
        ...
        gpsDict[5] = 0  # Altitude
        gpsDict[6] = (gpsInfo.alt * 1000, 1000)  # Altitude
        ...
        if sys.argv[1][0] == '-':
            try:
                if sys.argv[1].upper()[1] == 'H':
                    ...
                    if len(sys.argv) < 6:
                        notEnoughArguments()
                    info = GPSInfo(coord=(float(sys.argv[4]),
                            float(sys.argv[5])))
                    if len(sys.argv) > 6:
                        info.alt = int(sys.argv[6])
                    if len(sys.argv) > 7:
                        info.timeStamp = sys.argv[7]
