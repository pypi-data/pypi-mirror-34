...
class LdpTargetedRouterV6(Base):
	...
	def LdpTargetedIpv6Peer(self):
		"""Returns the one and only one LdpTargetedIpv6Peer object from the server.
		...
		return self._read(LdpTargetedIpv6Peer(self), None)

	@property
	def LdpTargetedPeer(self):
		"""Returns the one and only one LdpTargetedPeer object from the server.
		...
		return self._read(LdpTargetedPeer(self), None)
	...
	def remove(self):
		"""Deletes a child instance of LdpTargetedRouterV6 on the server.
	...
	def ClearAllLearnedInfo(self, Arg1):
		...
		Args:
			Arg1 (list(str[None|/api/v1/sessions/1/ixnetwork/topology])): This parameter requires a list of /topology/./ldpTargetedRouterV6 object references
