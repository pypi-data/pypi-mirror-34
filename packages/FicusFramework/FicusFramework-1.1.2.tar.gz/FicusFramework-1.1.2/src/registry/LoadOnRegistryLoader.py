...
log = logging.getLogger('LoadOnRegistryLoader')


def check_ficus_online(eureka_client):
    """
    检查ficus服务是否存在
    :param eureka_client:
    :return:
...
def createNewComputeExecutions(registry, taskHandlerNames, site, projectcode):
    ...
    for computeexecution in registry["computeexecutions"]:
        ...
        # region 补默认值
        if "code" not in computeexecution or computeexecution["code"] == "":
            # TODO
            computeexecution["code"] = "actorName" + "-" + index

        if "execution" not in computeexecution:
            computeexecution["execution"] = {"schedule": "0 0 0 1 1 ? 2099", "limits": -1, "params": []}
        else:
            if "schedule" not in computeexecution["execution"]:
                computeexecution["execution"]["schedule"] = "0 0 0 1 1 ? 2099"
            if "limits" not in computeexecution["execution"]:
                computeexecution["execution"]["limits"] = -1
            if "params" not in computeexecution["execution"]:
                computeexecution["execution"]["params"] = {}

        if "managed" not in computeexecution:
            computeexecution["managed"] = {"type": "STANDBY", "serviceName": "STANDBY"}
        else:
            if "type" not in computeexecution["managed"]:
                computeexecution["managed"]["type"] = "STANDBY"
            if "serviceName" not in computeexecution["managed"]:
                computeexecution["managed"]["serviceName"] = "STANDBY"

        if "outputs" not in computeexecution:
        ...
        if computeexecution["handler"] not in taskHandlerNames:
        ...
        if not FactDatasourceClient.exists_fds(fds):
            # 只要有一个fd不存在,就返回false
            # 表示校验异常了.
    ...
    for computeexecution in needRegistryList:
        ...
        ce = {"type": "JAVA", "sourceFdCodes": computeexecution["inputs"], "outputFdCodes": computeexecution["outputs"],
              ...
              "limitTimes": computeexecution["execution"]["limits"], "params": computeexecution["execution"]["params"],
...
def createNewCrawls(registry, taskHandlerNames, site, projectcode):
    ...
    for crawl in registry["crawls"]:
        ...
        if "managed" not in crawl:
            crawl["managed"] = {"type": "STANDBY", "serviceName": "STANDBY"}
        else:
        ...
        if crawl["handler"] not in taskHandlerNames:
...
def registry_after_started(app, eureka_client):
    """
    服务启动后,检测是否需要注册服务
    :return:
    """
    ...
    yaml = read_yaml_file(app, "registry.yml")
    ...
    # 说明有yml文件,开始进行处理
    registry = yaml["sobeyficus"]["registry"]
    ...
    if not check_ficus_online(eureka_client):
    ...
    # 找到所有的taskHandler的名称
    taskHandlerNames = TASK_HANDLERS.keys()
