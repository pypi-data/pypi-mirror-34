...
class Calendar(ttk.Frame):
    ...
    def __init__(self, master=None, year=None, month=None, 
        ...
        # Added Prev Year Button
        prev_yr = ttk.Button(hframe, style='L.TButton', 
        ...
        self._header.pack(side=LEFT, padx=5)
        self._header.configure(foreground='blue', font='Consolas 11')

        rbtn = ttk.Button(hframe, style='R.TButton', command=self._on_press_right_button)
        rbtn.pack(side=LEFT)

        #Added Next Year Button
        next_yr = ttk.Button(hframe, style='R.TButton', 
            command=self.next_year)
        next_yr.pack(side=LEFT)
        st = tk.ttk.Style()
        st.configure("G.TButton", width=4, 
            background='powderblue',font='calibri 8', fg='green' )
        self.jump_to = tk.ttk.Button(hframe, text='GoTo')
        self.jump_to.pack(side=tk.RIGHT, padx=5)
        self.jump_to.configure(style="G.TButton")
        ...
        for i, day_of_the_week in enumerate(days_of_the_week):
            tk.Label(self, text=day_of_the_week, background='grey90').grid(row=1, column=i, sticky=N+E+W+S)
    ...
    def _find_label_coordinates(self, date):
        ...
        (first_weekday_of_the_month - self._cal.firstweekday)//7 not 
    ...
    def _show_selection(self):
        ...
        i,j = self._find_label_coordinates(self._selected_date.date())
    ...
    def _go_to_year(self, year):
        ...
        self._build_calendar(year, 1) # reconstruct calendar
...
class CalenderWidget(ttk.Entry):
    def __init__(self, master, entrywidth=None, entrystyle=None,
        datevar=None, dateformat="%Y-%m-%d", onselect=None,
        firstweekday=calendar.SUNDAY,
        locale=None, activebackground='#b1dcfb',
        activeforeground='black', selectbackground='#003eff',
        selectforeground='white', borderwidth=2, relief="solid", *args, **kwargs):
        ...
        ttk.Entry.__init__(self, master,
            textvariable=self.date_var, font="Arial 13", **entry_config)
        ...
        self.calendar_frame = Calendar(self.winfo_toplevel(),
            firstweekday=firstweekday,
            locale=locale, activebackground=activebackground,
        ...
        # CTRL + PAGE UP: Move to the next month.
        self.bind("<Control-Prior>", lambda event: self.calendar_frame.next_month())

        # CTRL + PAGE DOWN: Move to the prev month.
        self.bind("<Control-Next>", lambda event: self.calendar_frame.prev_month())

        # CTRL + SHIFT + PAGE UP: Move to the next year.
        self.bind("<Control-Shift-Prior>", lambda event: self.calendar_frame.next_year())

        # CTRL + SHIFT + PAGE DOWN: Move to the previous year.
        self.bind("<Control-Shift-Next>", lambda event: self.calendar_frame.prev_year())
...
class DateField(CalenderWidget):
    def __init__(self, parent, readonly=False, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)

        if readonly:
            self.configure(state='readonly')

        self.calendar_frame.jump_to.configure(
        command=lambda event=None: self.calendar_frame._go_to_year(self.get_date()))
...
class DateTimeField(CalenderWidget):
    def __init__(self, parent, **kwargs):
        super().__init__(parent, entrywidth=40, dateformat='%Y-%m-%d %H:%M:%S', **kwargs)
...
if __name__ == "__main__":
    ...
    picker = DateTimeField(root, font="Arial 12")
