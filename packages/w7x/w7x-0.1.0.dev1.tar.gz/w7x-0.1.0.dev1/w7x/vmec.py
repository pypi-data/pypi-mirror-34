...
class Points3D(w7x.flt.Points3D):
    wsServer = w7x.Server.addrVmecServer
...
class PowerSeries(Profile):
    propDefaults = Profile.propDefaults
    propDefaults['ProfileType'] = 'power_series'
    propDefaults['coefficients'] = [0]
...
def startVmecString(inData, **kwargs):
    ...
    """
    defaultVmecId = None
    vmecServer = w7x.getServer(w7x.Server.addrVmecServer)
...
class Run(object):
    ...
    def getPhiEdgeEstimate(self, minVolume=25, maxVolume=30, islandWidth=0.075):
        self._parseThreed1()
        import symTools
        from symTools import fit, parameters
        ...
        iota = symTools.Variable('iota')
        r = symTools.Variable('r')
