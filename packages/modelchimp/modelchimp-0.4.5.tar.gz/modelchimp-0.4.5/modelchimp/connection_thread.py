...
class WSConnectionThread(Thread):
    ...
    def send(self, event):
        ...
        if self.is_connected():
            json_data = json.dumps({'message': event}, cls=ParamEncoder)
    ...
    def create_experiment(self, experiment_id, filename):
        ...
        try:
            with open(filename, 'rb') as f:
                ...
                model_id = json.loads(save_request.text)['model_id']
                print('Experiment is live at %smodel-detail/%s' % (self.address, model_id))
            return True
        except Exception:
