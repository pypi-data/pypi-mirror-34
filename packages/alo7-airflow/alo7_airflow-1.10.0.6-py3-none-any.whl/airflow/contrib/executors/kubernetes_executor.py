...
class KubeConfig:
    ...
    def __init__(self):
        ...
        # logging
        self.log_container_repository = conf.get(
            self.kubernetes_section, 'log_container_repository')

        self.log_container_tag = conf.get(
            self.kubernetes_section, 'log_container_tag')
        self.log_container = '{}:{}'.format(
            self.log_container_repository, self.log_container_tag)

        self.log_container_name = conf.get(
            self.kubernetes_section, 'log_container_name')
...
class KubernetesJobWatcher(multiprocessing.Process, LoggingMixin, object):
    ...
    def process_status(self, pod_id, status, labels, resource_version):
        if status == 'Pending':
            ...
            self.log.info('Event: %s Completed', pod_id)
            self.watcher_queue.put((pod_id, None, labels, resource_version))
        elif status == 'Running':
