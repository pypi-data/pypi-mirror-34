...
Some scattering length densities as guide to choose realistic values for SLD and solventSLD :
 - neutron scattering  unit nm\ :sup:`-2`:
    - protonated polyethylene glycol = 0.640e-6 A\ :sup:`-2` = 0.640e-4 nm\ :sup:`-2`
 ...
 - Xray scattering  unit nm^-2:
    - polyethylene glycol            = 1.09e-3 nm\ :sup:`-2` = 387 e/nm\ :sup:`3`
...
def cloudScattering(q, cloud, relError=50, V=0, formfactor=None, rms=0, ffpolydispersity=0,ncpu=0):
    ...
    The result is normalized by :math:`I0=(\sum b_i)^2` to equal one for q=0 (except for polydispersity).
    ...
    Notes
    -----
    We calculate the scattering amplitude :math:`F(q)` for :math:`N` atoms in a volume :math:`V` with scattering
...
def scatteringFromSizeDistribution(q,sizedistribution,func=beaucage,weight=None,**kwargs):
    r"""
    Scattering of a size distribution of objects.
    ...
    Examples
    --------
    The contribution of different simple size distributions::
     ...
     bi=[[12,70],[1,1]]
     Iq=js.ff.scatteringFromSizeDistribution(q=q,sizedistribution=bi,d=3,weight=lambda r:(r/12)**6)
     p.plot(Iq,legend='with aggregates')
     Iq=js.ff.scatteringFromSizeDistribution(q=q,sizedistribution=bi,d=3)
     p.plot(Iq,legend='with aggregates')
     # 2:1 concentration
     bi=[[12,70],[1,2]]
     p.plot(js.ff.scatteringFromSizeDistribution(q=q,sizedistribution=bi,d=2.5),legend='no aggregates')
     p.yaxis(scale='l')
     p.xaxis(scale='l')
...
def multilamellarVesicles(Q, R, N, phi, displace=0, dR=0, dN=0,shellthickness=0,ds=0, SLD=1, solventSLD=0,nGauss=100):
    ...
    dN : int, default=0
        ...
        Check .Ndistribution and .Nweight = Nweight for the used distribution.
    shellthickness: float,list of float,default=0
        Thickness of shells in symmetric layer in units nm.
        Zero assumes infinite thin layer with constant formfactor.
