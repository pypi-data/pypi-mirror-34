...
class itkImageTransformerICF2(ITKCommonBasePython.itkProcessObject):
    ...
    Base class for all process objects that transform an image into
    something else.

    ImageTransformer is the base class for all process objects that
    transform an image data. Specifically, this class defines the
    SetInput() method that takes a pointer to the input image. The class
    also defines some internal private data members that are used to
    manage streaming of data.
    ...
    ImageTransformers default to not releasing the bulk data incase that
    ...
    ImageTransformer's can be forced to use a memory management model
    similar to the default ProcessObject behaviour by calling
    ...
    C++ includes: itkImageTransformer.h 
    """
