...
See
`CHANGELOG.md <https://github.com/alecthomas/voluptuous/blob/master/CHANGELOG.md>`__.
...
However, an empty set (``set()``) is treated as is. If you want to
specify a set that can contain anything, specify it as ``set``:
...
pre-validated data from the first “pass” and so will not have to do its
own type checking on its inputs.
