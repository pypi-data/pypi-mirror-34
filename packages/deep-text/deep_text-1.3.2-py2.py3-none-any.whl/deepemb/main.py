...
from .default_custom import DefaultSkipThoughtTransform
from .train import SkipThoughtTrainer
...
def main_learn():
    ...
    try:
        ...
        print('deepemb_learn [skip-thought] -t <trainfile> -e <testfile> -c <configfile> -m <model save path> '
    ...
    if model_type == "skip-thought":
        config = config_ops.load_config(config_file_path, train_input_path, test_input_path)
        trainer = SkipThoughtTrainer(config, DefaultSkipThoughtTransform, need_transform, need_build_word2vec)
        model = trainer.train()
    else:
        print('The model type must be \'skip-thought\'')
        sys.exit()
...
def main_save():
    ...
    try:
        ...
        print('deepemb_save -p <checkpoint path> -c <config path> -m <model file path>')
        sys.exit(2)
    ...
    scopes = ['loss/loss',
              'encoder/rnn/while/Exit_2',
              'softmax_prev_prediction/prev_prediction',
              'decoder_prev_prediction/rnn/while/Exit_2',
              'softmax_post_prediction/post_prediction',
              'decoder_post_prediction/rnn/while/Exit_2']
