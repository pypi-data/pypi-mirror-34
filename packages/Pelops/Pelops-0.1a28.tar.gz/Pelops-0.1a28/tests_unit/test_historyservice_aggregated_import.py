...
from pelops.historyservice import HistoryService
...
class TestHistoryServiceAggregatedImport(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.mqtt_client = MQTTClient(cls.main_config["mqtt"], cls.logger)
    ...
    def test_00aggregated_import_all(self):
        ...
        def mqtt_request_handler(message):
            ...
            first = datetime.strptime(request["first"], HistoryService._TIME_FORMAT)
            last = datetime.strptime(request["last"], HistoryService._TIME_FORMAT)
            group_by = float(request["group-by"]) / 3
            next = first
            response = {"data": []}
            value = 0
            while next < last:
                response["data"].append({"time": next.strftime(HistoryService._TIME_FORMAT), "value": value})
                next = next + timedelta(seconds=group_by)
                value += 1

            response["len"] = value
            response["first"] = first.strftime(HistoryService._TIME_FORMAT)
            response["last"] = last.strftime(HistoryService._TIME_FORMAT)
        ...
        hs = HistoryService(self.config, 10, "/topic/test", False, self.update_available, self.mqtt_client, self.logger)
    ...
    def test_03aggregated_import_partial(self):
        ...
        self.config["use-dataservice"] = True
        hs = HistoryService(self.config, 20, "/topic/test", False, self.update_available, self.mqtt_client, self.logger)
        self.assertIsNotNone(hs)
        hs.start()
        t = time.time() - 100
