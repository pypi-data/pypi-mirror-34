...
class KubernetesRequestFactory:
    ...
    def extract_lifecycle(pod, req):
        req['spec']['containers'][0]['lifecycle']['preStop']['exec']['command'] = '["/bin/sh", "-c", "echo stop > /tmp/base_stop"]'
    ...
    @staticmethod
    def extract_log_containers(pod, req):
        if pod.log_containers:
            req['spec']['containers'][1] = pod.log_containers[0]
