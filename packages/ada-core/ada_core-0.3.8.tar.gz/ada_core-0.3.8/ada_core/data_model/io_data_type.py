...
class BinaryNumberTsInputType(BaseType):

    primitive_type = dict
    native_type = BinaryNumberTsInput

    def to_native(self, value, context=None):

        if isinstance(value, BinaryNumberTsInput):
            try:
                value.validate()
            except (DataError, ConversionError) as exp:
                raise ConversionError('Not validate BinaryTsInput: {}'.format(exp))
            ...
            data_model = BinaryNumberTsInput(value)
            try:
                data_model.validate()
            except (DataError, ConversionError) as exp:
...
try:
    ...
    class AlgorithmIODataType(Enum):
        ...
        BINARY_NUMBER_TS_INPUT = BinaryNumberTsInputType
        ...
        def deduceType(cls, input_value):
            ...
            try:
                if not type_validate:
                    binaryNumTsType = AlgorithmIODataType.BINARY_NUMBER_TS_INPUT.value()
                    binaryNumTsType.validate(input_value)
                    type_validate = True
                    return AlgorithmIODataType.BINARY_NUMBER_TS_INPUT.value
            except BaseError:
                type_validate = False
