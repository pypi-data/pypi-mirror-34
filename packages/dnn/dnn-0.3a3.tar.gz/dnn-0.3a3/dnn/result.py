...
class Result:    
    ...
    def update (self, summary = {}, *args, **karg): 
        try:
            self.accuracy = self.accfunc (self.logit, self.y, *args, **karg)
        except NotImplementedError:
            self.accuracy = 0.0    
        self.summaryfunc (self.is_validating  and 'recall' or 'valid', self.make_summary (summary), True)    
    ...
    def make_summary (self, d):
        d  ["test/cost/regularized"] = self.cost
        if isinstance (self.accuracy, (tuple, list)):
            if not self.labels:
                for i, acc in enumerate (self.accuracy):
                    d ["test/accuracy/{}".format (i + 1)] = acc                    
            else:                
                for i, label in enumerate (self.labels):
                    try: d ["test/accuracy/" + label.name] = self.accuracy [i]
                    except IndexError: break
        else:
            d ["test/accuracy"] = self.accuracy
        return d
