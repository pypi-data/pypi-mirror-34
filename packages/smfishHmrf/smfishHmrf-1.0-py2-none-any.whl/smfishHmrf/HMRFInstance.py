...
from smfishHmrf.DatasetMatrix import DatasetMatrix
...
class HMRFInstance:
	...
	def prepare(self):
		...
		ngene = expr.shape[0]
		ncell = expr.shape[1]
		os.system("cp %s/f%s.adjacency.txt %s/." % (src, file_prefix, dest))
		os.system("cp %s/f%s.edges.txt %s/." % (src, file_prefix, dest))
		os.system("cp %s/f%s.coordinates.txt %s/." % (src, file_prefix, dest))
		os.system("cp %s/f%s.blocks.txt %s/." % (src, file_prefix, dest))
		'''
		...
		'''
		fw = open("%s/genes" % self.dest, "w")
		for g in genes:
			fw.write(g + "\n")
		fw.close()
		fw = open("%s/f%s.expression.txt" % (dest, file_prefix), "w")
		for ic in range(ncell):	
			fw.write("%d " % (ic+1) + " ".join(["%.2f" % expr[j,ic] for j in range(ngene)]) + "\n")
		fw.close()
		'''
...
tc.hmrfem<-mritc.hmrfem.multi(y=par_y, neighbors=par_nei, beta=par_beta, numnei=par_numnei, 
blocks=par_blocks, mu=par_mu, sigma=par_sigma, verbose=T, 
spatialMat=diag(1,par_k), err=1e-7, maxit=50, dampFactor=par_damp)
