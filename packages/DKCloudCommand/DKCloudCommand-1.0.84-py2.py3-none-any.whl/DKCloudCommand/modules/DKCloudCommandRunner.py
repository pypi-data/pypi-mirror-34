...
class DKCloudCommandRunner(object):
    ...
    def _get_all_files(path):
        ...
        for item in os.listdir(path):
            ...
            if os.path.isfile(item_path):
                with codecs.open(item_path,'r', encoding='utf-8') as f:
                    result[item_path] = f.read().encode('utf-8')
    ...
    def update_file(dk_api, kitchen, recipe_name, recipe_dir, message, files_to_update_param):
        ...
        try:
            ...
            for file_to_update in files_to_update:
                try:
                    with codecs.open(file_to_update, 'r', encoding='utf-8') as f:
    ...
    def write_files(recipe_path, files_dict):
        for k, v in files_dict.iteritems():
            ...
            if contents is not None:
                DKFileHelper.write_file(full_path, contents)
    ...
    def add_file(dk_api, kitchen, recipe_name, message, api_file_key):
        ...
        try:
            with codecs.open(api_file_key, 'r', encoding='utf-8') as f:
                file_contents = f.read().encode('utf-8')
        except ValueError as e:
            s = 'ERROR: %s' % e.message
            rc.set(rc.DK_FAIL, s)
