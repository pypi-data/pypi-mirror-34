...
from publisher.utils import get_content_directories, WorkingDirectory, get_user_commit_selection, get_command_output, \
                            get_procedure_code, get_yes_no, get_input
from publisher.validators import validate_save
from publisher.exceptions import ProcedureCreationError
...
def create_procedure_branch(procedure):
    ...
    for remote_procedure in build_procedure_list():
        if remote_procedure.lower() == procedure.lower():
            print('Git is case sensitive, the procedure ' + remote_procedure +
                  ' already exists. The procedure you tried to create - ' +
                  procedure +
                  ' - has the same name. Please either correct the case of the procedure or create a new '
