...
RESOURCES = {
    ...
    'tags_keywords': {
        'endpoint': '/tags/list?site_id={}',
        'schema': load_schema('tags_keywords'),
        'key_properties': ['site_id','Tag_Id','Keyword_Id'],
    },
    'keywords': {
...
def sync_tags(project_id, site_id):
    ...
    with Transformer(pre_hook=format_timestamp_make_null) as transformer:
        for row in gen_request(url, {"format": "json", "results":"5000"}):
            row = dict(Id = row["Id"], Tag = row["Tag"], Type = row["Type"])
...
def sync_tags_keywords(project_id, site_id):
    url = get_url("tags_keywords", site_id)
    with Transformer(pre_hook=format_timestamp_make_null) as transformer:
        for row in gen_request(url, {"format": "json", "results":"5000"}):
            if row["Keywords"] == "none":
                pass
            else:
                for kw_id in row["Keywords"]["Id"]:
                    data = dict(Tag_Id = row["Id"], Keyword_Id = kw_id)
                    transformed_row = transformer.transform(data, RESOURCES["tags_keywords"]["schema"])

                    singer.write_record("tags_keywords", transformed_row, time_extracted=utils.now())
...
def sync_project(project_id, site_id):
    ...
    sync_tags_keywords(project_id, site_id)
