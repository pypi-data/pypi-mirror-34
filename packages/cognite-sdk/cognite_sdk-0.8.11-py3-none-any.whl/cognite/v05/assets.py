...
def get_assets(name=None, path=None, description=None, metadata=None, depth=None, fuzziness=None, **kwargs):
    ...
    params = {'name': name, 'description': description, 'path': path, 'metadata': (str(metadata) if metadata else None), 'depth': depth, 'fuzziness': fuzziness, 'cursor': kwargs.get('cursor'), 'limit': kwargs.get('limit', constants.LIMIT)}
