...
class MlRepo(Repo):
    ...
    def open_index(self):
        ...
        index = BackendMLIndex(self._connection_by_name('main'), self._config, self.__session, handle_api)
    ...
    def direct_bucket_upload(self):
        return self.data_volume_config.object_store_config.get('bucket_name') is not None
    ...
    def create_object_store(self):
        ...
        if bucket_name is None:
            store = BackendGCSObjectStore(
                ...
                self._config, self.__session, handle_api)
        else:
            store = GCSObjectStore(self._connection_by_name('gcs'))
    ...
    def remote_commit(self, message, isolation_token):
        ...
        return handle_api(self._config, self.__session, 'post', url, msg, is_async=True)
