...
class HTTPv1(HTTP):
    ...
    def _read_http_header(self, header):
        ...
        if match1 and match2:
            ...
            header = dict(
                request = dict(
                    method = self.decode_bytes(para1),
                    target = self.decode_bytes(para2),
                    version = self.decode_bytes(match2.group('version')),
                ),
            )
            ...
            header = dict(
                response = dict(
                    version = self.decode_bytes(match3.group('version')),
                    status = int(para2),
                    phrase = self.decode_bytes(para3),
                ),
            )
        else:
            raise ProtocolError('HTTP: invalid format', quiet=True)
        ...
        try:
            for item in lists:
                key = self.decode_bytes(item[0].strip().replace(b'request', b'request_field').replace(b'response', b'response_field'))
                value = self.decode_bytes(item[1].strip())
                if key in header:
