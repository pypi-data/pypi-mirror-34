...
from .train import CDSSMTrainer
...
def main_learn():
    ...
    try:
        ...
        print('deepmatch_learn [cdssm] -t <trainfile> -e <testfile> -c <configfile> -m <model save path> '
    ...
    if model_type == "cdssm":
        ...
        trainer = CDSSMTrainer(config, DefaultTransform, need_transform, need_build_word2vec)
        model = trainer.train()
    else:
        print('The model type must be \'cdssm\'')
        sys.exit()
...
def main_save():
    ...
    try:
        ...
        print('deepmatch_save -p <checkpoint path> -c <config path> -m <model file path>')
        sys.exit(2)
