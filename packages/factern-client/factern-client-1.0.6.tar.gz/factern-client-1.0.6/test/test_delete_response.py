# coding: utf-8

"""
    Factern API
"""


from __future__ import absolute_import

import unittest

import factern_client
from factern_client.com.factern.model.delete_response import DeleteResponse  # noqa: E501
from factern_client.rest import ApiException


class TestDeleteResponse(unittest.TestCase):
    """DeleteResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testDeleteResponse(self):
        """Test DeleteResponse"""
        # FIXME: construct object with mandatory attributes with example values
        # model = factern_client.models.delete_response.DeleteResponse()  # noqa: E501
        pass


if __name__ == '__main__':
    unittest.main()
