...
#    requirements = f.readlines()
    ...
    database = y.get('log_database', 'sqlite:///%s'%os.path.join(here, 'databases/log.sqlite3'))
    #database = y.get('log_database', 'sqlite:///log.sqlite3')
    print (database)
    engine = sqlalchemy.create_engine(database)
    app.make_db_session = scoped_session(sessionmaker(bind=engine))

    operations_database = y.get('chime_frb_operations_database', 'sqlite:///%s'%os.path.join(here,'databases/chime_frb_operations.sqlite3'))
