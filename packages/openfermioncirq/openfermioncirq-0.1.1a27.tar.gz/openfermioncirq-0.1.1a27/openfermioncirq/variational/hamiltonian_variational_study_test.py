...
def test_hamiltonian_variational_study_init_qubit_operator():
    ...
    study = HamiltonianVariationalStudy(
            u'study', ansatz, openfermion.QubitOperator((0, u'X')))
    assert study.hamiltonian == openfermion.QubitOperator((0, u'X'))


def test_hamiltonian_variational_study_noise():

    ansatz = SwapNetworkTrotterAnsatz(test_hamiltonian)
    study = HamiltonianVariationalStudy(u'study', ansatz, test_hamiltonian)
...
def test_hamiltonian_variational_study_optimize():
    ansatz = SwapNetworkTrotterAnsatz(test_hamiltonian)
    study = HamiltonianVariationalStudy(u'study',
                                        ansatz,
                                        test_fermion_op)
    study.optimize(
            OptimizationParams(
    ...
    result = study.results[u'run']
    assert all(result.data_frame[u'optimal_parameters'].apply(study.evaluate) ==
...
def test_hamiltonian_variational_study_save_load():
    datadir = u'tmp_ffETr2rB49RGP8WE8jer'
    study_name = u'test_hamiltonian_study'
    ...
    result = loaded_study.results[u'example']
    assert isinstance(result, OptimizationTrialResult)
    assert result.repetitions == 1
    assert result.params.cost_of_evaluate == 1.0

    # Clean up
    os.remove(u'{}/{}.study'.format(datadir, study_name))
    os.rmdir(datadir)
...
@pytest.mark.parametrize(
        u'ansatz_factory, trotter_algorithm, hamiltonian, atol', [
...
def test_trotter_ansatzes_evaluate_order_1(
    ...
    u"""Check that a Trotter aansatz with one iteration and default parameters
    ...
    preparation_circuit = cirq.Circuit.from_ops(
            prepare_gaussian_state(
                ...
                occupied_orbitals=xrange(len(qubits) // 2))
    )
    study = HamiltonianVariationalStudy(u'study',
                                        ansatz,
                                        hamiltonian,
...
def test_trotter_ansatzes_evaluate_order_2(
        ansatz_factory, trotter_algorithm, hamiltonian, atol):
    u"""Check that a Trotter ansatz with two iterations and default parameters
