...
class LabelEventFilter(QtCore.QObject):
    ...
    def eventFilter(self, obj: Union[QtWidgets.QGraphicsTextItem], event):
...
class Renderer(metaclass=ABCMeta):  # MixIn
    ...
    def __init__(self, *args, **parameters):
        ...
        self.item: Union[
            pg.PlotItem, pg.PlotCurveItem, pg.ImageItem, InfiniteLinePlot
        ] = None
    ...
    def allRendererParameterProcessing(self, parameters: Dict) -> Dict:
    ...
    def strColor(self) -> str:
        ...
        q_color = QtGui.QColor.fromRgb(
            self.view.color[0], self.view.color[1], self.view.color[2]
        )
    ...
    def setLimits(self):
        ...
        self.vb.setLimits(yMin=self.parameters["y_min"], yMax=self.parameters["y_max"])
...
class Waveform(Renderer):
    ...
    def getDefaultYRange(self) -> Tuple[float, float]:
        if self.track.min and self.track.max:
            ...
            return {np.dtype("int16"): (-32768, 32768), np.dtype("float"): (-1, 1)}[
                self.track.value.dtype
            ]
    ...
    def generateBlankPlotItems(self):
        ...
        self.vb.setMouseEnabled(x=True, y=False)
        self.vb.sigXRangeChanged.connect(
            self.generatePlotData, QtCore.Qt.DirectConnection
        )
    ...
    def generatePlotData(self):
        ...
        if ds == 1:
            ...
            while source_pointer < stop - 1:
                ...
                visible[
                    1 + target_pointer : 1 + target_pointer + chunk_len * 2 : 2
                ] = chunk_max
                target_pointer += chunk_len * 2
            visible = visible[:target_pointer]
        self.item.setData(
            x=np.linspace(start, stop, num=len(visible), endpoint=True) / self.track.fs,
            y=visible,
            pen=self.view.color,
        )
...
class Spectrogram(Renderer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.parameters = {"frame_size": 0.01, "normalized": 0, "y_min": 0, "y_max": 1}
    ...
    def compute_initial_levels(self):
        if self.parameters["normalized"]:
            ...
            centers = np.round(
                np.linspace(half, self.track.duration - half, 1000)
            ).astype(np.int)
    ...
    def generatePlotData(self):
        ...
        if X.shape[0]:
            ...
            self.item.setImage(
                image=np.fliplr(-X[:, bottom:top]), levels=[-self.vmax, -self.vmin]
            )
...
class Partition(Renderer):
    ...
    def clearViewBox(self):
        for _, item in enumerate(self.vb.addedItems[:]):
            try:
    ...
    def createNames(self):
        ...
        self.names = [self.genName(value) for value in self.track.value]
        for name in self.names:
            self.vb.addItem(name)

    def positionLabels(self):
        for index, label in enumerate(self.names):
            self.calcPartitionNamePlacement(label, index=index)
    ...
    def calcPartitionNamePlacement(
        self, label: pg.TextItem, index: int, emit_signal=False
    ):
    ...
    def removeSegment(self, line: pg.InfiniteLine, modify_track=True):
        ...
        self.calcPartitionNamePlacement(
            self.names[index - 1], index - 1, emit_signal=False
        )
    ...
    def receivePartitionBoundaries(self, index: int, x_min: float, x_max: float):
    ...
    def receivePartitionValue(self, index):
        ...
        self.names[index].textItem.document().contentsChanged.connect(self.nameChanged)

    def movePartition(self, line: pg.InfiniteLine):
        index = self.segments.index(line)
        self.track.time[index] = int(round(line.getXPos() * self.track.fs))
        self.calcPartitionNamePlacement(
            self.names[index - 1], index=index - 1, emit_signal=True
        )
        if index < len(self.segments) - 1:
            self.calcPartitionNamePlacement(
                self.names[index], index=index, emit_signal=True
            )
    ...
    def genLine(self, time, movable=False):
        line = pg.InfiniteLine(time, angle=90, pen=self.pen, movable=movable)
        line.parent = self
        return line
...
class PartitionEdit(Partition):
    ...
    def insertSegment(self, x_pos: float):
        ...
        index = np.searchsorted(self.track.time, np.array([time])).astype(int)[0]
...
def get_renderer_classes(
    accepts: Optional[tracking.Track] = None
) -> List[Type[Renderer]]:
