...
def move_dynamics(matrix, target_min_bound=0.0, target_max_bound=255.0, init_min_bound=0.0, init_max_bound=255.0, convert_uint=False, init_to_content=False):
    """ Transform pixel dynamics to another scale given by [target_min_bound, target_max_bound]

        :param matrix: numpy array
        :param target_min_bound: float of the lower bound of the new scale
        :param target_max_bound: float of the upport bound of the new scale
        :param convert_uint: True if you want unsigned integers in order to create images
    """
    # fix in case the input as minimum values lower than user definitions in init_min_bound
    smin = np.min(matrix)
    if smin < init_min_bound:
        init_min_bound = smin

    # fix in case the input as maximum values greater than user definitions in init_max_bound
    smax = np.max(matrix)
    if smax > init_max_bound:
        init_max_bound = smax

    if init_to_content == True:
        init_min_bound = smin
        init_max_bound = smax
    ...
    matrix = ((matrix - init_min_bound) / (init_max_bound - init_min_bound)) * dyn_range + target_min_bound
...
def isogradient(gdal_band, blur_radius=7):
    ...
    conv = move_dynamics(conv, target_min_bound=0.0, target_max_bound=1.0, init_to_content=True)
...
def bands_average(mapy, bands, weights=[]):
    ...
    for k in bands[1:stop_at_index]:
        ...
        band_mix += weights[w] * mapy.get_band(int(k)).ReadAsArray()
        w += 1
...
def integrate_shadows(mapy, bands, integration_horizon=5, offset=0):
    """ Integrate shadows over the next bands as they represent the coming shadows
        ...
        :param offset: start from a different index in bands, useful when recalled
    """
    integrated_list = []
    ...
    for start in bands[offset:]:
        ...
        if start > stop:
            ...
            bands_subset = np.append(bands_subset, bands[:(len(weights)-len(bands_subset))])
        else:
