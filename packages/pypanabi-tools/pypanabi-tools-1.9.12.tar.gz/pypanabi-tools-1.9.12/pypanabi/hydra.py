...
class Hydra(object):
    ...
    def __init__(self, brand, channel, rdl_schema=None, odl_schema=None):
        """
        Constructor
        ...
        self._rdl_schema = rdl_schema or 'hydra'
        self._odl_schema = odl_schema or 'ods'
    ...
    def connect_database(self, config, profile):
        """
    ...
    def get_tables_and_views(self, layer):
        if layer.lower() == 'rdl':
            for k, v in self._rdl_tables_views:
                print '{} = {}'.format(k, v)
        if layer.lower() == 'odl':
            for region in self._odl_tables_views:
                print region.upper()

                for k, v in self._odl_tables_views[region]:
                    print '{} = {}'.format(k, v)
    ...
    def extract(self, s3_path, json_as):
        ...
        try:
            ...
            self._logger.error("Could not load the path '{s3_path}' into the table '{sync_table}', skipping loading. [{error}]".format(s3_path=s3_path,
                                                                                                                                       sync_table=self._rdl_tables_views[self._region]['sync_table'],
                                                                                                                                       error=e))
            pass
...
class ClassicStream(Hydra):
    def __init__(self, brand, region, channel, rdl_schema=None, odl_schema=None):
        """
        Constructor
        :param brand: brand name OLX or Letgo [String]
