...
class Stepper(object):
    ...
    * Two qubit phase gates exp(i t |11><11|)
    ...
    def sample_measurements(
        ...
        if repetitions < 1:
            raise ValueError(
                u'Number of repetitions cannot be negative. Was {}'.format(
        ...
        sum_indices = tuple(
            x for x in xrange(self._num_qubits) if x not in indices)
        ...
        index_map = dict((v, k) for k,v in enumerate(sorted(indices)))
        ...
        # ordering correct.
        return np.transpose(
            [(1 & (result >> i)).astype(np.bool) for i in xrange(len(indices))][
            ::-1]).tolist()
...
def check_state(state, num_qubits):
