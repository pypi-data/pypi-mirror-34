        ...
        while idx < len(obj):
            ...
            try:
                ...
                for zero in range(match is not None):
                    ...
                    if match[1].lower() not in cls.tags: break

                    # Get the shape
                    shp = cls.tags[match[1].lower()]

                    # Fail if the tag is already opened
                    if match[1] in open_tags: break

                    try:

                        # Get the tag
                        t = shp.tag(attr=match[2])
                ...
                for zero in range(match is not None):
                    ...
                    if match[1] not in open_tags: break

                    # If the opening tag is last in the DOM
                    if isinstance(ret[-1], tag.Tag) and ret[-1].name == match[1]:
                    ...
                    # Close it
                    ret.append(tag.Tag(match[1], type=tag.Tag.END))

                    # Remove it from opened list
                    open_tags.remove(match[1])
