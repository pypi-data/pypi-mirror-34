...
class LearningSystem(WMLResource):
    ...
    def update(self, model_uid, changes):
        ...
        updated_details = self._update(url, details, changes, [
            {"name": self.ConfigurationMetaNames.FEEDBACK_DATA_REFERENCE, "type": dict,     "path": [u'feedback_data_reference']},
            {"name": self.ConfigurationMetaNames.MIN_FEEDBACK_DATA_SIZE,  "type": int,      "path": [u'min_feedback_data_size']},
            {"name": self.ConfigurationMetaNames.SPARK_REFERENCE,         "type": dict,     "path": [u'spark_service', u'credentials']},
            {"name": self.ConfigurationMetaNames.AUTO_RETRAIN,            "type": STR_TYPE, "path": [u'auto_retrain']},
            {"name": self.ConfigurationMetaNames.AUTO_REDEPLOY,           "type": STR_TYPE, "path": [u'auto_redeploy']}
        ])
