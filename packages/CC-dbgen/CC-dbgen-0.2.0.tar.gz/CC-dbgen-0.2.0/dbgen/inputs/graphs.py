...
from dbgen.support.datatypes.rule      import (Rule,Plan,PureSQL,PyBlock
                                              ,SqlBlock,Block,SimpleUpdate
                                              ,SimpleInsert)
...
get_chargemol_struct = Rule(name='chargemol_struct'
    ,query = """SELECT chargemol_job.job_id,J.stordir,J.anytraj AS raw
                ...
                AND NOT J.deleted"""
    ,plan= Plan([PyBlock(get_cell,'cell',[Arg('raw')])
                ,PyBlock(get_atom,args = [Arg('raw')]) # returns ([ax]m,[ay]m,...)
                ,SqlBlock(name = 'insert_cell'
                       ,text = mkInsCmd('cell',cell_cols,sqlite=False)
                       ,args = cell_args)
                ,SqlBlock(name='query_cell'
                       ,text=('SELECT C.cell_id FROM cell AS C WHERE '
                              +addQs(['C.%s'%x for x in cell_cols],' AND '))
                       ,args=cell_args)

                 ,SqlBlock(name  = 'insert_struct'
...
populate_bond = Rule('bond'
    ...
    ,plan  = Plan( unpackargs
                 + [PyBlock(parse_chargemol if local else parse_chargemol_sherlock
                            ,'parse_chargemol'
                            ,args=noIndex(['stordir','json_to_traj']))
                    ,PyBlock(json_to_traj,args=[Arg('anytraj')])
                    ,SqlBlock(name = 'insert_bond'
...
graph_p_blks  = [PyBlock(mk_graph,args = noIndex(['atoms']+mk_graph_args[1:]))
                ,PyBlock(json_to_traj,'atoms',[Arg('rawstruct')])
                ,PyBlock(graph_to_json,'raw',[Arg('mk_graph')])] # type: List[Block]
