...
def sync_repos(ctx, repo_specs, force):
    ...
    for repo_spec in repo_specs:
        ...
        click.echo("✨ Syncing repo {}.".format(repo_path))
        try:
        ...
        if workspace_id not in workspace_info:
            workspace_info[workspace_id] = []
        workspace_info[workspace_id].append(
            {'name': repo_spec['name'], 'commit_hash': commit_hash})

    if len(invalid_repos) > 0:
        click.echo(click.wrap_text("The following repos could not be synced to spell: \n - " +
                                   "\n - ".join(invalid_repos)), err=True)
...
def push_workspace(ctx, git_repo, commit_ref, force=False):
    ...
    workspace_client = WorkspacesClient(owner=username, token=token, **ctx.obj["client_args"])
