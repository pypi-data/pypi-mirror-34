...
formatter_dict = {
    ...
    4: logging.Formatter('%(asctime)s - %(name)s - "%(filename)s" - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s -               File "%(pathname)s", line %(lineno)d ', "%Y-%m-%d %H:%M:%S"),  # 这个也支持日志跳转
}
...
class ColorHandler(logging.Handler):
    ...
    def __init__(self, filter_logger_name_list=None):
        ...
        self.formatter_new = logging.Formatter(
            '%(asctime)s - %(name)s - "%(filename)s" - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s              \033[0;93;107mFile "%(pathname)s", line %(lineno)d, in %(funcName)s\033[0m',
            "%Y-%m-%d %H:%M:%S")
        self.formatter_new = logging.Formatter(
    ...
    def emit(self, record):
        ...
        if self.formatter is formatter_dict[4] or self.formatter is self.formatter_new:
            ...
            self.__emit_for_fomatter4(record)
        else:
            self.__emit(record)

    def __emit_for_fomatter4(self, record):
        #              \033[0;93;107mFile "%(pathname)s", line %(lineno)d, in %(funcName)s\033[0m
        try:
            msg = self.format(record)
            if record.levelno == 10:
                print('\033[0;32m%s\033[0m' % self._my_align(msg, 200) + ' ' * 10 + '\033[0;93;107mFile "%s", line %d\033[0m' % (record.pathname, record.lineno))  # 绿色
            elif record.levelno == 20:
                print('\033[0;34m%s\033[0m' % self._my_align(msg, 200) + ' ' * 10 + '\033[0;93;107mFile "%s", line %d\033[0m' % (record.pathname, record.lineno))  # 青蓝色
            elif record.levelno == 30:
                print('\033[0;92m%s\033[0m' % self._my_align(msg, 200) + ' ' * 10 + '\033[0;93;107mFile "%s", line %d\033[0m' % (record.pathname, record.lineno))  # 蓝色
            elif record.levelno == 40:
                print('\033[0;35m%s\033[0m' % self._my_align(msg, 200) + ' ' * 10 + '\033[0;93;107mFile "%s", line %d\033[0m' % (record.pathname, record.lineno))  # 紫红色
            elif record.levelno == 50:
                print('\033[0;31m%s\033[0m' % self._my_align(msg, 200) + ' ' * 10 + '\033[0;93;107mFile "%s", line %d\033[0m' % (record.pathname, record.lineno))  # 血红色
        except (KeyboardInterrupt, SystemExit):
            raise
        except:  # NOQA
            self.handleError(record)
...
class LogManager(object):
    ...
    def __add_handlers(self):
        if self._is_add_stream_handler:
            for h in self.logger.handlers + logging.getLogger().handlers:
