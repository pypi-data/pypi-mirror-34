...
class SyncToLocalProcesses(object):
    ...
    def create_export_task_db(self, profile, dbtools, incremental):
        ...
        sql = """insert into assetic_sync_manager
            ...
            useinternalnames,replacespaces,spacedelimiter,setemptyasnull,status,
    ...
    def run_search_for_pages(self, searchguid, searchfilter=None):
        """
...
class DB_Tools(object):
    ...
    def data_sync_to_db(self, allrows, columns, dbtable, keyfield, logtable):
        """
        For a array containing csv delimted data sync to db.
        Assums data in columns parameter is the headers for the csv array
        Param1 allrows: the array of data to sync
        Param2 columns: the list of column headers in same order as data
        Param3 dbtable: the database table to sync with
        Param4 keyfield: the primary key used to determine if new or update
        Param5 logtable: the name of the table to log inserts,updates,deletes
        ...
        for logcolumn in existinglogcolumns:
            if (logcolumn in columns and logcolumn in existingcolumns):
                ...
                if (logcolumn not in ['action', 'syncdate']):
                    ##this column in log table will be ignored
                    msg = ("column [{0}] in log table is not in either the "
                           "export data or the target table [{1}] and so it "
        ...
        if dbtable_exists == False:
            sqlsetkeynotnull = "alter table {0} alter column {1} " \
                               "nvarchar(500) not null".format(dbtable,
                                                               keyfield)
            sqlkeypk = "alter table {0} add CONSTRAINT {0}_key_PK PRIMARY KEY(" \
                       "{1})".format(dbtable, keyfield)
            knullable == "NO"
        else:
        ...
        if knullable != None and knullable.upper() == "YES":
            msg = "Key field {0} in table {1} should be set as 'NOT NULL'" \
                  " with a unique index/primary key constraint".format(
                keyfield, dbtable)
            self.logger.warning(msg)
            ##SQL to make the key field nullable
            sqltempkeynotnull = "alter table {0} alter column {1} nvarchar({2})" \
                                " COLLATE {3} not null".format(tmpdbtable,
                                                               keyfield, kmax,
                                                               kcollation)
        sqltempkeypk = "alter table {0} add CONSTRAINT tmp{1}key_PK PRIMARY KEY" \
                       "({2})".format(tmpdbtable, dbtable, keyfield)
...
class FullAssetExport(object):
    ...
    def __init__(self, api_client=None):
        ...
        :param hdrmappings: the header columns, use to build dict
        """
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client
    ...
    def export_asset(self, search_filter,export_defs):
        ...
        The same assets are exports for each def, but the attributes etc
        ...
        for x in export_defs:
            ...
            mapping = self.prepare_fields_from_config(
                x["config_file"])
        ...
        if len(template_list) > 0:
            chk = self.process(search_filter, template_list, attributes,
                               inclusions)
            if chk != 0:
                return chk
        return 0
    ...
    def prepare_fields_from_config(self, config_file):
        ...
        :return: list of field dict that define mapping of field
        """
        ...
        structure = ("output_field", "level_type", "asset_field"
                     ...
                     , "nm_record_type", "constant", "field_size")
    ...
    def process(self, asset_filter, templates, attributes=None,
                inclusions=[]):
        """
        Control method.  Get assets and then output to one or more files
        :param asset_filter: optional filter that restricts the assets to
        process
        ...
        allassets = self.asset_tools.get_list_of_complete_assets(asset_filter,
                                                                 attributes,
                                                                 inclusions)
        self.logger.info("{0} assets to export to csv".format(len(allassets)))
        if len(allassets) == 0:
            return 0
        for template in templates:
            self.logger.info("Start Create file {0}".format(template["file"]))
            chk = self.process_assets(allassets, **template)
            if chk == 0:
                self.logger.info("End Create file {0}".format(template["file"]))
        self.logger.info("Process New Assets - Complete")
        return chk
    ...
    def process_assets(self, allassets, **kwargs):
        ...
        csvstring = self.format_as_csv(alldata)
        # prepend row
    ...
    def asset_to_template(self, asset, template):
        ...
        for k, v in six.iteritems(template):
            if v.constant is not None:
                ...
                elif v.level_type.lower() == "component":
                    for comp in asset.components:
                        if getattr(comp.component_representation,
                                   "component_type") == v.component_type:
                            try:
                                data[k] = getattr(comp.component_representation
                                                  , v.asset_field)
                            except Exception as ex:
                                msg = "component mapping issue: {0}".format(ex)
                                self.logger.warning(msg)
                                data[k] = None
                    for comp in asset.components:
                        if getattr(comp.component_representation,
    ...
    def format_as_csv(self, alldata):
        """
...
class CompleteAssetExportFieldMappings(object):
    ...
    def __init__(self, output_field=None, asset_field=None, level_type=None,
                 ...
                 nm_record_type=None, constant=None, field_size=0):
        """
