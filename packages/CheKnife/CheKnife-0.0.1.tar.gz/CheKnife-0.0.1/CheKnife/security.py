...
class DictionarySettings(EncryptedStoredSettings):
    test = None
    config = ConfigParser()
    alias = None
    alias_fixed = None
    ...
    def secret_fields_prompt(self, config):
        ...
        for field in self.public_fields:
            config.set(self.alias_fixed, py3_unicode(field), py3_unicode(self.cipher.encrypt(prompt('Database {}'.format(field)))))
        config.set(self.alias_fixed, py3_unicode('PASSWORD'), py3_unicode(self.cipher.encrypt(hidden_prompt())))
