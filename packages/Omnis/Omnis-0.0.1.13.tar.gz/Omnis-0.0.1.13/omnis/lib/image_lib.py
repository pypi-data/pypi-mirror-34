...
def resize_image(img, target_size, cv_interpolation = cv2.INTER_LINEAR):
    """This function returns an resized image with specified target size.
...
def directory_images_to_arrays(directory_path, target_size):
    """This function recursively iterates over child files and directories of directory_path.
    Image files will be converted to array_of_images and their parent directory names will be converted to array_of_lables.
    ...
    Returns:
        array_of_images, array_of_labels [(ndarray, ndarray)] --
            array_of_images is an ndarray of resized images. array_of_labels is a label array of images.
