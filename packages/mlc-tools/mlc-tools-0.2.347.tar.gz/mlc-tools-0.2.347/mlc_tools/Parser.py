...
class Parser:
    ...
    def parse_dir(self, directory, filter_code):
        files = fileutils.get_files_list(directory)
        for file in files:
            if file.endswith('.mlc'):
                if filter_code is not None and not filter_code(file):
                    continue
                file = directory + file
                text = open(file).read()
                if self.hash.is_file_changed(file, text):
                    self.has_changes = True
                    tmp = self.classes
                    self.classes = []
                    self.parse(text)
                    self.hash.fix_file(file, text)
                    for cls in self.classes:
                        self.hash.add_cls(file, cls)
                    for cls in self.classes_for_data:
                        self.hash.add_cls_data(cls)
                    tmp.extend(self.classes)
                    self.classes = tmp
                else:
                    hashed = self.hash.get_classes_from_file(file)
                    for cls in hashed:
                        cls.is_changed = False
                        if cls.is_storage:
                            self.classes_for_data.append(cls)
                    self.classes.extend(hashed)
                    hached = self.hash.get_classes_for_data()
                    for cls in hached:
                        exist = False
                        for curr in self.classes_for_data:
    ...
    def link(self):
        ...
        for cls in self.classes:
            if cls.is_visitor_base and self.get_type_of_visitor(cls) != cls.name:
    ...
    def is_visitor(self, cls):
        if cls.is_visitor or cls.is_visitor_base:
            return True
        result = False
        for c in cls.behaviors:
    ...
    def get_type_of_visitor(self, cls):
        if not cls.is_visitor and not cls.is_visitor_base:
            return None
