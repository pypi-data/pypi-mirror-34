...
class Constraints:
    ...
    def from_dictionary(json):
        try:
            c = Constraints(json["MaxValue"], json["MinValue"], json["MinIndex"], json["MaxIndex"], json["Link"])
        except KeyError: #apparently at some points constraints didn't have these things maybe?
    ...
    def serialize(self):
        ...
        #I do not know why this was changed to that
        return {"isConstrained": self.isConstrained,
                "consMin": self.MinValue,
                "consMax": self.MaxValue,}
...
class Parameter:
    ...
    def serialize(self):
        "serialize used for __str__ and __repr__"
        my_dict= {"Value": self.value,
                ...
                "linkIndex": -1,
                "sigma": self.sigma}
        my_dict.update(self.constraints.serialize())
        return my_dict
...
class Model:
    ...
    def get_mutable_params(self):
        '''
        this does not interface with dplus directly and does not need to be dplus compatible
        :return:
        '''
        mut_array = []
    ...
    def basic_json_params(self):
        '''
        :param use_grid: this is from domain, but is applied to every single model because
        that's what the dplus code expects
...
class ModelWithChildren(Model):
    ...
    def basic_json_params(self):
        basic_dict = super().basic_json_params()
        for child in self.Children:
            basic_dict["Submodels"].append(child.basic_json_params())
        return basic_dict
        ...
        for layer_index in range(len(json["Parameters"])):
            for param_index in range(len(json["Parameters"][layer_index])):
                param = Parameter(value=json["Parameters"][layer_index][param_index],
                                  ...
                                  constraints=Constraints.from_dictionary(json["Constraints"][layer_index][param_index]))
                try:
...
class Population(ModelWithChildren):
    ...
    def load_from_dictionary(self, json):
        ...
        self.population_size_param = Parameter(value=self.population_size, mutable=self.population_size_mut)
...
class Domain(ModelWithChildren):
    ...
    def load_from_dictionary(self, json):
        ...
        self.scale_param = Parameter(value=self.scale, mutable=self.scale_mut)
        try:
            self.constant = json["Constant"] #TODO: add back if necessary
            self.constant_mut = json["ConstantMut"]
            self.constant_param = Parameter(value=self.constant, mutable=self.constant_mut)
