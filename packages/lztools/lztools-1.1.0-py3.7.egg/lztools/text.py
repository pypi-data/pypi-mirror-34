...
def center_on(value:str, text:str) -> str:
    return u"{:^{}}".format(value, Ansi.ansi_length(text))

def wall_text(text:str, width:int=80, wall:str= "|", text_alignment="<", h_padding=2, colerizer=None) -> str:
    pad = _get_padding(h_padding)
    text_alignment = _get_alignment(text_alignment)

    result, adjusted = "", width - Ansi.ansi_length(wall) * 2 - h_padding * 2
    executed = False
...
def wrap_lines(text: str, width: int = 80) -> str:
    for line in text.splitlines():
        yield from (line[i:i + width] for i in range(0, Ansi.ansi_length(line), width))
...
def trim_end(remove:str, the_text:str) -> str:
    while the_text.endswith(remove):
        the_text = the_text[:-Ansi.ansi_length(remove)]
    return the_text
