...
class MarginalizedPosterior(Posterior):
    ...
    def mean(self,categories,posterior="Default",fix_sources="Default"):
        ...
        assert(len(set(self.keys_all)&set(categories))==len(cat_count_dict.keys())),"Counts label(s) passed to mean function don't match labels of the source instances"
        ...
        if fixed_cat == []:
            ...
            pinned_mean = np.prod([fix_sources[cat]**cat_count_dict[cat] for cat in fixed_cat])
        ...
        if terr_exists: 
            func = lambda **x: (np.sum(np.log([x[cat]**cat_count_dict[cat] for cat in free_cat]))+
                                np.log(posterior(**x))-
                                np.log(self.priorDict[self.prior_type](**x))-
                                self.poissonLogWeight(**x)-
                                (self.N*np.log(x[self.key_terr])-
                                self.N*(np.log(self.N)-1)))
        else:
            func = lambda **x: (np.sum(np.log([x[cat]**cat_count_dict[cat] for cat in free_cat]))+
                                np.log(posterior(**x))-
    ...
    def getOneDimMean(self,category,posterior="Default",fix_sources="Default"):
        ...
        assert(category in self.keys_all),"Category passed to getOneDimMean function doesn't match any labels of the source instances"
    ...
    def pastro_update(self,categories,bayesfac_dict,mean_values_dict):
        ...
        assert(len(set(self.keys_all)&set(categories)) == len(categories)),"categories passed to pastro function don't match labels of the astro/terr source instances"
        assert(set(self.keys_all) == set(mean_values_dict.keys())), "keys of mean_values_dict don't match labels of the astro/terr source instances"
        assert(set(self.keys_astro) == set(bayesfac_dict.keys())), "keys of bayesfac_dict don't match labels of the astro source instances"
        ...
        categories_astro = list(set(categories)-set([self.key_terr]))
        numerator = num_terr + np.sum([mean_values_dict[key]*bayesfac_dict[key] for key in categories_astro], axis=0)
        denominator = mean_values_dict[self.key_terr] + np.sum([mean_values_dict[key]*bayesfac_dict[key] 
                                                           for key in self.keys_astro], axis=0)
