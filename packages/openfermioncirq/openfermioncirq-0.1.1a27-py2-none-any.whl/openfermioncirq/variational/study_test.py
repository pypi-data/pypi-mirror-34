...
class ExampleAnsatz(VariationalAnsatz):
    ...
    def param_names(self):
        return [u'theta{}'.format(i) for i in xrange(2)]
    ...
    def operations(self, qubits):
        a, b = qubits
        yield cirq.RotXGate(half_turns=self.params[u'theta0']).on(a)
        yield cirq.RotXGate(half_turns=self.params[u'theta1']).on(b)
        yield cirq.CZ(a, b)
        yield cirq.RotXGate(half_turns=self.params[u'theta0']).on(a)
        yield cirq.RotXGate(half_turns=self.params[u'theta1']).on(b)
...
class ExampleStudy(VariationalStudy):
    ...
    def value(self,
        ...
        measurements = trial_result.measurements[u'all']
...
test_study = ExampleStudy(u'test_study',
                          test_ansatz,
                          preparation_circuit=preparation_circuit)
test_study_noisy = ExampleStudyNoisy(u'test_study_noisy',
                                     test_ansatz,
                                     preparation_circuit=preparation_circuit)
...
def test_variational_study_optimize_and_summary():
    ...
    study = ExampleStudy(u'study', test_ansatz)
    ...
    assert study.summary.replace(u"u'", u"'").strip() == u"""
...
def test_variational_study_save_load():
    ...
    assert result.params.algorithm.options == {u'maxiter': 1}
    assert result.params.cost_of_evaluate == 1.0
