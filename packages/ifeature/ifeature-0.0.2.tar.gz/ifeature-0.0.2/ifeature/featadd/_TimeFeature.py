...
class TimeFeature(object):
    ...
    def get_feats_time(data, group=None, feats=None, ts='ts'):
        ...
        data[ts + 'year'] = data[ts].apply(lambda x: x.year)
        data[ts + 'month'] = data[ts].apply(lambda x: x.month)
        data[ts + 'day'] = data[ts].apply(lambda x: x.day)
        data[ts + 'weekday'] = data[ts].apply(lambda x: x.weekday())
        data[ts + 'diff'] = data.groupby(group)[ts].diff().apply(lambda x: x.days).fillna(0)  ##########
        data[ts + 'time_interval'] = data[ts].agg(lambda x: x.max() - x).apply(lambda x: x.days)
