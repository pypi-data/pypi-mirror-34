...
from .transport.ecr import Ecr


class InvocationDocker:
    @property
    def git_clone_image(self):
        return self.active_config.general.get('git_image', 'missinglinkai/git-lfs:env')
    ...
    async def try_ecr_image_auth_if_needed(self, image, volumes):
    ...
    async def decrypt_data(self):
        ...
        # res = await  asyncio.get_event_loop().run_in_executor(self.crypto_helper.decrypt_invocation, self)
        await self.log_cb(f'Data Decrypted. {len(res)} items decrypted', step_name='Decrypting Environment')
        try:
            self.secured_data = {'file': {}, 'env': {}}
            for item in res:
    ...
    async def pull_image(self, image, force=False):
        ...
        volumes[os.environ['ML_CONFIG_VOLUME']] = {'bind': "/root"}

        await self.try_ecr_image_auth_if_needed(image, volumes)
    ...
    async def clone_data(self):
        ...
        return await self.run_mali_command(data_mount, 'data', 'clone', str(self.data_volume), '--query', self.data_query, '--destFolder', self.data_dest_folder, '--no_progressbar')

    async def run_mali_command(self, mounts, *args, update_mali=True):
        args = [str(x) for x in args]
        logger.info(f"Run mali command {args}")
        ...
        logger.info(f"Run mali command {args}: %s", mounts)
        return await  self.run_with_callbacks_and_validate_result(
            f'mali_command: {args}',
            image=MALI_IMG,
    ...
    def _get_ssh_identity(self):
        ...
        return CryptoHelper(self.active_config).default_cipher().export_private_key_bytes('PEM').decode('utf-8')
    ...
    async def pull_code(self):
        ...
        root_mount = self.build_artifact_volume('/root')
        mounts = {
            code_mount[1]: {'bind': "/code"},
            root_mount[1]: {'bind': "/root"}
        }
        ...
        return await  self.run_with_callbacks_and_validate_result(
            ...
            volumes=mounts,
            env={'NoStrictHostKeyChecking': '1', 'SSH_IDENTITY': self._get_ssh_identity()}
        )
    ...
    async def _try_stage(self, stage_name, stage):
        try:
            ...
            await  self.log_cb(f'Failed to: {str(ex)}', step_name=stage_name)
            logging.exception(f'Failed to: {str(ex)}')
