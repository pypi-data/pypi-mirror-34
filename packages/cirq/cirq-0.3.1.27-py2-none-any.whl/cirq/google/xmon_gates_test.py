...
def proto_matches_text(proto, expected_as_text):
    expected = text_format.Merge(expected_as_text, type(proto)())
    return unicode(proto) == unicode(expected)
...
def test_parameterized_value_from_proto():
    ...
    m3 = operations_pb2.ParameterizedFloat(parameter_key=u'rr')
    assert from_proto(m3) == Symbol(u'rr')


def test_measurement_eq():
    eq = cirq.testing.EqualsTester()
    eq.make_equality_group(lambda: cg.XmonMeasurementGate(key=u''))
    eq.make_equality_group(lambda: cg.XmonMeasurementGate(u'a'))
    eq.make_equality_group(lambda: cg.XmonMeasurementGate(u'b'))
    eq.make_equality_group(lambda: cg.XmonMeasurementGate(key=u'',
                                                         invert_mask=(True,)))
    eq.make_equality_group(lambda: cg.XmonMeasurementGate(key=u'',
                                                         invert_mask=(False,)))
...
def test_single_qubit_measurement_to_proto():
    assert proto_matches_text(
        cg.XmonMeasurementGate(u'test').to_proto(GridQubit(2, 3)),
        u"""
        measurement {
            targets {
                row: 2
        ...
        """)
    assert proto_matches_text(
        cg.XmonMeasurementGate(u'test', invert_mask=[True])
...
def test_multi_qubit_measurement_to_proto():
    assert proto_matches_text(
        cg.XmonMeasurementGate(u'test').to_proto(
...
def test_invalid_measurement_gate():
    with pytest.raises(ValueError, match=u'length'):
        cg.XmonMeasurementGate(u'test', invert_mask=[True]).to_proto(
            GridQubit(2, 3), GridQubit(3, 4))
...
def test_z_eq():
    ...
    eq.make_equality_group(
        lambda: cg.ExpZGate(half_turns=Symbol(u'a')))
    eq.make_equality_group(
        lambda: cg.ExpZGate(half_turns=Symbol(u'b')))
...
def test_z_to_proto():
    assert proto_matches_text(
        cg.ExpZGate(half_turns=Symbol(u'k')).to_proto(GridQubit(2, 3)),
        u"""
        exp_z {
            target {
                row: 2
...
def test_z_parameterize():
    parameterized_gate = cg.ExpZGate(half_turns=Symbol(u'a'))
    assert parameterized_gate.is_parameterized()
    with pytest.raises(ValueError):
        _ = parameterized_gate.matrix()
    resolver = ParamResolver({u'a': 0.1})
...
def test_cz_eq():
    ...
    eq.make_equality_group(lambda: cg.Exp11Gate(half_turns=Symbol(u'a')))
    eq.make_equality_group(lambda: cg.Exp11Gate(half_turns=Symbol(u'b')))
    eq.add_equality_group(
...
def test_cz_to_proto():
    assert proto_matches_text(
        cg.Exp11Gate(half_turns=Symbol(u'k')).to_proto(
...
def test_cz_potential_implementation():
    assert not cirq.can_cast(cirq.KnownMatrix,
                             cg.Exp11Gate(half_turns=Symbol(u'a')))
    assert cirq.can_cast(cirq.KnownMatrix, cg.Exp11Gate())


def test_cz_parameterize():
    parameterized_gate = cg.Exp11Gate(half_turns=Symbol(u'a'))
    assert parameterized_gate.is_parameterized()
    with pytest.raises(ValueError):
        _ = parameterized_gate.matrix()
    resolver = ParamResolver({u'a': 0.1})
...
def test_w_eq():
    ...
    eq.make_equality_group(
        lambda: cg.ExpWGate(half_turns=Symbol(u'a')))
    eq.make_equality_group(lambda: cg.ExpWGate(half_turns=0))
    eq.make_equality_group(
        lambda: cg.ExpWGate(half_turns=0, axis_half_turns=Symbol(u'a')))
    eq.add_equality_group(
        cg.ExpWGate(half_turns=0, axis_half_turns=0.5),
        cg.ExpWGate(half_turns=0, axis_rads=np.pi / 2))
    eq.make_equality_group(
        lambda: cg.ExpWGate(
            half_turns=Symbol(u'ab'),
            axis_half_turns=Symbol(u'xy')))
...
def test_w_str():
    assert unicode(cg.ExpWGate()) == u'X'
    assert unicode(cg.ExpWGate(axis_half_turns=0.99999, half_turns=0.5)) == u'X^-0.5'
    assert unicode(cg.ExpWGate(axis_half_turns=0.5, half_turns=0.25)) == u'Y^0.25'
    assert unicode(cg.ExpWGate(axis_half_turns=0.25,
                           half_turns=0.5)) == u'W(0.25)^0.5'



def test_w_to_proto():
    assert proto_matches_text(
        cg.ExpWGate(half_turns=Symbol(u'k'),
    ...
    assert proto_matches_text(
        cg.ExpWGate(half_turns=0.5,
                    axis_half_turns=Symbol(u'j')).to_proto(GridQubit(2, 3)),
        u"""
        exp_w {
            target {
                row: 2
...
def test_w_potential_implementation():
    assert not cirq.can_cast(cirq.KnownMatrix,
                             cg.ExpWGate(half_turns=Symbol(u'a')))
...
def test_w_parameterize():
    parameterized_gate = cg.ExpWGate(half_turns=Symbol(u'a'),
                                     axis_half_turns=Symbol(u'b'))
    assert parameterized_gate.is_parameterized()
    with pytest.raises(ValueError):
        _ = parameterized_gate.matrix()
    resolver = ParamResolver({u'a': 0.1, u'b': 0.2})
    resolved_gate = parameterized_gate.with_parameters_resolved_by(resolver)
...
def test_trace_bound():
    ...
    assert cg.ExpZGate(half_turns=cirq.Symbol(u'a')).trace_distance_bound() >= 1
    assert cg.ExpWGate(half_turns=cirq.Symbol(u'a')).trace_distance_bound() >= 1


def test_has_inverse():
    assert cg.ExpZGate(half_turns=.1).has_inverse()
    assert cg.ExpWGate(half_turns=.1).has_inverse()
    assert not cg.ExpZGate(half_turns=cirq.Symbol(u'a')).has_inverse()
    assert not cg.ExpWGate(half_turns=cirq.Symbol(u'a')).has_inverse()


def test_measure_key_on():
    q = GridQubit(0, 0)

    assert cg.XmonMeasurementGate(key=u'').on(q) == cirq.GateOperation(
        gate=cg.XmonMeasurementGate(key=u''),
        qubits=(q,))
    assert cg.XmonMeasurementGate(key=u'a').on(q) == cirq.GateOperation(
        gate=cg.XmonMeasurementGate(key=u'a'),
        qubits=(q,))
...
def test_symbol_diagrams():
    ...
    c = cirq.Circuit.from_ops(
        cg.ExpWGate(axis_half_turns=cirq.Symbol(u'a'),
                             half_turns=cirq.Symbol(u'b')).on(q00),
        cg.ExpZGate(half_turns=cirq.Symbol(u'c')).on(q01),
        cg.Exp11Gate(half_turns=cirq.Symbol(u'd')).on(q00, q01),
    )
    assert c.to_text_diagram() == u"""
(0, 0): ───W(a)^b───@─────
                    │
(0, 1): ───Z^c──────@^d───
    """.strip()
...
def test_w_diagram_chars():
    ...
    c = cirq.Circuit.from_ops(
        ...
        cg.ExpWGate(axis_half_turns=cirq.Symbol(u'a')).on(q),
    )
    assert c.to_text_diagram() == u"""
(0, 1): ───X───W(0.25)───Y───W(a)───
    """.strip()
