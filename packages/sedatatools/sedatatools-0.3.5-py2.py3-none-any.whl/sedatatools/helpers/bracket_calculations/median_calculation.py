...
def calculate_median_from_brackets(brackets: pd.Series) -> int:
    ...
    cumulative_brackets = [sum([b for i2, b in enumerate(brackets) if i2 < i]) + el for i, el in enumerate(brackets)]
...
def calculate_mean_from_brackets(brackets: pd.Series) -> int:
    ...
    mean = round(sum(min_max_brackets_df['weight_count']) / sum(min_max_brackets_df['value_brackets'])).astype(int)
    return mean
