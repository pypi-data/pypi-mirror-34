...
def flags(args):
    ...
    if args.files:
        ...
        -f - Names CSV objects to convert. If you attempt to name the same object twice,
        ...
        csvs_to_convert = args.files
        csvs_in_bucket = converter.list_csv_objects_names()
        objs_new_names = args.names

        if len(csvs_to_convert) != len(objs_new_names):
        ...
        objs_to_convert = [obj for obj in csvs_to_convert if obj in csvs_in_bucket]
        
        for converted_obj, name in zip(objs_to_convert, objs_new_names):
            print("Now Converting: {} --> {}.parquet".format(converted_obj, name))
        
        converter.convert_selected_objects(objs_to_convert, objs_new_names)
