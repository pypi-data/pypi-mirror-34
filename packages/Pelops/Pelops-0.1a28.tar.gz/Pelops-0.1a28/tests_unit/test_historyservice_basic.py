...
class TestHistoryServiceBasic(unittest.TestCase):
    ...
    def test_00create(self):
        hs = HistoryService(self.config, 100, "/topic/test", False, self.update_available, self.mqtt_client,
                            self.logger)
    ...
    def test_06direct_avg_gaps(self):
        self.config["aggregator"] = "avg"
        hs = HistoryService(self.config, 10, "/topic/test", False, self.update_available, self.mqtt_client,
                            self.logger)
        self.assertIsNotNone(hs)
        hs.start()
        hs._aggregation_timestamp = 0
    ...
    def test_07mqtt_avg(self):
        ...
        hs = HistoryService(self.config, 10, "/topic/test", True, self.update_available, self.mqtt_client, self.logger)
        self.assertIsNotNone(hs)
        hs.start()
