...
def plot_mean_percentile_traj(tvec, data1, data2=None, edates=None, ntraj=0, 
                              ylim=None, ylabel=None, title=None, c1='r', c2='k'):
...
if DO_SMOOTH:
    ...
    while start < nsteps:
        ...
        fig = plot_mean_percentile_traj(
            tt, smps, ntraj=NTRAJ, edates=et, ylim=YLIM,
...
if DO_SMOOTH and DO_UNBIASED_SMOOTH:
    ...
    while start < nsteps:
        ...
        et = [t for t in events_dates if tt[0] <= t <= tt[-1]]
        fig = plot_mean_percentile_traj(
            tt, smps, unb_smps, edates=et, ylim=YLIM,
            # ylabel= r"$" + STATE_VAR + "$",
            ylabel="$\pi_{" + STATE_VAR + "|Y_{0:N}}$",
...
if DO_SMOOTH and DO_FILT:
    ...
    while start < nsteps:
        ...
        et = [t for t in events_dates if tt[0] <= t <= tt[-1]]
        fig = plot_mean_percentile_traj(
            tt, smps1, smps2, edates=et, ylim=YLIM,
            ylabel=r"$" + STATE_VAR + "$",
