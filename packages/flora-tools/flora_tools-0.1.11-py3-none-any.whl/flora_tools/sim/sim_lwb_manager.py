...
class SimLWBManager:
    ...
    def run(self):
        ...
        if self.next_slot is not None:
            self.node.em.register_event(self.next_slot['offset'], self.node, SimEventType.GENERIC, self.process_slot)
            return True
        else:
            return False
    ...
    def process_sync_slot(self, slot):
        if self.node.role is 'base':
            message = SimMessage(slot['modulation'], slot['offset'], self.node, slot['payload'],
                                 ...
                                 power_level=0)
            SimLWBSlot(self.node, slot, self.process_sync_slot_callback, tx_node=self.node,
                       message=message)
        else:
            SimLWBSlot(self.node, slot, self.process_sync_slot_callback, tx_node=self.node.current_base,
                       message=None)
    ...
    def process_slot_schedule_slot(self, slot):
        if self.node.role is 'base':
            ...
            SimLWBSlot(self.node, slot, self.process_slot_schedule_slot_callback, tx_node=self.node, message=message)
        else:
            SimLWBSlot(self.node, slot, self.process_slot_schedule_slot_callback, tx_node=self.node.current_base, message=None)
    ...
    def process_data_slot(self, slot, tx_node: 'sim_node.SimNode'):
        if tx_node is self.node:
            ...
            SimLWBSlot(self.node, slot, self.process_data_slot_callback, tx_node=self.node,
                       message=message)
    ...
    def process_contention_slot(self, slot):
        if not self.node.role is 'base':
            ...
            SimLWBSlot(self.node, slot, self.process_contention_slot_callback, tx_node=self.node,
                       message=message)
    ...
    def process_ack_slot(self, slot):
        if self.ack_operation:
            ...
            SimLWBSlot(self.node, slot, self.process_slot_schedule_slot_callback, tx_node=self.node,
                       message=message)
