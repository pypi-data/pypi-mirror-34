...
import requests
from watson_machine_learning_client.utils import get_url, MODEL_DETAILS_TYPE, DEFINITION_DETAILS_TYPE, EXPERIMENT_DETAILS_TYPE, INSTANCE_DETAILS_TYPE, FUNCTION_DETAILS_TYPE, load_model_from_directory, STR_TYPE, STR_TYPE_NAME, docstring_parameter, meta_props_str_conv, str_type_conv, is_python_2, get_file_from_cos
...
class Repository(WMLResource):
    ...
    _FunctionMetaNames = FunctionMetaNames()
    ...
    def store_model(self, model, meta_props=None, training_data=None, training_target=None, pipeline=None):
        ...
        # TODO should be removed, as already the keys are equal - verifying

        # if self.ModelMetaNames.FRAMEWORK_LIBRARIES in meta_props:
        #     meta_props[MetaNames.FRAMEWORK_LIBRARIES] = meta_props[self.ModelMetaNames.FRAMEWORK_LIBRARIES]

        # if self.ModelMetaNames.OUTPUT_DATA_SCHEMA in meta_props:
        #     meta_props[MetaNames.OUTPUT_DATA_SCHEMA] = meta_props[self.ModelMetaNames.OUTPUT_DATA_SCHEMA]

        # if self.ModelMetaNames.LABEL_FIELD in meta_props:
        #     meta_props[MetaNames.LABEL_FIELD] = meta_props[self.ModelMetaNames.LABEL_FIELD]

        # if self.ModelMetaNames.TRANSFORMED_LABEL_FIELD in meta_props:
        #     meta_props[MetaNames.TRANSFORMED_LABEL_FIELD] = meta_props[self.ModelMetaNames.TRANSFORMED_LABEL_FIELD]
    ...
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def _store_function(self, meta_props, function):
        """
            ...
            - generator function, which takes no argument and as result return 'score' function
            ...
            >>> stored_function_details = client.repository.store_function(name, filepath)
            ...
            >>>    client.repository.FunctionMetaNames.RUNTIME_URL: "https://ibm-watson-ml.mybluemix.net/v3/runtimes/53dc4cf1-252f-424b-b52d-5cdd9814987f",
            ...
            Then:
            >>> stored_function_details = client.repository.store_function(metadata, filepath)
            """
        ...
        if type(function) is STR_TYPE:
            ...
            try:
                ...
                fun_args = inspect.getargspec(function).args

                if function.__name__ is 'score':
                    print('Recognized \'score\' function.')
                    file_content = code
                elif fun_args is None or len(fun_args) == 0:
                    print('Recognized generator function.')
                    file_content = """
{}
...
""".format(code, function.__name__)
                    ...
                    raise WMLClientError("Function passed is not \'score\' function nor generator function. Generator function should have no arguments.")
        ...
        try:
            ...
            if self.FunctionMetaNames.TYPE in meta_props:
                function_metadata.update({u'type': meta_props[self.FunctionMetaNames.TYPE]})

            if self.FunctionMetaNames.TAGS in meta_props:
                function_metadata.update({u'tags': meta_props[self.FunctionMetaNames.TAGS]})

            if self.FunctionMetaNames.RUNTIME_URL in meta_props:
                function_metadata.update({u'runtime_url': meta_props[self.FunctionMetaNames.RUNTIME_URL]})
    ...
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def _delete_function(self, artifact_uid):
        """
    ...
    def get_details(self, artifact_uid=None):
        """
           Get metadata of stored artifacts. If uid is not specified returns all models, definitions, experiment and functions metadata.

           :param artifact_uid:  stored model, definition, experiment or function UID (optional)
           :type artifact_uid: {str_type}
    ...
    def get_model_details(self, model_uid=None, limit=None):
        ...
        if model_uid is not None:
            url = '/'.join(self._href_definitions.get_model_last_version_href(model_uid).split('/')[:-1])
            d = self._get_artifact_details(url, model_uid, limit, 'models')
            # print('D', d)
    ...
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def _get_function_details(self, function_uid=None, limit=None):
        """
    ...
    @staticmethod
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def _get_function_uid(function_details):
        """
    ...
    @staticmethod
    @docstring_parameter({'str_type': STR_TYPE_NAME})
    def _get_function_url(function_details):
        """
    ...
    def list(self):
        ...
        values = list(set(model_values + definition_values + experiment_values + function_values))
    ...
    def _list_functions(self, limit=None):
        """
