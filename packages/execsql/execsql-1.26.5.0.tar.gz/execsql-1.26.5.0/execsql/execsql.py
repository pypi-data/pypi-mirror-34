...
class SqlCmd(object):
	...
	def commandline(self):
		...
		return self.sql if self.command_type == "sql" else "-- !x! " + self.sql
...
class ScriptCommands(object):
	...
	def script_lines(self, script_id):
		...
		if s in self.scripts.keys():
			return [cmd.commandline() for cmd in self.scripts[s]]
		else:
			return []
	...
	def sub_exists(self, template_str):
		if subvars.sub_exists(template_str):
			return True
		return template_str.lower() in self.counters.keys()
		...
		for cmd_index, cmditem in enumerate(self.command_list):
			...
			while True:
				...
				if m:
					...
					cmditem.sql = cmditem.sql.replace(u'!!$'+m.group(1)+u'!!', unicode(new_count))
					match_found = True
				if not match_found:
					break
			self.cmds_run += 1
