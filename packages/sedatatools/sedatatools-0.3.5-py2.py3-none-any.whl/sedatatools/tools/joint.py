...
def checks_stats(statist):
    for i, stat in enumerate(statist):
        if stat[0] > 0 and stat[0] != statist[i - 1][0]:
            print(f'Some rows are lost between dfs {i} and {i-1}')


def write_stats(stat):
    for i, s in enumerate(stat):
        print(f'Table nr. {i+1}: {s}')
...
if __name__ == '__main__':
    ...
    if auto_keys:
        ...
        if not keys:
            print('Please add keys!')
            sys.exit()
    if len(keys) != len(file_names):
        pass
        # sys.exit('NUMBER OF KEYS MUST HAVE MATCHING NUMBER OF FILES!!!')
