...
# file module version (seperate from fpd version)
__version__ = '0.1.1'
_min_version = '0.1.0'
...
def _read_header_and_data(fp, fmt_ver, im_bytesize):
    '''
...
def binary_to_hdf5(binfns, hdrfn, dmfns=[], h5fn=None, detY=256, detX=256, 
                   ds_start_skip=0, row_end_skip=1, memmap=False, ow=False,
                   sort_binary_file_list=True, strict=True, embed_header_file=True,
                   embed_source=True, count_extra=True, chunks=None,
                   scan_chunks=(16, 16), im_chunks=(16, 16), compression_opts=4,
                   repack=True, scanYalu=None, scanXalu=None, *args, **kwargs):
    '''
    
    Convert the data produced by the Merlin Medipix 3 readout system
    into an hdf5 file composed of EMD [1] data sets.
    ...
    detY : int
        Detector pixels in Y.
    detX : int
        Detector pixels in X.
    memmap : bool
        If True, binary files are memory mapped.
    ow : bool
    ...
    chuncked file. The copying is done by only loading one chunk at a time, so
    this is RAM efficient but diskspace expensive. Increasing chuncking in this
    way can significantly reduced the final filesize and improve processing
    speed.
    ...
    _logger.warn('This function is deprecated. Please use the `MerlinBinary` class instead.')
    
    #TODO
    #test emd file (GUI is very buggy)
    #allow user supplied dict in kwargs? 
    
    #aperture?
        #from corrector?
    #corrector settings?
    #free lens control settings?
    ...
    if type(binfns) in _str_type_list:
        binfns = [binfns]
    else:
        if sort_binary_file_list:
            m = "Sorting binfns by frame number, deactivite sorting by 'setting sort_binary_file_list=False'."
            _logger.info(m)
            binfns = _sort_multiple_binary_file_input(binfns)
    # if dmfns is string, convert to list for looping over 1 or more DM files.
    if type(dmfns) in _str_type_list:
        dmfns = [dmfns]
    
    # parse header file
    Mhfp = Merlin_hdr_file_parser(hdrfn, strict=strict)
    # parse first binary header
    with open(binfns[0], "rb") as f:
        Mbhp = Merlin_binary_header_parser(f, Mhfp.fmt_ver)
    ...
    def _image_array_properties(detY, detX, Mbhp):
        '''
        Determine image array properties from 
        '''
    ...
    nbytes, det_dtype, im_bytesize = _image_array_properties(detY, detX, Mbhp)
    
    # Determine scanX/Y and their axes and units.
    # If non None, use these over those in DM file
    if len(binfns) == 1 and scanXalu is None and scanYalu is None and len(dmfns) == 0:
        ...
        if row_end_skip != 0:
            _logger.info("Do you really want to set 'row_end_skip' to %d?" %(row_end_skip))
        if ds_start_skip != 0:
            _logger.info("Do you really want to set 'ds_start_skip' to %d?" %(ds_start_skip))
        # This will fail in the unlikely case of the the header size
        # changing mid file, but should work in most cases.
        nframes = int(os.path.getsize(binfns[0]) / (Mbhp.header_bytesize +
                                                    im_bytesize))
        scanXalu = (nframes, 'xaxis', 'pixels')
        scanYalu = (1, 'yaxis', 'pixels')
    elif scanYalu is None or scanXalu is None or (
            scanXalu is not None and scanXalu[:2]==(None, None)):
        # must use DM file
        pauX, pauY, tagd, dm_im, dm_bin = _parse_dm_file(dmfns[0])
    
    if scanYalu is not None:
        scanYaxis, scanYlabel, scanYunits = scanYalu
    ...
    if scanXalu is not None:
        scanXaxis, scanXlabel, scanXunits = scanXalu
        if scanXaxis is None:
            # generate 'scan' size and an index axis from header
            scanXaxis = int(Mhfp.hd['frames_in_acquisition'])
    ...
    scanXbindata = scanX+row_end_skip    
    
    # FPD dataset axes details
    # TODO specify colour and detector axes and units
    # could be custom image tag in DM.
    Caxis = [x+1 for x in range(colourN)]
    dim_axes = [scanYaxis, scanXaxis, Caxis, list(range(detY)), list(range(detX))]
    dim_names = [scanYlabel, scanXlabel] + 'colour, detY, detX'.split(', ')
    dim_units = [scanYunits, scanXunits, 'colour_index', '', '']
    ...
    if not Mhfp.CM:
        ...
        del dim_units[2]
        pass
    dim_lens = [len(t) for t in dim_axes]
    _logger.info('FPD data shape: ' + str(dim_lens))
    
    # set chunking
    if chunks is not None:
        # use chunks directly
        if not len(chunks) == len(dim_lens):
            message = "'chunks' length (%d) does not match data length (%d)." %(len(chunks), len(dim_lens))
            raise Exception(message)
    else:
        # check for lower precedence chunking parameters
        chunks = scan_chunks + (1,)*(len(dim_lens)-4) + im_chunks
    
    # coerce chunks to be <= shape
    chunks = tuple([min(chk, shp) for chk, shp in zip(chunks, dim_lens)])
    _logger.info('FPD data chunking: ' + str(chunks))
    
    # condition filename
    if h5fn is None:
        h5fn = os.path.splitext(binfns[0])[0]+'.hdf5'
    ...
    if repack:
        ...
        h5f_tmp_fpd = h5f_tmp.create_dataset("fpd_data", shape=dim_lens, dtype=det_dtype)
    else:
        # warn about chunking
    ...
    with h5py.File(h5fn, 'w', driver=None) as h5f:
        ...
        if embed_header_file:
            _embed_header_in_hdf5_group(fpd_expt_grp, Mhfp)
        
        # sum_im (sum of all detector signals at each scan pixel)
        det_pix_max_val = 2**Mbhp.bitdepth_data-1
        ...
        # Write DM files
        _write_dm_files_to_hdf5(fpd_expt_grp, emd_ds_default, dmfns,
                                dim_lens, dim_names, dim_axes, dim_units)
        ...
        fpd_dataset = _create_fpd_data_hdf5_dataset(fpd_expt_grp, emd_ds_default, 
                                                    det_dtype, dim_lens, dim_names, 
                                                    dim_axes, dim_units, chunks, 
                                                    compression_opts)
        
        # Binary header params
        bhd_dss = _create_bin_header_param_hdf5_dataset(fpd_expt_grp, emd_ds_default, 
                                                        det_pix_max_val, dim_lens, 
                                                        dim_names, dim_axes, 
                                                        dim_units, Mbhp.params)
        
        # data and image dimensions / size
        ims_shape = dim_lens[:-2]
        total_no_ims = np.prod(ims_shape)
        
        # (scanY, scanXbindata[, colourN])
        ims_inc_extra_shape = ims_shape[:]
        ...
        stop = False
        pbar = tqdm(total=total_no_ims, mininterval=0, leave=True, unit='images')
        for binfn_index, binfn in enumerate(binfns):
            if stop == True:
            ...
            with open(binfn, "rb") as f:
                ...
                if memmap:
                    ...
                    fp = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
                
                # create partial for looping
                read_binary_func = partial(_read_header_and_data, fp, Mhfp.fmt_ver, im_bytesize)
                ...
                for (hdr_str, hdr_params, data_buffer) in bin_file_iter:
                    if j<ds_start_skip:
                        ...
                        try:
                            ...
                            if row_end_skip and scanXind>scanX-1:
                                i += 1
                                # skip row_end_skip
                                pass
                            else:
                                # process bin_data
                                bitdepth_data = Mbhp.bitdepth_data
                                bitdepth_bin = Mbhp.bitdepth_bin
                                raw_mode = Mbhp.raw_mode
                                
                                im_array = _raw_merlin_to_array(data_buffer, det_dtype, 
                                                                bitdepth_data, bitdepth_bin,
                                                                raw_mode, detY, detX)
                                
                                # sum_im image
                                fpd_sum_im_dataset[s] = im_array.sum()
                                # sum_dif image
                                if Mhfp.CM:
                                    fpd_sum_dif_dataset[sYi_sXi_opCi[2:],:,:] += im_array
                                else:
                                    fpd_sum_dif_dataset[:,:] += im_array
                                # fpd bin_data
                                if repack:
                            ...
                            if e1 or e2:
                                if count_extra:
                                    if len(binfns)==1:
                                        ...
                                        # multiple files
                                        no_extra_images = len(binfns) - (binfn_index+1)
                    ...
                    j+=1
        if k+1 < total_no_ims:
            m = 'Only %d of expected %d images read!' %(k+1, total_no_ims)
            _logger.warning(m)
        pbar.close()
...
class MerlinBinary:
    def __init__(self, binfns, hdrfn, dmfns=[], detY=256, detX=256,
                 ds_start_skip=0, row_end_skip=1, scanYalu=None, scanXalu=None,
                 sort_binary_file_list=True, strict=True, memmap=False,
                 *args, **kwargs):
        '''
    ...
    def _determine_axes(self):
        ...
        if len(self._binfns) == 1 and self._scanXalu is None and self._scanYalu is None and len(self._dmfns) == 0:
            ...
            self._scanYalu = (1, 'yaxis', 'pixels')
                ...
                self._scanXalu is not None and self._scanXalu[:2]==(None, None)):
            # must use DM file
        ...
        self._dim_axes = [scanYaxis, scanXaxis, self._Caxis, list(range(self._detY)), list(range(self._detX))]
        self._dim_names = [scanYlabel, scanXlabel] + 'colour, detY, detX'.split(', ')
        self._dim_units = [scanYunits, scanXunits, 'colour_index', '', '']
    ...
    def write_hdf5(self, h5fn=None, chunks=None, scan_chunks=(16, 16), im_chunks=(16, 16),
                   ...
                   embed_source=False, count_extra=True, mask=None):
        '''
        ...
        the fpd data is written to a temporary file, then copied to the compressed /
        chuncked file. The copying is done by only loading one chunk at a time, so
        this is RAM efficient but disk space expensive. Increasing chuncking in this
        ...
        memmap_possible = self._memmap_check(throw_error=False)
        if memmap_possible:
        ...
        with h5py.File(h5fn, 'w', driver=None) as h5f:
                                ...
                                if self._row_end_skip and scanXind > self._scanX-1:
                                    ...
                                    if mask is not None:
                                        CI.interpolate_image(image=im_array, 
                                                             ...
                                                             in_place=True)
                                        # this is repeated if memmapping file
                                        # but it is relatively fast, so ok for now
            ...
            if memmap_possible:
                ...
                if mask is None:
                    ...
                    # loop over chunks only in non-image axes
    ...
    def __getitem__(self, key):
        ...
        k = 0   # index of saved data
        stop = False
        pbar = tqdm(total=np.prod(index_shape[:-2]), mininterval=0, leave=True, unit='images')
        if _p3:
...
def fpd_to_hysperspy(fpg, fpd_check=True, assume_yes=False, group_names=None):
    ...
    if hs_non_lazy:
        ...
        titles = [d['original_metadata']['General']['title'] for d in ds]      
        indices = _check_titles(titles, group_names, hs_non_lazy)
        titles_selection = [titles[i] for i in indices]
...
class DataBrowser:
    ...
    def plot_nav_im(self):
        self.f_nav, ax = plt.subplots(subplot_kw=dict(adjustable='box-forced', aspect='equal'))
        self.f_nav.canvas.set_window_title('nav')

        im = ax.imshow(self.nav_im, interpolation='nearest', cmap='gray')
        
        rect_c = 'r'
        if self.nav_im.ndim==3: rect_c = 'w'    #'k'
    ...
    def plot_dif(self):
        self.f_dif, ax = plt.subplots(subplot_kw=dict(adjustable='box-forced',
                                                      aspect='equal'))
