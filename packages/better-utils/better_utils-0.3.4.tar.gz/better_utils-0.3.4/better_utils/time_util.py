...
class TimeUtil:
    ...
    def str_to_str(s, p):
        ...
        :return:
        '''
        return time.strftime(p, time.localtime(TimeUtils.str_to_unix(s)))
    ...
    def getMonthFirstAndLastDay(s):
        ...
        :return:
        '''
        year = int(TimeUtils.str_to_str(s, "%Y"))
        month = int(TimeUtils.str_to_str(s, "%m"))
        weekDay, monthCountDay = calendar.monthrange(year, month)
        firstDay = datetime.date(year, month, day=1)
        lastDay = datetime.date(year, month, day=monthCountDay)
        firstDay = TimeUtils.str_to_str(str(firstDay), "%Y%m%d")
        lastDay = TimeUtils.str_to_str(str(lastDay), "%Y%m%d")
    ...
    def getWeekFirstAndLastDay(s):
        ...
        :return:
        '''
        d = datetime.datetime.strptime(TimeUtils.str_to_str(s, "%Y%m%d"), "%Y%m%d")
        firstDay = str(d - datetime.timedelta(days=d.weekday()))
        lastDay = str(d + datetime.timedelta(days=6 - d.weekday()))
        firstDay = TimeUtils.str_to_str(firstDay, "%Y%m%d")
        lastDay = TimeUtils.str_to_str(lastDay, "%Y%m%d")
        return firstDay, lastDay

    @staticmethod
    def now(p="%Y-%m-%d %H:%M:%S"):
        return TimeUtils.unix_to_str(TimeUtils.now_unix(), p)
