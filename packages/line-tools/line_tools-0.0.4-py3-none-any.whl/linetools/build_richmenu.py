...
def do_new(user: str, config: Dict[str, str]):
    ...
    if resp.status != 200:
        raise Exception("Error creating menu: {0}\n{1}".format(resp.reason, resp.data))
    else:
    ...
    if resp.status != 200:
        raise Exception("Error uploading menu image {0}: {1}\n{2}".format(image, resp.reason, resp.data))
    else:
...
def process(the_args):
    try:
        ...
        if 'verbose' in the_args and the_args.verbose is True:
...
def main():
    ...
    base_parser.add_argument('--verbose', action='store_true', help='Print debugging information')
