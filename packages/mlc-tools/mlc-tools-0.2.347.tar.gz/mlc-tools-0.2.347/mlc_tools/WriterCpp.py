...
class WriterCpp(Writer):
    ...
    def _write_class_hpp(self, class_):
        ...
        if not class_.is_changed:
            out[FLAG_HPP] = self.parser.hash.get_class_body(class_, '.h')
            return out
        ...
        self.parser.hash.fix_class_body(class_, out[FLAG_HPP], '.h')
        return out

    def _write_class_cpp(self, class_):
        out = Writer.write_class(self, class_, FLAG_CPP)
        if not class_.is_changed:
            out[FLAG_CPP] = self.parser.hash.get_class_body(class_, '.cpp')
            return out
        ...
        self.parser.hash.fix_class_body(class_, out[FLAG_CPP], '.cpp')
        return out
    ...
    def add_methods(self, class_):
        ...
        if class_.is_test or class_.name.startswith('ITest'):
            return class_
