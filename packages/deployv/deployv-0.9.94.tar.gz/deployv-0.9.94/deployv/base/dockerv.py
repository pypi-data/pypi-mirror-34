...
class DockerV(object):
    ...
    def setting_default_ports(self):
        """Setting ports by default. Searching all ports in use to filter with the ports
        ...
        ports = [key for key, val in self.__config.get('ports').items() if not val]
        netstat = psutil.net_connections()
        used_ports = [
            nt.laddr.port for nt in netstat if nt.status in ['LISTEN', 'ESTABLISHED']]
        available_ports = [p for p in range(port_start, port_end) if p not in used_ports]
        if not ports or not available_ports:
            return
        self.__config.get('ports').update(dict(zip(ports, available_ports)))
