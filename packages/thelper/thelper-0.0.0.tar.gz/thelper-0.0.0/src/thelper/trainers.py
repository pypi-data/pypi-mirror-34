...
trainers_logger = logging.getLogger("thelper.trainers")
...
class BaseTrainer:
    ...
    def __init__(self,model,loss,metrics,resume,config,train_logger=None):
        ...
        self.optimizer = getattr(torch.optim,config['optimizer_type'])(model.parameters(),**config['optimizer'])
        self.lr_scheduler = getattr(torch.optim.lr_scheduler,config['lr_scheduler_type'],None)
