...
class TestDeployment(unittest.TestCase):
    ...
    def test_01_create_deployment_uid(self):
        TestDeployment.deployment_details = self.client.deployments.create(asset_uid=self.model_uid, name="Deployment test PLP")
    ...
    def test_01_create_deployment_invalid_uid(self):
        with self.assertRaises(WMLClientError) as context:
            self.client.deployments.create(asset_uid="sadrjfdjsf899ssksksss", name="Deployment test PLP")

        self.assertIn("Invalid uid", str(context.exception))

    def test_01_create_deployment_empty_uid(self):
        with self.assertRaises(WMLClientError) as context:
            self.client.deployments.create(asset_uid="", name="Deployment test PLP")
    ...
    def test_01_create_deployment_empty_name(self):
        with self.assertRaises(WMLClientError) as context:
            self.client.deployments.create(asset_uid=self.model_uid, name="")

        self.assertIn("Invalid", str(context.exception))

    def test_01_create_deployment_none_name(self):
        with self.assertRaises(WMLClientError) as context:
            self.client.deployments.create(asset_uid=self.model_uid, name=None)
    ...
    def test_01_create_deployment_no_name(self):
        with self.assertRaises(TypeError):
            self.client.deployments.create(asset_uid=self.model_uid)
