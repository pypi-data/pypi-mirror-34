...
def app_request_ended(sender, response, **extra):
    if request.endpoint == 'write_log' and 200 <= response.status_code < 300:
        return
    log_request_ended(response)


def app_request_exception(sender, exception, **extra):
    log_request_failed(exception)
...
def log_request_ended(response):
...
def log_request_failed(exception):
    if not hasattr(request, 'logged'):
        status = exception.code if isinstance(exception, HTTPException) else 500
        logger.info(msg=_get_log_message(str(status)),
                    extra={"colony.request.url": request.url,
...
def _get_log_message(http_status: str):
    ...
    return "Response {0} ({1:.3f} sec) - {2}".format(http_status, took, func_str or request.url)
