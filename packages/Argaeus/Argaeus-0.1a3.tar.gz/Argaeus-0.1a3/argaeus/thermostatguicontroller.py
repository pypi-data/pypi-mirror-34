...
class ThermostatGUIController(AbstractMicroservice):
    ...
    def __init__(self, config, mqtt_client=None, logger=None):
        ...
        self._button_controller = BehaviorController(self._config["button-controller"], self._mqtt_client,
                                                     self._logger)
        self._mode_controller = ModeController(self._button_controller, self._config["mode-controller"],
                                               self._mqtt_client, self._logger)
        self._set_point_controller = SetPointController(self._button_controller, self._mode_controller,
                                                        self._config["setpoint-controller"],
                                                        self._mqtt_client, self._logger)
        self._operation_controller = OperationController(self._button_controller,
                                                         self._config["operation-controller"], self._mqtt_client,
                                                         self._logger)
