...
class Verse:
    ...
    def addDefaultEnv(self):
        ...
        process = subprocess.Popen(['mkdir', '-p', os.path.join(self.rootdir, 'cache/aliases')])
        process.wait()
...
def create():
    ...
    if not os.path.exists('./verse/scripts'):
        os.mkdir('./verse/scripts')

    with open('./verse/Versefile', 'w') as the_file:
        the_file.write("NAME\n\n"
                       "BACK 162 35 173\n"
                       "FORE 255 255 255\n\n"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == 'run':
            ...
            try:
                create()
                print('Successfully created Versefile in ./verse/Versefile')
            except Exception as e:
                print('error: ' + str(e))
