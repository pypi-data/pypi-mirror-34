...
class PumpingStationMixin(OptimizationProblem):
    ...
    def post(self):
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ts = self.get_timeseries(p.energy_price_symbol)
                price = self.interpolate(self.times()[1:], ts.times, ts.values)

                pump_on = self.__additional_results[p.symbol + "_status"][1:] == 1.0
        ...
        if any_pump_on:
            for g, obj_val in self._pump_minimization_function_values:
                if isinstance(g, MinimizePumpCostGoal) and not any_costs_made:
                    continue
                elif obj_val > 100 or obj_val < 0.01:
        ...
        assert len([isinstance(g, _MinimizePumpGoalType) for g in self.goals()]) == 1
        for ps in self.pumping_stations():
            for p in ps.pumps():
            ...
            for ps in self.pumping_stations():
                for p in ps.pumps():
                    ...
                    power_ts = Timeseries(times, power_calculated)
