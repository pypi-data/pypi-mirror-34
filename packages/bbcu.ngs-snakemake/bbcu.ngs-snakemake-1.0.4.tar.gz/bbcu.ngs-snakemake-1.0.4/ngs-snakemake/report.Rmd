---
title: "Report PIPELINE_TYPE Pipeline: `r basename(dirname(dirname(getwd())))`"
...
ANNOTAT_TYPE
deseq_eval <- TRUE
htseq-count (DOI: \[10.1093\/bioinformatics\/btu638](http:\/\/dx.doi.org\/10.1093\/bioinformatics\/btu638)) (intersection-strict mode???????)
INTERMINE_WEB_QUERY
INTERMINE_WEB_BASE
INTERMINE_CREATURE
INPUT_FOLDER
...
Histogram showing the percent of reads discarded after trimming the adapters (after the removing of the adapters some read and polyA/T or low quality reads may be too short and the pipeline discards them). Median, mean and std of too short discarded of all samples were", round(median_discarded), round(mean_discarded), round(std_discarded), "reads.
...
The p-adjusted values will be corrected by fdrtool (R package, DOI: [10.1186/1471-2105-9-303](https://doi.org/10.1186/1471-2105-9-303)) if one of the following applies:

- The pipeline compute the fractions of the p-values in range between [0.25,1) for the following bins: (0.25,0.5], (0.5,0.75], (0.75,1), and the fractions of the p-values in one of the bins is not between 0.28 to 0.38.

- the fraction of the p-values in the range [0,0.25] is lower from the fraction of the p-values of the range (0.25,1) by 0.05.
...
")
```
Commands log can be downloaded from <a href=../commands_log.txt download>here</a>.
