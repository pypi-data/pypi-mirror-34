...
class WordItem(object):
    ...
    def __init__(self, word):
        ...
        self.uk_sound = None
        self.us_sound = None
        self.uk_phonetics = None
        self.us_phonetics = None
        self.star = ""
    ...
    def __str__(self):
        return "%s %s %s\n%s\n%s\n%s" % (
            self.vocabulary, self.uk_phonetics, self.us_phonetics, self.uk_sound, self.us_sound, self.description)
...
class MacmillanService(DictService):
    ...
    def _analysis(self, word, out):
        ...
        uk_phonetic_pattern = re.compile(
            r"<font color=\"#ff5400\">UK</font>\s<a href=\"(?P<uk_sound>[^\"]+)\">"
        ...
        if matcher is not None:
            ...
            word_item.uk_phonetics = matcher.group("uk_phonetic")
        ...
        # # 测试提取us音标
        us_phonetic_pattern = re.compile(r"<font color=\"#ff5400\">US</font>\s<a href=\"(?P<us_sound>[^\"]+)\">")
        ...
        if matcher is not None:
            ...
            if us_phonetic_pattern is not None:
                word_item.us_phonetics = us_phonetic_pattern.group("us_phonetic")
        ...
        if matcher is not None:
            ...
            result_dict["star"] += 1
            word_item.star = matcher.group("star")
        else:
        ...
        for key, items in desc_items.items():
            ...
            for in_item in items:
                word_item.description_html += (
                        "&nbsp;&nbsp;&nbsp;&nbsp;<font color=\"#b904af\"><b>%d.</b></font> " % index + in_item + "<br>")
                index += 1
        # 记录主要解释信息，取第一条
        ...
        # 如果美式音标为空，则美式与英文一致
        if word_item.us_phonetics is None:
            word_item.us_phonetics = word_item.uk_phonetics
        return word_item
    ...
    def get_write_data(self, name):
        ...
        if item is None:
            return list(map(lambda i: "", range(0, 8)))
        else:
            # 添加单词，音标
            items = [item.word, item.uk_phonetics, item.us_phonetics]
            # 添加音频
            if item.uk_sound is not None:
                items.append("[sound:%s]" % item.uk_sound)
            if item.us_sound is not None:
                items.append("[sound:%s]" % item.us_sound)
            # 添加星级，英文简介，所有简介
            items.extend([item.star, item.description, item.description_html])
            return items
