...
def gds2img(n, m, lib, decenter=True):
    """
    ...
    """
    return bnds2img(n, m, gds2bnds(lib), decenter)
...
def bnds2img(n, m, bnds, decenter=True):
    """
    ...
    for bnd in bnds:
        ...
        path = bnd.xy
        if decenter:
            path = [(item[0] + n / 2, item[1] + m / 2) for item in path]
        # Paint on the canvas.
        img_elem = Image.new('L', (n, m), 0)
        ImageDraw.Draw(img_elem).polygon(path, outline=1, fill=1)
        img_elem = np.array(img_elem)
        img[img_elem != 0] = (np.array(img_elem[img_elem != 0]) * (bnd.data_type / 10000.0 * 255))
...
def img2gds(image, levels=None, min_area=None, tolerance=None, center=False, scale=None, smooth=None):
    """
    ...
    lib.append(
        img2struct('img2gds', image, levels=levels, min_area=min_area, tolerance=tolerance, center=center, scale=scale,
                   smooth=smooth))
    return lib


def img2struct(name, image, levels=None, min_area=None, tolerance=None, center=False, scale=None, smooth=None):
    """
    Convert an image to a gds library.
    :param image: image to convert
    :param levels: number of threshold levels (minimum 2, i.e. black and white)
    ...
    for bnd in img2bnds(image, levels, min_area, tolerance, center, scale, smooth):
        struct.append(bnd)
    return struct


def img2bnds(image, levels=None, min_area=None, tolerance=None, center=False, scale=None, smooth=None):
    """
    ...
    """
    return polys2bnds(img2polys(image, levels, min_area, tolerance, center, scale, smooth))
...
def polys2bnds(polygon_lists):
    ...
    for i, polygons in enumerate(polygon_lists):
        ...
        for polygon in polygons:
            ...
            bnds_i.append(Boundary(i, int(value_i * 10000), bnd_i))
...
def assemble_polygons(polygons):
    """
    From a number of (potentially overlapping) polygons and holes, a single polygon is assembled.
