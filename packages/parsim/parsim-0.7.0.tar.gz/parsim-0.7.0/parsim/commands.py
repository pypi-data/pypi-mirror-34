...
def doe(argv=None):
    ...
    doe_args = {}
    try:
        for a in args.doe_args:
            exec(a, globals(), doe_args)
    except (SyntaxError, NameError):
        print('\nERROR processing DOE keyword arguments!\n')
        raise
    assert len(args.doe_args) == len(doe_args), 'Unexpected entries in doe_args: %s' % (repr(doe_args))
