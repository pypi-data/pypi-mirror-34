...
def add_job(job_info):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/js-service/", json=request)
        r.raise_for_status()
        return r.json()
...
def reschedule(job_info):
    ...
    try:
        r = requests.put(f"{ficusServerGetter.server_url}remote/js-service/", json=request)
        r.raise_for_status()
        return r.json()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
...
def remove(job_id):
    ...
    try:
        r = requests.delete(f"{ficusServerGetter.server_url}remote/js-service/{job_id}")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def pause(job_id):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/js-service/{job_id}/pause")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def resume(job_id):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/js-service/{job_id}/resume")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def stop(job_id):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/js-service/{job_id}/stop")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def trigger_job(job_id, params=None):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/js-service/{job_id}/trigger", json=params)
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def list_jobs():
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/js-service")
        r.raise_for_status()
        if len(r.content) > 0:
            ll = r.json()
...
def get_jobs(actor_name):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/js-service/{actor_name}")
        r.raise_for_status()
        if len(r.content) > 0:
            ll = r.json()
            if isinstance(ll, list):
...
def find_by_site_and_code(site, project_code, code):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/js-service/{site}/{project_code}/{code}")
        r.raise_for_status()
        if len(r.content) > 0:
            return Munch(r.json())
        else:
