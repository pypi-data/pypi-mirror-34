...
class SimpleRpmBuild(RpmBuild):
    ...
    def get_built_rpm(self, arch, name=None):
        ...
        builtRpmName="%s-%s-%s.%s.rpm"%(name, self.version, self.release, archSuffix)
        if arch=="SRPMS":
    ...
    def add_simple_payload_file_random(self, size=100):
        ...
        name = "%s-%s-%s-%s-%s-%s.txt" % (self.epoch, self.name, self.version, self.release, self.get_build_archs()[0], len(self.sources))
