...
formatter_dict = {
    ...
    4: logging.Formatter('%(asctime)s - %(name)s - "%(filename)s" - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s -  File "%(pathname)s", line %(lineno)d, in %(funcName)s ', "%Y-%m-%d %H:%M:%S"), # 这个也支持日志跳转
}
...
class ColorHandler(logging.Handler):
    ...
    def __init__(self, filter_logger_name_list=None):
        ...
        self.formatter_new = logging.Formatter(
            '%(asctime)s - %(name)s - %(filename)s - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s                    \033[0;43mFile "%(pathname)s", line %(lineno)d, in %(funcName)s\033[0m',
            "%Y-%m-%d %H:%M:%S")
        self.formatter_new = logging.Formatter(
            '%(asctime)s - %(name)s - "%(filename)s" - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s              \033[7mFile "%(pathname)s", line %(lineno)d, in %(funcName)s\033[0m',
            "%Y-%m-%d %H:%M:%S")
