...
conn = pyodbc.connect(driver="{SQL Server}", server=settings['s'], database=settings['db'], uid=settings['u'],
                      pwd=settings['p'])
curs = conn.cursor()
...
def get_info_from_database(project_id):
    tables_with_types = {}
    curs.execute("select name from sys.tables where name like '" + project_id + "%' order by 1;")
...
def get_info_from_metadata(project_id):
    doc = ET.parse('C:/Projects/Website-ASP.NET/pub/ReportData/Metadata/' + project_id + '.xml')
    ...
    for key, values in db_formats.items():
        ...
        for k, v in values.items():
            if k in metadata_formats[key[-table_suffix_cutoff:]].keys():
                if checkDataType(metadata_formats[key[-table_suffix_cutoff:]][k]) != v:
                    differences.append([key, k, v, 'should be:', checkDataType(metadata_formats[key[-table_suffix_cutoff:]][k])])
...
if __name__ == '__main__':
    db_formats = get_info_from_database(settings['project_id'])
