...
class CNN:
    ...
    def prepare_y(self, label_array, class_indices, num_classes):
        """Prepares y for a keras model.(ex. y_train)
        
        Arguments:
            label_array {ndarray} -- An array of labels.
            class_indices {dict} --
                A dictionary of labels to classes.
    ...
    def train(self,
        ...
        (ex. number of unique labels(classes) in the label_array or directory)
        ...
        if self.is_image_data == True:
            ...
            if type(self.model) == type(None):
                unique_labels, num_classes = numpy.lib.arraysetops.unique(self.label_array, return_counts = True)
                ...
                num_classes = len(self.class_dictionary)
            y_train = self.prepare_y(self.label_array, reverse_dict(self.class_dictionary), num_classes)
    ...
    def train_with_images(self, train_datagen, batch_size, steps_per_epoch, epochs, verbose, callbacks, shuffle, use_fit_generator):
        if use_fit_generator == False:
            if self.get_image_from == 'directory':
                images, labels = directory_images_to_arrays(self.data_path, self.input_shape[:2])
                self.prepare_train_data(is_image_data = True, get_image_from='argument', data_array=images, label_array=labels)
            if type(self.model) == type(None):
                unique_labels, num_classes = numpy.lib.arraysetops.unique(self.label_array, return_counts = True)
                class_indices = self.create_class_indices(unique_labels)
                self.init_class_dictionary_and_model(class_indices, num_classes)
    ...
    def evaluate(self, data_array, label_array, is_image_data=True):
        ...
        Returns:
            [list] -- Test loss and test accuracy will be returned as a list.
        """
        try:
            if data_array.shape[1:] != self.input_shape:
