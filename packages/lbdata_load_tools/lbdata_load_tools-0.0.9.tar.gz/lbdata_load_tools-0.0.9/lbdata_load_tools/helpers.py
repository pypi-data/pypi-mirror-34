...
def create_staging_table(cur, table_src, table_stage, exclude):
    """
    Create staging table exactly similar to the specified table_name
    """
    cur.execute('create temp table %s (like %s);', (AsIs(table_stage), AsIs(table_src), ))
    # Drop columns in exclude array
    for c in exclude:
        cur.execute('alter table %s drop column %s;', (AsIs(table_stage), AsIs(c), ))
...
@contextmanager
def staging_table(cur, table_src, table_stage, exclude=[]):
    """
    Provides a context manager for using staging tables and be sure that they will correctly be dropped
    """
    create_staging_table(cur, table_src, table_stage, exclude)
    try:
        yield
    finally:
        drop_table(cur, table_stage)
