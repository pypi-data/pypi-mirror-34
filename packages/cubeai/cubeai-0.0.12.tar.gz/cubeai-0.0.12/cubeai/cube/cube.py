...
class Cube:
    ...
    def __str__(self):
        ...
        result += '\n'
        for i in range(n):
            result += row(self._array[i, :, Face.LEFT.value]) + ' ' + \
                row(self._array[i, :, Face.FRONT.value]) + ' ' + \
                row(self._array[i, :, Face.RIGHT.value]) + ' ' + \
...
def convert_real_cube_3x3(front, right, left, back, up, down):
    ...
    """
    cube = Cube()
    cube.set_face(Face.FRONT, face_convert_str(front))
    cube.set_face(Face.RIGHT, face_convert_str(right))
    cube.set_face(Face.LEFT, face_convert_str(left))
    cube.set_face(Face.BACK, face_convert_str(back))
    cube.set_face(Face.UP, face_convert_str(up))
    cube.set_face(Face.DOWN, face_convert_str(down))
    return cube
