    ...
    def instr_pos_to_line_column(self, pos):
        ...
        while t < pos:
            if self.instr[t] == "\r":
                ...
                line_start = t
            t += 1
        return (line, pos - line_start + 1, pos)

    def to_html(self, instr, outfile = None, *, outer_pos = 0):
            ...
            if ch == "‘":
                ...
                if prevc in 'PР': # Рисунок обрабатывается по-особенному
                    ...
                    if (next_char() == "\n" # чтобы переход на новую строку/перевод строки после `}` игнорировался
                            and not in_unordered_list and ordered_list_current_number == None): # если находимся внутри списка, то пропуска новой строки делать не нужно
                        i += 1
                        writepos = i + 1
                    ...
                    if prev_char() in '*_-~':
                        ...
                        if re.match(r"#[0-9абсдефАБСДЕФabcdefABCDEF]{6}$", str_in_b):
                            def c(c):
                                r = 'ABCDEF'['абсдеф'.index(c.group().lower())]
                                return r.lower() if c.group().islower() else r
                            str_in_b = re.sub("[абсдефАБСДЕФ]", c, str_in_b)
                        elif re.match(r"[0-8]{3}|[0-8]$", str_in_b):
                            new_str = "#"
                            for ii in range(3) if len(str_in_b) == 3 else (0, 0, 0):
...
def to_html(instr, outfile = None, ohd = 0, *, habrahabr_html = False):
    return Converter(habrahabr_html, ohd).to_html(instr, outfile)
