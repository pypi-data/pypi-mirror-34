...
class TextDiagramDrawer(object):
    ...
    def transpose(self):
        ...
        out.entries = dict(((y, x), v) for (x, y), v in self.entries.items())
    ...
    def render(self,
        ...
        pipe = u'│' if use_unicode_characters else u'|'
        dash = u'─' if use_unicode_characters else u'-'
        if crossing_char is None:
            crossing_char = u'┼' if use_unicode_characters else u'+'

        emp_pipe = u'┃' if use_unicode_characters else u'#'
        emp_dash = u'━' if use_unicode_characters else u'#'
        ...
        extend_char = [[u' '] * w for _ in xrange(h)]
        ...
        for col in xrange(w):
            col_width = max(1, max(len(grid[y][col]) for y in xrange(h)))
            for row in xrange(h):
                missing = col_width - len(grid[row][col])
                grid[row][col] += extend_char[row][col] * missing

        return u'\n'.join(u''.join(row).rstrip() for row in grid)
