...
class MldMcastIPv6GroupList(Base):
	...
	def MldUcastIPv6SourceList(self):
		"""Returns the one and only one MldUcastIPv6SourceList object from the server.
		...
		return self._read(MldUcastIPv6SourceList(self), None)
	...
	def MldJoinGroup(self, Arg1):
		"""Executes the mldJoinGroup operation on the server.

		Join Group

		Args:
			Arg1 (list(str[None|/api/v1/sessions/1/ixnetwork/topology])): This parameter requires a list of /topology/./mldMcastIPv6GroupList object references
	...
	def MldJoinGroup(self, Arg1, SessionIndices):
		"""Executes the mldJoinGroup operation on the server.
	...
	def MldLeaveGroup(self, Arg1, SessionIndices):
		"""Executes the mldLeaveGroup operation on the server.
