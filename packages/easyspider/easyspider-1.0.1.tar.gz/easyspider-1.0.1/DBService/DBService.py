...
# @Last Modified time: 2018-06-14 16:10:52
...
class MysqlService(DBService):
    ...
    def join_query_map(self, query_map, item_join_symbol="=", multi_join_symbol=" and "):
        ...
        """
        return multi_join_symbol.join(map(lambda item: '`%s`%s%s' % (self.mysql_escape(item[0]), item_join_symbol, item[1] and '"%s"' % self.mysql_escape(item[1]) or "null"), query_map.items()))
