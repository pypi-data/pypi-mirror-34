...
def produce_simulation_test_parameters(
    ...
    u"""Produce objects for testing Hamiltonian simulation.
...
@pytest.mark.parametrize(
        u'hamiltonian, time, initial_state, exact_state, order, n_steps, '
        u'algorithm, result_fidelity', [
...
def test_simulate_trotter_omit_final_swaps():
    ...
    assert (circuit_with_swaps.to_text_diagram(transpose=True).strip() ==
            (circuit_without_swaps.to_text_diagram(transpose=True).strip() + u"""
│        ×ᶠ─────────×ᶠ         ×ᶠ─────────×ᶠ
│        │          │          │          │
×ᶠ───────×ᶠ         ×ᶠ─────────×ᶠ         │
│        │          │          │          │
...
def test_simulate_trotter_bad_order_raises_error():
    ...
    with pytest.raises(ValueError):
        _ = simulate_trotter(qubits, hamiltonian, time, order=-1).next()
...
def test_simulate_trotter_bad_hamiltonian_type_raises_error():
    ...
    with pytest.raises(TypeError):
        _ = simulate_trotter(qubits, hamiltonian, time,
                                  algorithm=None).next()
    with pytest.raises(TypeError):
        _ = simulate_trotter(qubits, hamiltonian, time,
                                  algorithm=LINEAR_SWAP_NETWORK).next()
...
def test_simulate_trotter_unsupported_trotter_step_raises_error():
    ...
    class EmptyTrotterAlgorithm(TrotterAlgorithm):
        supported_types = set([openfermion.DiagonalCoulombHamiltonian])
    algorithm = EmptyTrotterAlgorithm()
    with pytest.raises(ValueError):
        _ = simulate_trotter(qubits, hamiltonian, time, order=0,
                                  algorithm=algorithm).next()
    with pytest.raises(ValueError):
        _ = simulate_trotter(qubits, hamiltonian, time, order=1,
                                  algorithm=algorithm).next()
    with pytest.raises(ValueError):
        _ = simulate_trotter(qubits, hamiltonian, time, order=0,
                                  algorithm=algorithm, control_qubit=control).next()
    with pytest.raises(ValueError):
        _ = simulate_trotter(qubits, hamiltonian, time, order=1,
                                  algorithm=algorithm, control_qubit=control).next()
