...
from api.exceptions import ServiceNoInstanceException, ServiceInnerException, IllegalArgumentException
...
def fd(fd_code):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}")
        r.raise_for_status()
        if len(r.content) > 0:
            return Munch(r.json())
        else:
...
def size(fd_code):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}/size")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def is_empty(fd_code):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}/empty")
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
...
def collect(fd_code):
    ...
    try:
        r = requests.get(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}/all")
        r.raise_for_status()
        if len(r.content) > 0:
            ll = r.json()
...
def query(fd_code, query_str, parameters={}):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}/query",
                          json=parameters, params={'query': query_str})
        r.raise_for_status()
        if len(r.content) > 0:
            ll = r.json()
            if isinstance(ll, list):
...
def add_datas(fd_code, result_list):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}", json=request)
        r.raise_for_status()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
        ficusServerGetter.change_new_server()
...
def update_datas(fd_code, result_list):
    ...
    try:
        r = requests.put(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}", json=request)
        r.raise_for_status()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
        ficusServerGetter.change_new_server()
...
def save_or_update_datas(fd_code, result_list):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}/upsert", json=request)
        r.raise_for_status()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
        ficusServerGetter.change_new_server()
...
def delete_all(fd_code):
    ...
    try:
        r = requests.delete(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}")
        r.raise_for_status()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
        ficusServerGetter.change_new_server()
...
def delete_query(fd_code, query_str):
    ...
    try:
        r = requests.delete(f"{ficusServerGetter.server_url}remote/fd-service/{fd_code}", params={'query': query_str})
        r.raise_for_status()
    except requests.exceptions.ConnectionError or requests.exceptions.Timeout:
        # 这里说明是网络连接失败
        ficusServerGetter.change_new_server()
...
def exists_fds(fds):
    ...
    try:
        r = requests.post(f"{ficusServerGetter.server_url}remote/fd-service/exists", json=fds)
        r.raise_for_status()
        if len(r.content) > 0:
            return r.json()
        else:
