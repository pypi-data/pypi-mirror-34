...
from .utils import save_week_report, get_datetime_with_scope_offset
...
class ClassWeekReport():
    ...
    def auto_save_db(self, class_id=None, offset=-1):
        self.start_dt, self.end_dt = get_datetime_with_scope_offset(scope='week', offset=offset)
    ...
    def _get_user_last_user_section_id(self, user):
        try:
            from bee_django_course.exports import get_user_course_section_list
            section_list = get_user_course_section_list(user)
            if section_list.exits():
    ...
    def save_report(self, c, type_str, user, mentor):
        ...
        save_week_report(class_id=c.id, year=self.year, week=self.week, type_int=type_int, start_date=self.start_dt,
                         ...
                         live_mins=live_mins,
                         feed_count=self._get_user_feed_count(user),
                         live_days=live_days,
                         live_count=live_count,
                         last_user_section_id=self._get_user_last_user_section_id(user))
