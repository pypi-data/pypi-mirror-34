...
from .encoders import CNN1D
from .decoders import tCNN1ds2
from text_autoencoder.layers.embedding import get_embedding


class VAECNN2tCNN(VAELearningManager, BaseVariationalAutoencoder):
    ...
    def __init__(
            ...
            n_steps: int = 10,
            latent_size: int = 50,
            encoder_input_dropout_ratio: float = 0.2,
            decoder_input_dropout_ratio: float = 0.2,
            encoder_filter_structures: List[Tuple[int, int, int]] = None,
            decoder_filter_structures: List[Tuple[int, int]] = None,
            l2_regularization: float = 1e-4,
            kl_lambda: object = None,
            max_grad_norm: float = 5.0,
            model_name: str = "VAECNN2tCNN",
            initialize_graph: bool = True,
        ) -> None:
        self.embedding_table = embedding_table.astype('float32')
        self.vocab_size, self.embedding_size = embedding_table.shape
        self.n_steps = n_steps
        self.latent_size = latent_size

        self.encoder_input_dropout_ratio = encoder_input_dropout_ratio
        self.decoder_input_dropout_ratio = decoder_input_dropout_ratio

        # encoder_filter_structures: filter_size, stride, output_channels
        if encoder_filter_structures is None:
            encoder_filter_structures = [(3, 2, 128), (3, 2, 256)]
        # decoder_filter_structures: filter_size, output_channels
        if decoder_filter_structures is None:
            decoder_filter_structures = [(3, 128), (3, 200)]

        self.encoder_filter_structures = encoder_filter_structures
        self.decoder_filter_structures = decoder_filter_structures
        ...
        if self.initialize_graph:
            super(VAECNN2tCNN, self).__init__()
    ...
    def build_graph(self):
        ...
        is_training_place = tf.placeholder(
            dtype=tf.bool,
            name=TENSOR_NAMES["IS_TRAINING_PLACE"],
        )
        encoder_input_dropout_place = tf.placeholder_with_default(
            0.0,
            shape=(),
            name="encoder_input_{}".format(
                TENSOR_NAMES["DROPOUT_PLACE"],
            ),
        )
        decoder_input_dropout_place = tf.placeholder_with_default(
            0.0,
            shape=(),
            name="decoder_input_{}".format(
                TENSOR_NAMES["DROPOUT_PLACE"],
            ),
        )
        ...
        with tf.variable_scope("encoder"):
            z_mean, z_std = CNN1D(
                ...
                input_dropout=encoder_input_dropout_place,
                output_size=self.latent_size,
                filter_structures=self.encoder_filter_structures,
                is_training=is_training_place,
            )
        ...
        with tf.variable_scope("decoder"):
            output_logits, _ = tCNN1ds2(
                input_=z,
                input_dropout=decoder_input_dropout_place,
                maxlen=self.n_steps,
                output_size=self.vocab_size,
                filter_structures=self.decoder_filter_structures,
                is_training=is_training_place,
            )
    ...
    def compute_loss(self, x, z_mean, z_std, decoder_output_logits, step):
        with tf.variable_scope("loss"):
            ...
            # VAE loss
            vae_loss = recon_loss
            # + self.kl_lambda(step=step) * kl_divergent_loss
    ...
    def _encode_on_batch(
            self,
            x_batch: np.ndarray,
            seqlen_batch: np.ndarray = None,
        ) -> List[Tuple[np.ndarray, np.ndarray]]:

        x_place = self.graph.get_tensor_by_name(
            name=TENSOR_NAMES["X_PLACE"] + ":0",
        )
        is_training_place = self.graph.get_tensor_by_name(
    ...
    def _train_on_batch(
        ...
        encoder_input_dropout_place = self.graph.get_tensor_by_name(
            name="encoder_input_{}:0".format(
                TENSOR_NAMES["DROPOUT_PLACE"],
            ),
        )
        decoder_input_dropout_place = self.graph.get_tensor_by_name(
            name="encoder_input_{}:0".format(
                TENSOR_NAMES["DROPOUT_PLACE"],
            ),
        )
        ...
        decoder_output_indices, \
            ...
            train_summary, _ = self.sess.run(
                ...
                feed_dict={
                    x_place: x_batch,
                    encoder_input_dropout_place:
                        self.encoder_input_dropout_ratio,
                    decoder_input_dropout_place:
                        self.decoder_input_dropout_ratio,
    ...
    def save(self, path: str):
        hyper_params = {
            ...
            "encoder_input_dropout_ratio": self.encoder_input_dropout_ratio,
            "decoder_input_dropout_ratio": self.decoder_input_dropout_ratio,
            "encoder_filter_structures": self.encoder_filter_structures,
            "decoder_filter_structures": self.decoder_filter_structures,
    ...
    def __del__(self):
        if self.initialize_graph:
            super(VAECNN2tCNN, self).__del__()
