...
# Andere Browser:
from ..unitraccgroups.utils import learner_group_id
from ..unitraccfeature.utils import AGB_UID

# Dieser Browser:
from .crumbs import register_crumbs
from .data import (REQUIRED_FIELDS, DD_MM_YYYY, ACTIVE_VALUES,
        VAT_PERCENTAGE,  # TODO: evtl. datumsabhängig in Datenbank ablegen
        PAYPAL_PAYMENT_STATES,
        PAYPAL2UNITRACC_STATUS,  # direkt textuelle Angabe
        OLD2NEW_STATUS,  # für Übergang 
        ACCEPTED_OR_BETTER,
        UNTOUCHED_ORDER_WHERE,
        AUTHOR_BRAIN_TO_DB, USER_MAIL_SUBJECT,
        TIMEOUT_MSGID,
        )
from .utils import (extract_float, make_amount_formatter,
        paypal_langcode, DEFAULT_CURRENCYCODE, normal_currency,
        sendMail,
        article_dicts,  # als nächstes für insert_backend_booking
        in_cart,
        make_secret,
        format_decimal, MF_KWARGS,  # Formatierung von Geldbeträgen
        all_user_names,
        )
from .exceptions import (UnitraccBaseException, BookingConfigurationError,
        )

# Unitracc-Tools:
from ...tools.forms import (tryagain_url, back_to_referer,
        )
from visaplan.tools.dicts import (
        updated, subdict, subdict_forquery,
        )
from visaplan.tools.debug import pp, log_or_trace
from visaplan.plone.tools.log import getLogSupport
from ...tools.hubs import make_hubs
from visaplan.tools.classes import WriteProtected

# Logging und Debugging:
gls_kwargs = {
    'defaultFromDevMode': 1,
    }
logger, debug_active, DEBUG = getLogSupport(**gls_kwargs)
lot_kwargs = {'debug_level': debug_active or 1,
              'logger': logger,
              'log_result': True,
              'log_args': True,
              }
...
from ...tools.context import getActiveLanguage
# ------------------------------------ ] ... Reparaturcode ]
...
class IBooking(Interface):
    ...
    def get_current_order_articles(self):
        """
    ...
    def insert_backend_booking(self):
        """
    ...
    def get_paypal_button(self, order):
        """
    ...
    def get_sending_mail_address(self, config=None):
        """
        Ermittle die sendende Mail-Adresse (Subportal-Eigenschaft)
        """

    def make_greeting(self, order):
        """
...
class Browser(BrowserView):
    ...
    def get_current_order_articles(self, sql=None, complete=None):
        ...
        """
        self.context.getBrowser('authorized').raise_anon()
        if sql is not None:
    ...
    def _get_order_articles_and_calculation(self,  # -- [ _goaac ... [
        ...
        for row in calculated:
            ...
            if row['step'] == 10:
                row['article_title'] = context.getAdapter('translate')(
                        '${percentage} VAT',
...
#... (hier evtl. noch Feintuning der MwSt-Angaben)
    ...
    def add_article(self):  # ------------------- [ add_article ... [
        ...
        getBrowser('authorized').raise_anon()
    ...
    def update_order(self, hub=None, info=None): # --------- [ update_order ... [
        ...
        with self.sql as sql:
            ...
            try:
                ...
                for dic in sorted(form.values(), key=attrgetter('number')):
                    dic = dict(dic)
    ...
    def insert_backend_booking(self):  # - [ i._backend_booking ... [
        ...
        context = self.context
        getAdapter = context.getAdapter
        getAdapter('checkperm')('Manage Orders')
        request = context.REQUEST
        form = request.form
        portal = getAdapter('portal')()
        ...
        retry_url = tryagain_url(request, form.keys())

        # ------------------------ ] ... insert_backend_booking ... [
        # TH: ist das nötig?
        sm = getAdapter('securitymanager')
        sm(userId='system')
        sm.setNew()
        hub, info = make_hubs(context)
        ...
        try:
            ...
            if (not form.get('lastname', None) and not form.get('company', None)):
                message(_('Please fill first and lastname or company'),
                        'error')
                return redirect(retry_url)
            ...
            if not user:
                message(_('Please use a valid user'),
                        'error')
                return redirect(retry_url)
            # prepare article
            ...
            with self.sql as sql:
                ...
                try:
                    ...
                    message = ('This object is already present in your '
                               'shopping cart.')
                    hub['message'](message, 'info')
                    return back_to_referer(request=request)
        ...
        with self.sql as sql:
            ...
            if status:
                ...
                if status:
                    ...
                    if not date_paid:
                    ...
                    elif not isinstance(date_paid, basestring):
                        ...
                        try:
                            ...
                            message("Unsupported date format (${raw})",
                                    'error',
                                    mapping=locals())
                            ok = False
            ...
            for key, value in form.items():
                ...
                if (field == "article_uid"
                    ...
                    try:
                        ...
                        message(_("Booking not possible. Incomplete record."
    ...
    def delete_article(self):
        ...
        try:
            ...
            message('No order and/or article ids specified',
                    'error')
            return back_to_referer(request=request)
        except Exception as e:
        ...
        if checkperm('unitracc: Manage Orders'):
            oid = form.get('set')
            go_kwargs = {
                'oid': order_id,
                }
        ...
        if not oid:
            ...
            message('No properties selected. Unable to delete.',
                    'error')
            return back_to_referer(request=request)
        ...
        with self.sql as sql:
            ...
            for order in self._get_orders(sql, **go_kwargs):
                ...
                if deleted:
                    message('The selected article has been successfully '
            ...
            # TODO: bessere Meldung 
            if not order:
                message('No access possible. The operation was interrupted.',
                        'error')
            return back_to_referer(request=request)
    ...
    def process_cart(self):
        ...
        with self.sql as sql:
            ...
            if form.get('formid', '') == formid:
                ...
                userbrain = getBrowser('author').getBrainByUserId(userid)
                ...
                for getKey, key in AUTHOR_BRAIN_TO_DB.items():
                    ...
                    if val is not None:
                        if key == 'country':
                            countrylist = getBrowser('voccountry')
    ...
    def process_order_register(self):
        ...
        with self.sql as sql:
            ...
            if not form.get('company'):
                if not form.get('firstname') or \
                   not form.get('lastname'):
                    not_filled = True
                    message(_('Please enter a name or a company'))
            ...
            try:
                ...
                message('Invalid data!',  # z. B. unbekannte Feldnamen
                        'error')
    ...
    def submit_order(self):
        ...
        if not form.get('booking_tac'):
            message('Please accept the terms and conditions.',
                    'error')
            redirect = request.RESPONSE.redirect
            return redirect(tryagain_url(request, form.keys()))
        ...
        with self.sql as sql:
            ...
            # Mail schicken
            # user = getBrowser('author').getByUserId(user_id)
    ...
    def check_access(self, web=False):
        ...
        if not form.get('formid'):
            msg = _('No access possible. The operation was interrupted.')
            message_ = True
            hack = True
    ...
    def save_payment(self):
        ...
        if not form.get('payment_type'):
            msgstr = _('Please choose a payment method.')
        ...
        with self.sql as sql:
            ...
            if not order['paypal_allowed']:
                ...
                if not ppi_rows:  # eher theoretische Möglichkeit
                    ...
                    message('PayPal payment is disabled!', 'error')
                    return context.restrictedTraverse('order_choose_payment_method')
    ...
    def paypal_finished(self):
        ...
        with self.sql as sql:
            try:
                ...
                logger.info('paypal_finished: Formularfelder %s', (dict_of_values.keys(),))
