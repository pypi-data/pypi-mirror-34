...
class Pipeline:
    ...
    def __init__(self, path, name):
        ...
        try:
            ...
            raise Exception ('Can\'t find the pipeline API version for pipeline %s' % self.__name__)
        version_diff = compare_version(api_version, PIPELINES_API)
        if version_diff != 0:
            raise Exception ('%s pipeline API version - %s - differs from '
                             'the supported API version - %s -' % (self.__name__, api_version, PIPELINES_API))
