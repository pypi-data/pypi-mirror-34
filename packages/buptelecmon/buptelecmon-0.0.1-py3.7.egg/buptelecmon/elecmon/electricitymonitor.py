...
import buptelecmon.elecmon.exceptions
...
class ElectricityMonitor(object):
    def __init__(self, username, password):
        ...
        self._logger = buptelecmon.logger.register(__name__)
        ...
        try:
            self.log_in()
        except buptelecmon.elecmon.exceptions.LoginFailed as e:
        ...
        if res.status_code == requests.codes.ok:
            ...
            if 'e' in response: # Login Response
                if response['e'] == '9999':
                    ...
                    raise buptelecmon.elecmon.exceptions.LoginFailed(response['m'])
            else: # Error Response
                if 'status' in response:
                    raise buptelecmon.elecmon.exceptions.RemoteFailed(
                        str(response['status'])+' '+response['name'], res.text
                    )
                else:
                    raise buptelecmon.elecmon.exceptions.RemoteFailed(
                        'The remote server has encountered an error.', res.text
                    )
        else:
    ...
    def _query(self, url, data=None):
        ...
        if res.status_code == requests.codes.ok:
            try:
                ...
                raise buptelecmon.elecmon.exceptions.RemoteFailed('The remote server has encountered an error.', res.text)
            ...
            raise buptelecmon.elecmon.exceptions.NeedLogin('Need to log in.')
        else:
            res.raise_for_status()
    ...
    def convert_partment(self, dormitory_list):
        ...
        for i in range(len(dormitory_list)):
            ...
            if found: # This partment name can be converted to partment id
                ...
                raise buptelecmon.elecmon.exceptions.PartmentNameNotFound('"'+partment+'" is not in the partment list.')
    ...
    def loop(self, dormitory_list, callback_function, params=None, time_interval=60):
        ...
        while True:
            try:
                self.convert_partment(dormitory_list)
                break
            except buptelecmon.elecmon.exceptions.RemoteFailed as e:
        ...
        while True:
            ...
            for dormitory in dormitory_list:
                ...
                if attempt > 3:
                    ...
                    self._logger.error(
                        'After 3 attempts we still can not get the data, the data points have been skipped. Please wait 60 seconds.'
                    )
