...
def log_call(func):
    def func_wrapper(*args, **kwargs):
        ...
        if len(bound_args.arguments) == 0 or not bound_args.arguments.get('self'):
            logger = logging.getLogger('log_call_default_logger')
        else:
            logger = getattr(bound_args.arguments.get('self'), 'logger', None) or \
                     getattr(bound_args.arguments.get('self'), '_logger', None) or \
                     logging.getLogger('log_call_default_logger')

        args_list = ["{}: '{}'".format(k, v) for (k, v) in bound_args.arguments.items()]
        args_list = args_list[1:]

        logger.debug(func.__qualname__ + "(" + ", ".join(args_list) + ")")
