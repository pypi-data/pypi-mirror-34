...
from apisdk.connection.Headers import *
from apisdk.connection.Connection import *
import apisdk.masking.Masking
...
class HttpConnection(Headers, Connection):
    ...
    def http_connection_request(self, logger):
        try:
            ...
            # Setting the response values to the Merchant Configaration object
            ...
            if not (self.request_type.upper() == GlobalLabelParameters.DELETE):
                mask_values = apisdk.masking.Masking.masking(r.content)
                message = mask_values
            ...
            if self.http_merchant_config.enable_log is True:
                ...
                logger.info("Response Code Message:   " + authenticationsdk.util.Utility.get_response_code_message(
                    r.status_code))
            ...
            if self.request_type.upper() == GlobalLabelParameters.POST or self.request_type.upper() == GlobalLabelParameters.PUT:
                if self.http_merchant_config.enable_log is True:
                    ...
                    masked_payload = apisdk.masking.Masking.masking(payload)
    ...
    def set_digest(self):
        ...
        set_digest_header = {GlobalLabelParameters.DIGEST: GlobalLabelParameters.DIGEST_PREFIX + encoded_digest}
