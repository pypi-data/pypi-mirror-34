...
from ..api import default_api_retry, ApiCaller
...
class BackendMetadataDB(BackendMixin, BaseMetadataDB):
    ...
    def query(self, query_text, **kwargs):
        ...
        result = ApiCaller.call(
            self._config, self._session, 'get', url, retry=default_api_retry(stop_max_attempt_number=self.max_query_retry), is_async=is_async, on_response=on_response_create(ctx))

        if isinstance(result, (tuple, list)):  # FIXIT: backend should start send dict as result instead of list
        ...
        if 'result_url' in result:
            sync_response_from_async = _CachedSessionWrapper.convert_result_to_response(result['result_url'], result['total_data_points'], result['total_size'], result['explicit_query'])
