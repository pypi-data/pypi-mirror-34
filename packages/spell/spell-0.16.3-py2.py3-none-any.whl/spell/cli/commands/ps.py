...
def prettify_status(run):
    ...
    elif run.status in ("failed", "complete", "killed", "stopped"):
        return status
...
def ps(ctx, raw, trunc, workspace):
    ...
    with api_client_exception_handler():
        ...
        if workspace is None:
            ...
            workspaces = workspace_client.get_workspaces_by_name(workspace)
            if len(workspaces) == 0:
                logger.error("No workspace found named %s", workspace)
                return
            runs = run_client.list_runs([str(ws.id) for ws in workspaces])
    ...
    for run in runs:
        ...
        run.ws_name = run.workspace.name if run.workspace else None
