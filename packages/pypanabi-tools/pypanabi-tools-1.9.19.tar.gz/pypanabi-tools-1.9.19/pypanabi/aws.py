...
class MySQL(object):
    def __init__(self, host, user, password, database, port, charset='UTF8', cursor_factory=pymysql.cursors.DictCursor, autocommit=True):
        """
        Constructor
...
class Athena(object):
    ...
    def get_results(self, query_id, next_token=None, max_results=100):
        """
        Get query results
        :param query_id: query identifier in Athena [String]
...
class S3(object):
    def __init__(self, bucket, profile=None, arn=None):
        """
        Constructor
    ...
    def copy(self, source_prefix, destination_prefix, destination_bucket_name=None, destination_profile=None,
        ...
        try:
            ...
            for obj in self.bucket.objects.filter(Prefix=source_prefix):
                ...
                suffix = sep.join([x for x in suffix.split(sep) if x.strip()])

                destination_key = '/'.join([destination_prefix, suffix])
...
class Kinesis(object):
    def __init__(self, aws_region, stream, shard_id, last_seq=None, arn=None, min_time_lag=5, max_stream_process_time_in_sec=600):
        """
        Constructor
        :param aws_region:
        :param stream:
        ...
        while True:
            ...
            try:
                ...
                if e.response['Error']['Code'] != 'ProvisionedThroughputExceededException':
                    raise e
                self._logger.error('Got ProvisionedThroughputExceededException - will sleep for 5 seconds')
                time.sleep(5)
                continue

            if records_response is None:
                self._logger.info('ShardIterator was closed.')
                break
            else:
                self._logger.info('Got {} records ({} minutes behind latest)'.format(len(records_response['Records']), round(records_response['MillisBehindLatest'] / (1000 * 60))))

            for record in records_response['Records']:
                if self._is_record_empty(record):
                    self._logger.info('Empty record')
                    break
            ...
            if not next_shard_iterator:
                self._logger.info('Shutting down stream, REASON: exhaust stream shard iterator.')
                break

            timelag = round(records_response['MillisBehindLatest'] / 1000)
            self._logger.info('Timelag {time}'.format(time=timelag))

            if timelag < self._min_time_lag:
                self._logger.info('Shutting down stream, REASON: stream events caught up with present.')
                break
            stream_processing_time = time.time() - time_streaming_start
            self._logger.info('Processing stream since {time}'.format(time=stream_processing_time))
            if stream_processing_time > self._max_stream_process_time_in_sec:
                self._logger.info('Shutting down stream, REASON: reach max streaming duration.')
                break
