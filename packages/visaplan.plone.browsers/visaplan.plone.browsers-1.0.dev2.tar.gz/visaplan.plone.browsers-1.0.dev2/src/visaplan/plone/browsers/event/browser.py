# -*- coding: utf-8 -*- äöü
from dayta.browser.public import *
...
# dieser Browser:
from .data import (NAECHSTE_10, GLEITENDES_QUARTAL, MANUAL_RANGE,
        HEUTE, GLEITENDE_WOCHE, GLEITENDER_MONAT, DEFAULT_RANGE,
        GLEITENDES_QUARTAL, GLEITENDES_JAHR,
        # hier aus Gründen der leichteren Testbarkeit: 
        get_range,
        )
from .utils import (calc_range, manual_range, same_day_next_month,
        range_choices,
        query_dict, today_00, shortened_query,
        )
...
class Browser(BrowserView):
    ...
    def desktop(self):
        ...
        with StopWatch('@@event.desktop',
            ...
            if not theid:
                theid = getBrowser('auth').getId()

            stopwatch.lap('Vorbereitungen')
            userid, group_ids = getBrowser('groupsharing'
