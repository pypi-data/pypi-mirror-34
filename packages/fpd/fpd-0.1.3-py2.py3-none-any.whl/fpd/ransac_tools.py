...
def ransac_im_fit(im, mode=1, residual_threshold=0.1, min_samples=10,
    ...
    fit = model.my_model(model.params, *args).reshape(im.shape)
    ...
    if plot:
        ...
        images = [im, masked_where(inliers==False, im), fit, cor_im]
        ...
        #f, axs = plt.subplots(1, 4, sharex=True, sharey=True)
        #_ = axs[0].matshow(im, cmap=cmap)
        #_ = axs[1].matshow(masked_where(inliers==False, im), cmap=cmap)
        #_ = axs[2].matshow(fit, cmap=cmap)
        #_ = axs[3].matshow(cor_im, vmin=vmin, vmax=vmax)
