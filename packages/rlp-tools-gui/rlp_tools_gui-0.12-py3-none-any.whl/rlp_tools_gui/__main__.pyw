    ...
    def run(self):
        try:
            ...
            for x in range(lastline,len(rustext)):
                if potlastid==rustext[x]:
                    additionalstr=rustext[x+4:]
                    del rustext[x+4:]
                    break
...
class ExampleApp(PyQt5.QtWidgets.QMainWindow, form.Ui_MainWindow):
    def __init__(self):
        ...
        if dst_path:
            self.DSTpo_path = os.path.join(os.path.dirname(__file__), r'translation\DST.po')
            transpath = os.path.dirname(__file__)+ r'/translation'
            try:
                my_file = open(self.DSTpo_path)
                my_file.close()
                # my_abs_path = self.DSTpo_path.resolve()
            except Exception as e:
                u = "https://raw.githubusercontent.com/CunningFox146/RLP/master/DST.po"
                urllib.request.urlretrieve(u, self.DSTpo_path)
            ...
            subprocess.Popen('explorer "'+transpath.replace('/','\\')+'"')
        else:
    ...
    def save_empty_strings(self):
        ...
        filename=os.path.join(os.path.dirname(__file__), 'translation\\lost_in_translation.po')
        if filename:
