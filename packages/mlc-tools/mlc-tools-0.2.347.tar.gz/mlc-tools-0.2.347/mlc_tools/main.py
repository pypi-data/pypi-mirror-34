...
class Generator:
    ...
    def __init__(self, configs_directory='', **kwargs):
        ...
        def get_bool(arg, default='yes'):
            value = get(arg)
            if value is None:
                return default if isinstance(default, bool) else default == 'yes'
            if not value:
                return False
        ...
        self.use_cache = get_bool('use_cache', True)
    ...
    def _parse(self):
        ...
        self.parser.hash = Hash(self.configs_directory)
        if self.use_cache:
            self.parser.hash.load(
                language=self.language,
                serialize_format=self.serialize_format,
                side=self.side,
                namespace=self.namespace,
                path_to_protocols=self.path_to_protocols,
                generate_intrusive=self.generate_intrusive,
                generate_factory=self.generate_factory,
                version=__version__)

        self.parser.set_configs_directory(self.configs_directory)
        self.parser.generate_patterns()
        self.parser.parse_dir(self.configs_directory, self.filter_code)
        self.parser.link()
    ...
    def generate_data(self,
        ...
        if self.data_directory:
            ...
            for class_ in self.parser.classes_for_data:
                if class_.is_storage:
                    classes.append(class_)
            self.data_parser = DataParser(classes, self.serialize_format, self.data_directory, self.filter_data, self.parser.has_changes)
            self.data_parser.flush(self.out_data_directory)
        Log.message('mlc data (format: {}) generate successful'.format(self.serialize_format))
