...
class SandboxDeploymentStateTracker:
    def __init__(self, logger: Logger, app_requests: List[AppRequestInfo],
        ...
        self._app_deployment_end_statuses = {app_request.name: None for app_request in app_requests}
        self._remaining_app_config_counts = {app_request.name: app_request.instances_count * 2
    ...
    def _set_is_deployment_ended(self):
        ...
        if self._is_deployment_ended():
            ...
            # with MessagingService(self._messaging_props, self._logger) as msg_service:

            mqstr = "{{\"SpaceId\":\"{SPACEID}\",\"SandboxId\":\"{SANDBOXID}\"," \
                  "\"DeploymentEndStatus\":\"{DEPLOYMENTSTATUS}\"}}"\
                  .format(SPACEID=self.space_id, SANDBOXID=self._sandbox_id, DEPLOYMENTSTATUS=end_status)

            #self._messaging_service.publish(mqstr)
            self._messaging_service.publish2('CSMSCommon.Model.Events:SandboxDeployed',
                                             {
