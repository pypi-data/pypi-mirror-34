...
def resource_optimize_and_deploy(
    ...
    :param list kube_resource_type_list:            list of strings containing the kubernetes resource
    ...
    pipeline resource_optimize_and_deploy --host community.cloud.pipeline.ai --user_id <YOUR-USER-ID> --resource_type model --name mnist --tag <YOUR-TAG-NAME> --resource_subtype tensorflow --runtime_list \[python,tfserving\] --chip_list \[cpu\] --resource_id <YOUR-RESOURCE-ID> --kube_resource_type_list \[deploy,svc,ingress,routerules\]

    Deploy all kubernetes resources on cpu and gpu chips served by tflite runtime::

    pipeline resource_optimize_and_deploy --host community.cloud.pipeline.ai --user_id <YOUR-USER-NAME> --resource_type model --name mnist --tag <YOUR-TAG-NAME> --resource_subtype tensorflow --runtime_list \[tflite\] --chip_list \[cpu,gpu\] --resource_id <YOUR-RESOURCE-ID> --kube_resource_type_list \[deploy,svc,ingress,routerules\]
...
def resource_upload(
    ...
    * Initialize training resource
    ...
    pipeline resource-upload --host community.cloud.pipeline.ai --user_id <YOUR-USER-ID> --resource_type model --resource_subtype tensorflow --name mnist --tag <YOUR-TAG-NAME> --path ./model
    ...
    # *********** resource-source-add ********************************
    print('Initialize training resource...')
    ...
    You can now Optimize, Deploy and start your model Live in Production
    with a default 0 traffic route using:

    UI:  https://%s
    
    CLI:  pipeline resource_optimize_and_deploy
     
    Example:
        
        pipeline resource-optimize-and-deploy --host %s --user_id %s --resource_type model --name %s --tag %s --resource_subtype %s --runtime_list \[%s\] --chip_list \[cpu\] --resource_id %s --kube_resource_type_list \[%s\]
    
     ''' % (host, host, user_id, name, tag, resource_subtype, runtime_list, resource_id, kubernetes_resource_type_list))
...
def model_archive_tar(
    ...
    with _tarfile.open(tar_path, '%s:%s' % (filemode, compression)) as tar:
        tar.add(model_path, arcname=_model_subdir_name, exclude=exclude, filter=_filter_tar)
        # tar.list()
