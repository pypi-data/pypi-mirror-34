...
class Util:
    ...
    def data_to_json(value, widget):
        if (widget.compress_data):
            return (base64.encodebytes(gzip.compress((json.dumps(value, default=Util.json_serial)).encode('utf-8'), compresslevel=9))).decode('utf-8')
        else:
    ...
    def multi_options_to_json(value, widget):
        res = []
        for opt in value:
            res.append([opt[0],(base64.encodebytes(gzip.compress(opt[1].encode('utf-8'), compresslevel=9))).decode('utf-8')])
        return res
