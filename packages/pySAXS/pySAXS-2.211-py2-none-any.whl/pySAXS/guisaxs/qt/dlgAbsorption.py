...
class dlgAbsorption(QtWidgets.QDialog):#,dlgAbsorptionui.Ui_absorption):
    ...
    def ConstructUI(self):
        ...
        self.lineXRayMeasuredTransmission.textChanged.connect(self.UpdateFormula)
    ...
    def UpdateFormula(self,verbose=False):
        ...
        if sum(self.ActiveAtomes)<=0.0:
            ...
            Thi=absorption.getThickness(self.ActiveFormula,\
                                        TrM,density,self.energy)
            #print Thi
            ...
            if self.verbose:
                        ...
                        self.printTXT("Energy : %6.2f (keV) " % self.energy)
                        self.printTXT("Density : %6.2f " % density)
                        self.printTXT("Compound Mu_en/rho  = %6.2f (cm2/g) "% MuRho)
