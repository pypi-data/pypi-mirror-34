        ...
        if sess is None:
            ...
            if save_dir is not None:
                if dilatation_filter is not None:
                    numpy.savetxt(fname='%s/dilatation' % save_dir,
                                  X=numpy.reshape(dilatation_filter, [-1]),
                                  header='%s: dilatation filter' % MobileNetV2.__name__,
                                  footer='shape: [%d, %d, %d, %d]' % dilatation_filter.shape)

                if depthwise_filter is not None:
                    numpy.savetxt(fname='%s/depth-wise' % save_dir,
                                  X=numpy.reshape(depthwise_filter, [-1]),
                                  header='%s: depth-wise filter' % MobileNetV2.__name__,
                                  footer='shape: [%d, %d, %d, %d]' % depthwise_filter.shape)

                if compress_filter is not None:
                    numpy.savetxt(fname='%s/compress' % save_dir,
                                  X=numpy.reshape(compress_filter, [-1]),
                                  header='%s: compress filter' % MobileNetV2.__name__,
                                  footer='shape: [%d, %d, %d, %d]' % compress_filter.shape)

                if residuals_filter is not None:
                    numpy.savetxt(fname='%s/residuals' % save_dir,
                                  X=numpy.reshape(residuals_filter, [-1]),
                                  header='%s: residuals filter' % MobileNetV2.__name__,
                                  footer='shape: [%d, %d, %d, %d]' % residuals_filter.shape)
                if bias is not None:
                    numpy.savetxt(fname='%s/bias' % save_dir,
                                  X=bias,
                                  header='%s: bias' % MobileNetV2.__name__,
                                  footer='shape: [%d]' % bias.shape)
