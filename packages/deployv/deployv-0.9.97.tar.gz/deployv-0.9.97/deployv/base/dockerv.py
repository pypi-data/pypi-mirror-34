...
class DockerV(object):
    ...
    def set_default_ports(self):
        ...
        """
        instance_ports = self.__config.get('ports')
        ...
        available_ports = [p for p in range(port_start, port_end) if p not in used_ports]
        ...
        if self._InstanceV__full_config.instance_config.get('instance_type') == 'develop':
            new_ports = {port: 0 for port, mapping in instance_ports.items() if
                         mapping not in available_ports}
            instance_ports.update(new_ports)
        ports = [key for key, val in instance_ports.items() if not val]
        if not ports:
            # There are no ports to map, try to use the ones specified
            return
        instance_ports.update(dict(zip(ports, available_ports)))
        return
    ...
    def _parse_ports(self, ports):
        """ Parses the ports received returned by the `get_ports_dict` method
        and returns a list of the host ports used by the containers. This is
        required because the ports returned by the `get_ports_dict` method can
        be either `int` or `list`.

        :param ports: dictionary of port bindings returned by `get_ports_dict`
        :return: List of integers representing the host ports used
        """
        res = []
        ...
        for port in host_ports:
            if not isinstance(port, list):
                continue
            sub_ports = [int(p.split(':')[1]) for p in port]
            res.extend(sub_ports)
            host_ports.remove(port)
        res.extend(host_ports)
        res = list(set(res))
        return res
