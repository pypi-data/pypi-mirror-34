...
class SwapNetworkTrotterAnsatz(VariationalAnsatz):
    ...
    def param_bounds(self):
        ...
        for param_name in self.param_names():
            if param_name.startswith(u'U') or param_name.startswith(u'V'):
                bounds.append((-1.0, 1.0))
            elif param_name.startswith(u'T') or param_name.startswith(u'W'):
    ...
    def operations(self, qubits):
        ...
        for i in xrange(self.iterations):
            ...
            def one_and_two_body_interaction(p, q, a, b):
                if u'T{}_{}'.format(p, q) + suffix in self.params:
                    yield XXYYGate(half_turns=self.params[
                              u'T{}_{}'.format(p, q) + suffix]).on(a, b)
                if u'W{}_{}'.format(p, q) + suffix in self.params:
                    yield YXXYGate(half_turns=self.params[
                              u'W{}_{}'.format(p, q) + suffix]).on(a, b)
                if u'V{}_{}'.format(p, q) + suffix in self.params:
                    yield cirq.Rot11Gate(half_turns=self.params[
                              u'V{}_{}'.format(p, q) + suffix]).on(a, b)
            yield swap_network(
            ...
            yield (cirq.RotZGate(half_turns=
                       self.params[u'U{}'.format(p) + suffix]).on(qubits[p])
                   for p in xrange(len(qubits))
                   if u'U{}'.format(p) + suffix in self.params)
    ...
    def default_initial_params(self):
        ...
        U_pattern = re.compile(u'U([0-9]*)-?([0-9]*)?')
        TWV_pattern = re.compile(u'(T|W|V)([0-9]*)_([0-9]*)-?([0-9]*)?')

        params = []
        for param_name in self.param_names():
            if param_name.startswith(u'U'):
