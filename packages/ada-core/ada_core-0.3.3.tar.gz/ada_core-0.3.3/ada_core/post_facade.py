...
class PostFacade(object):
    ...
    def postHandlerJob(self, input_data, json_config):

        if input_data is None or not input_data or not isinstance(input_data, Mapping):
    ...
    def _parseHandlerInput(self, input_data, handlerConfig):
        ...
        def parseSimpleInput(input_data, input_config):
            ...
            if key != 'data_entity':
                raise exceptions.ParametersNotPassed('The input currently must be data_entity')
            data_key = list(input_config.values())[0]
            if data_key not in input_data.keys():
